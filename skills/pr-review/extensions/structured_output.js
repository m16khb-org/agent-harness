...
export default function prReviewStructuredOutput(pi) {
  pi.registerFlag("pr-review-max-turns", {
    type: "string",
    description: "Reserve the final PR Review agent turn for structured output",
  })
  let turns = 0
  let submitted = false
  pi.on("turn_end", () => {
    turns += 1
    const maxTurns = Number.parseInt(String(pi.getFlag("pr-review-max-turns") || ""), 10)
    if (submitted || !Number.isInteger(maxTurns) || maxTurns < 1 || turns < maxTurns - 1) return
    const outputTool = pi.getActiveTools().find((name) => name.startsWith("submit_pr_review_"))
    if (outputTool) pi.setActiveTools([outputTool])
  })

  registerOutputTool(
    pi,
    "submit_pr_review_finder",
    "Submit PR Review finder result",
  ...
  registerOutputTool(
    pi,
    "submit_pr_review_verdict",
    "Submit PR Review verdict",
