...
Outputs (inside --out, default: <repo>/.issueops/issues/<issue>/review/<provider>-<n>/ when the
MR/PR names its issue (branch prefix `<issue>-…` or `#<issue>` in title/description), otherwise
<repo>/.issueops/tmp/pr-review/mr-<n>/ — both paths are gitignored working areas):
...
# 이 스킬이 게시한 리뷰 마커.
REVIEW_MARKER_RE = re.compile(r"<!--\s*pr-review-review\s+head=([0-9a-f]{7,40})")
REVIEW_MARKER_TOKENS = ("pr-review-review", "pr-review-finding")
...
def run(cmd: list[str], cwd: str | None = None, check: bool = True) -> str:
    try:
        ...
        raise SystemExit(f"command timed out after {CALL_TIMEOUT}s: {' '.join(cmd[:5])}... (remote slow? set PR_REVIEW_CALL_TIMEOUT)")
...
def default_out_dir(repo_dir: str, provider: str, meta: dict, n: int) -> Path:
    ...
    if issue is not None:
        return Path(repo_dir) / ".issueops" / "issues" / str(issue) / "review" / f"{provider}-{n}"
    return Path(repo_dir) / ".issueops" / "tmp" / "pr-review" / f"mr-{n}"
...
PACK_CAP_BYTES = int(os.environ.get("PR_REVIEW_PACK_CAP_BYTES", str(150_000)))
...
REFUTED_HISTORY = ".issueops/pr-review/refuted.jsonl"   # 레포에 커밋되는 팀 메모리 (review/ 디렉터리는 gitignore 됨)
