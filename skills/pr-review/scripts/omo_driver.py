#!/usr/bin/env python3
"""PR Review find+verify driver for Omo: `omo -p` agent fan-out on zai/glm-5.3-flash.
...
PR_REVIEW_OUTPUT_EXTENSION = Path(__file__).parents[1] / "extensions" / "structured_output.js"
KILL = 70
...
OUTPUT_TOOL_BY_KIND = {"finder": "submit_pr_review_finder", "verdict": "submit_pr_review_verdict"}
...
Your FINAL action must be exactly one submit_pr_review_finder tool call matching the schema below. Do not print the result as assistant text.
...
Your FINAL action must be exactly one submit_pr_review_verdict tool call with this object. Do not print it as assistant text:
...
def prepare_omo_inputs(a: dict) -> Path:
    ...
    input_dir = checkout / f".pr-review-input-{uuid.uuid4().hex}"
    input_dir.mkdir()
    try:
...
def normalize_pinned_model(provider: str, model: str) -> tuple[str, str]:
    if provider != PINNED_PROVIDER or model not in {PINNED_MODEL, PINNED_MODEL_REF}:
        raise ValueError(f"PR Review Omo driver is pinned to {PINNED_MODEL_REF}; got {provider}/{model}")
...
class OmoRunner:
    ...
    def _cmd(self, sid: str, thinking: str, permission_preset: str = "read-only",
        ...
        if output_tool:
            cmd.extend(["--extension", str(PR_REVIEW_OUTPUT_EXTENSION),
                        "--permission", f"{output_tool}=allow"])
            if max_turns is not None:
                cmd.extend(["--pr-review-max-turns", str(max_turns)])
