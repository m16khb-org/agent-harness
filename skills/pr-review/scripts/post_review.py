#!/usr/bin/env python3
"""Render and (optionally) post a PR Review review from findings.json to a GitLab MR or GitHub PR.
...
BRAND = "![pr-review](https://img.shields.io/badge/pr-review-inspection-312B4B?labelColor=C9BBF2)"


CALL_TIMEOUT = int(os.environ.get("PR_REVIEW_CALL_TIMEOUT", "90"))
...
def run(cmd: list[str], check: bool = True) -> str:
    try:
        ...
        raise SystemExit(f"command timed out after {CALL_TIMEOUT}s: {' '.join(cmd[:4])}... (remote slow? set PR_REVIEW_CALL_TIMEOUT)")
...
def render_gate_finding(f: dict) -> str:
    ...
    if f.get("evidence") and not m:
        parts += ["", "```", *f["evidence"][:8], "```"]
    parts += ["", f"<!-- pr-review-finding id={f['id']} -->"]
    return "\n".join(parts)
...
def level_disclosure(level: str) -> str:
    ...
    that claim is the whole reason a PR Review review is worth more than a one-pass bot's.
    """
    if level == "max":
...
def render_summary(ctx: dict, data: dict, posted: list[dict], skipped: list[dict], gate_md: str | None = None,
    ...
    L = [f"## PR/MR 리뷰 — {verdict_ko} (인라인 {n_inline}, 레벨 `{level}`)", "", (data.get("summary") or "").strip(), ""]
    if posted:
    ...
    L += [f"<sub>head `{head[:12]}` · {level_disclosure(level)} 잘못된 지적은 👎 + 이유를 남겨 주세요. 다음 리뷰 규칙에 반영합니다.</sub>",
          f"<!-- pr-review-review head={head} level={level} -->"]
    return "\n".join(L)
...
def post_gitlab(ctx: dict, posted: list[dict], bodies: dict, summary_body: str) -> tuple[list[dict], bool]:
    ...
    marker_ok = any(f"pr-review-review head={refs['head_sha']}" in (x.get("body") or "") for d in discs for x in d.get("notes", []))
...
def post_github(ctx: dict, posted: list[dict], bodies: dict, summary_body: str, verdict: str, allow_approve: bool = False) -> tuple[list[dict], bool]:
    ...
    if rid:
        ...
        marker_ok = f"pr-review-review head={refs['head_sha']}" in (got.get("body") or "")
        ...
        for f in posted:
            results.append({"id": f["id"], "remote_id": next((c["id"] for c in rc if f"pr-review-finding id={f['id']}" in c.get("body", "")), None),
                            "ok": True, "verified": any(f"pr-review-finding id={f['id']}" in b for b in ids_seen)})
