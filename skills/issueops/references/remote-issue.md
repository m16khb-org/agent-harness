...
Assign the created remote issue to the currently authenticated user. For GitHub, resolve the login with `gh api user --jq .login` and pass it to `gh issue create --assignee "$login"` or apply it immediately with `gh issue edit "$ISSUE_URL" --add-assignee "$login"` before linking the issue. For GitLab, resolve the concrete current user with `glab api user --jq .username` and pass it through the supported assignee flag. Create an issue-based MR with `glab mr create --related-issue <iid> --assignee "$username"`, whose `--assignee` takes usernames rather than numeric ids. The older issue-based `glab mr for` is deprecated by glab itself (`use 'glab mr create --related-issue <issueID>'`, verified 2026-08-27) and took a numeric id instead; do not start new work on it. Do not use placeholders such as `@me`.
...
| Assignee | `gh issue create --assignee` / `gh issue edit --add-assignee`. | GitLab concrete username through `--assignee`, `glab mr create --related-issue` included; never `@me`. The deprecated `glab mr for` took a numeric current-user id instead. |
...
The same VCS linking rules apply without a hook: an issue create/edit body must not contain a `Plan Link` section (on any provider) or a `Related Issues` section on GitLab (where related issues belong in native linked items per the table above). Issue/PR/MR create commands need a label flag and an assignee flag, including deprecated GitLab issue-based MR commands such as `glab mr for` and structured MCP `glab_mr_for`. When an active IssueOps cycle has `branch_prepare.base_branch`, PR/MR create must also expose a target/base branch matching that recorded parent work branch. This one is enforced before the remote write: command policy denies `gh pr create` / `glab mr create` with `pr_target_branch_required` when no `--base` / `-B` / `--target-branch` is present, and `pr_target_branch_mismatch` when the value differs from the recorded parent branch. The expected branch comes back as a `pr_target_branch_expected=<branch>` warning, so retarget to that value rather than guessing. Cycles without a prepared branch are not judged, so ordinary PRs outside IssueOps are unaffected. `--copy-issue-labels` and `--with-labels` count as label evidence for PR/MR create, including structured MCP-style tool input such as `flags.copy_issue_labels: true` or `flags.with_labels: true`, but they do not imply assignment; pass the current authenticated user through an assignee flag or structured field such as `--assignee`, `assignee`, or `flags.assignee`. GitLab placeholders such as `@me` are rejected, and `glab mr for` requires a numeric assignee id. Attach GitLab related issues with the issue links API, drop the Plan Link section, and include copied or explicit labels plus the current authenticated user assignee before retrying.
