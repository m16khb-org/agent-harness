...
Codex supervised startup uses an explicit per-attempt attestation; Claude and GJC do not. First run `codex --help` as a standalone observation and require `--dangerously-bypass-hook-trust`. Then run this exact source-checkout observation:

```bash
agent-harness issueops handoff codex-hooks-list --id <exact-cycle-id> --json
```

The helper accepts only a canonical bounded cycle id, selects the persisted worker root, resolves the installed Codex executable to an absolute target, and owns exact `<resolved-codex> -C <worker> app-server --stdio` argv plus cwd, the `codex_hooks_list_v1` environment allowlist, a 15-second process-group timeout, unique redacted process audit, and initialize/initialized/hooks-list JSONL. It exposes no executable, worker, method, stdin, timeout, or config input and fails closed on protocol order, JSON-RPC errors, foreign IDs, cwd mismatch, raw depth/node growth, secret-bearing key/value pairs, or final encoded-output violations. Direct app-server and synthetic/unbound `write_stdin` remain blocked: Codex continuation writes do not emit PreToolUse, so a caller-controlled session cannot enforce a method denylist. Do not call `config/batchWrite` or trust hooks from this recipe. Attest only when the response has exactly the requested cwd, warnings and errors are both empty, required SessionStart and PreToolUse command hooks are enabled, and every untrusted or modified entry is the exact generated agent-harness hook command for the current installed binary. Any unrelated untrusted/modified entry or missing ownership hook fails closed. Record the reviewed keys, `currentHash` values, source paths, binary target, and returned audit id without copying secrets. Explicit nonsecret Orca environment-key allowlist: never dump broad ORCA-prefixed env output or use prefix filtering for identity probes. Allow only explicitly named nonsecret keys such as `ORCA_TERMINAL_HANDLE`, `ORCA_TAB_ID`, and `ORCA_WORKTREE_ID`, and never record secret values in tests, docs, logs, or evidence.

Preview `handoff start` without confirmation first. For Codex it must report `codex_hook_trust_bypass_required=true` and `codex_hook_trust_bypass_attested=false`. After the review above, run a second no-confirm preview with every delivery option plus `--allow-codex-hook-trust-bypass`; require `codex_hook_trust_bypass_attested=true` and record the reviewed context hash from that final attested preview. Preview returns `context_sha256`. Final confirmed start must add `--expected-context-sha256` with the exact final attested preview hash plus `--confirm`; all delivery options remain identical. The confirmed start recomputes the sealed context and fails closed before any terminal, task, dispatch, or journal mutation when the submitted hash is missing, malformed, or differs from the freshly recomputed sealed context — including post-preview source drift and delivery-option drift. Confirm must never introduce a delivery option absent from that final preview. The adapter then launches only that Codex worker with `codex --dangerously-bypass-hook-trust`. Omit the flag for Claude, GJC, inline mode, and any unreviewed Codex attempt. This optional additive field remains ContextVersion 1 for legacy retry compatibility. Retry preserves the prior delivery options as defaults and clears the per-attempt attestation; an unsealed retry may explicitly replace those defaults before the next preview when the prior attempt has durable retry cleanup receipts. The coordinator must review and attest the resulting complete packet again. General automatic trust probing remains issue #17.
...
agent-harness issueops handoff start \
  ...
  --stop-condition "owner may push only the exact branch and create a draft PR/MR; stop before merge, cleanup, force-push/history rewrite, head mismatch, or scope expansion" \
  --result-format "published final head, changed files, Turing report, verification, and durable draft PR/MR artifact" \
