...
| Branch / Worktree Prep | Create the provider-linked branch, then choose `issueops worktree prepare --orchestrator auto\|orca\|inline`; link the plan, run `worktree prepare-tools`, and record compatibility review plus `execution_decision` before `implement`. | Worktree guard blocks source-checkout mutation and wrong linked-worktree targets; supervised ownership blocks mutation until the fresh worker claims. | Base branch is user-selected/unclear/conflicting, dependency/config linking has secret risk, or external mutation recovery is ambiguous. |
