...
In an ownership handoff, the exact lifecycle ID selects one isolated cycle even when several active cycles share the source checkout. The acknowledged native owner performs the remaining IssueOps gates, implementation, MR/PR publication, and `agent-harness issueops handoff complete` from the canonical worker root. Direct `orca` controllers and human cleanup remain source-session responsibilities.
...
6. Worktree selection, plan, and tool preparation: after creating the provider-linked branch and approving the design, the generic recipe is `agent-harness issueops worktree prepare --id "$ISSUEOPS_ID" --orchestrator auto --json`. Inspect `resolved_mode`; when it is `orca`, load `references/orca-handoff.md` and create a fresh owner through the ownership-transfer path. A pre-mutation `auto` probe failure returns the unchanged inline path with no handoff record. Never pass `--orchestrator inline` unless the current user explicitly requested inline execution or a documented recovery procedure requires it. If the user requests a full handoff, the source session stops work on that exact cycle after dispatch, while remaining free to research and create other cycles. The owner performs all subsequent exact-ID gates and implementation inside the worker root. Produce and link the issue-based implementation plan under the target repo's planning convention, then run `agent-harness issueops worktree prepare-tools --id "$ISSUEOPS_ID" --json` or MCP `issueops_prepare_worktree_tools` from the authenticated owner/preparation session.
...
- Ownership cleanup authority: `handoff complete` stops at `cleanup_pending_human_decision`. After the human merges, any fresh authenticated exact-source session may preview the inventory and receive explicit cleanup approval. The completed owner cannot approve cleanup. Record only the current ordered receipts after each operation is observed; a stale receipt never authorizes worktree removal.
...
Load `references/orca-handoff.md` for a full Orca transfer and `references/cleanup-state.md` for its terminal boundary. Workspace provisioning before ownership transfer is mandatory: prepare the worker, complete plan/gates, make a plan-only commit, preview/confirm start, then let the owner claim and acknowledge context. A prep-only linked worker may update its exact cycle by lifecycle ID before ownership dispatch. The source main worktree remains available before, during, and after handoff for unrelated work.

Fence only the canonical worker root, exact cycle ID, native owner, or persisted Orca resource. Exact-ID routing takes precedence over source-wide inference, so parallel cycles remain independent. Owner orientation grants post-handoff authority for the selected cycle, and `handoff complete` moves to `cleanup_pending_human_decision` with no automatic cleanup.
