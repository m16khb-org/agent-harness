...
| Branch / Worktree Prep | Create the provider-linked branch, then run `issueops worktree prepare --orchestrator auto`; link the plan, run `worktree prepare-tools`, and record compatibility review plus `execution_decision` before `implement`. Explicit inline is only the authorized exception documented below. | Worktree guard blocks source-checkout mutation and wrong linked-worktree targets; supervised ownership blocks mutation until the fresh worker claims. | Base branch is user-selected/unclear/conflicting, dependency/config linking has secret risk, or external mutation recovery is ambiguous. |
...
6. Worktree selection, plan, and tool preparation: after creating the provider-linked branch and approving the design, the only generic recipe is `agent-harness issueops worktree prepare --id "$ISSUEOPS_ID" --orchestrator auto --json`. Inspect `resolved_mode`; when it is `orca`, load `references/orca-handoff.md` and create a fresh worker through the supervised path. A pre-mutation `auto` probe failure returns the unchanged inline path with no handoff record. Never pass `--orchestrator inline` unless the current user explicitly requested inline execution or a documented recovery procedure requires it. Absence of an explicit handoff request is not authorization for inline, and coordination cost or implementation simplicity is not a valid inline reason. Explicit inline also requires `--inline-reason user-requested|recovery`; keep it in the exceptional compatibility/recovery section of `references/worktree-context.md`, not a generic recipe. If the user requests a full handoff, the coordinator must stop implementation after dispatch and wait for the fresh worker result. Supervised dispatch seals coordinator and worker mailbox recipients separately from refreshable terminal control, and completed finish uses the durable automatic best-effort projection contract in that reference—never a manual worker shell send. In the resulting worktree, produce and link the issue-based implementation plan under the target repo's planning convention, then run `agent-harness issueops worktree prepare-tools --id "$ISSUEOPS_ID" --json` or MCP `issueops_prepare_worktree_tools`. This step persists dependency readiness and supported install action on the IssueOps record.
...
- `references/orca-handoff.md`: optional Orca mode selection, coordinator/worker ownership commands, crash-safe exact-one recovery, and worker stop boundary.
...
- Template before remote writes: render remote issue and child-task bodies through `issueops remote render-template` or their `create-* --template/--body-file` forms. For an accepted supervised handoff, only the sealed source coordinator session may accept or publish; push only the exact accepted `FinalHead`, persist `issueops handoff publish --confirm`, then create the PR/MR through the shared CLI/MCP `issueops remote create-pr` claim wrapper with an already-rendered literal `--body`; PR/MR `--body-file` and direct provider create are forbidden. Any durable `remote_create_claim` blocks ordinary verification and automatic retry. Only the exact source coordinator native session sealed at handoff start may run `issueops remote reconcile-create`; mailbox text is routing only. Reconcile searches bounded exact-project+head candidates without base prefiltering, requires a non-null provider array, validates publication before the probe and again before finalize/approved zero-clear, adopts exactly one live verified candidate whose base, source project, canonical title, and rendered-body hash match the claim, and retains unknown on zero without explicit approval plus authoritative proof, multiple candidates, force-push/fingerprint drift, mismatch, or transport ambiguity. A genuine accepted raw-v5 receipt without `coordinator_session` remains guarded and byte-identical until the exact source coordinator explicitly uses `handoff publish --approve-legacy-coordinator-seal --confirm`; that locked transition seals the current native identity and re-attested v6 receipt together. Privileged MCP handoff tools match exact agent-harness names, never a foreign namespace suffix. Confirmed writes canonicalize supervised title/body once and fail closed on a missing or stale receipt, nested/cyclic URL rewrite or any unlocked effective Git config origin, effective push-target/provider/branch/ref/SHA drift, invalid project selector, critical template validation, missing label/assignee, Korean artifact failure, and target/base mismatch.
...
- Supervised cleanup authority: failed/cancelled Orca handoffs require a durable `approve-cleanup` disposition before task or terminal cleanup. Record `task_terminal`, then `terminal_quiescent`. That receipt is a stale snapshot and never authorizes raw worktree removal; stop at an explicit user cleanup boundary, then only verify and record `worktree_removed` after external/manual deletion with complete canonical inventory. Accepted handoffs cannot use cleanup to bypass the publish boundary.
...
- `worktree_tools_prepared` / `worktree_dependencies_ready`: run `issueops worktree prepare-tools` or MCP `issueops_prepare_worktree_tools` after linking the plan. If the package manager requires manual dependency reuse, symlink, copy, or install, do that in the linked worktree and rerun prepare-tools until the durable evidence is ready.
