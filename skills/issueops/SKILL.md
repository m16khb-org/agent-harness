...
- Template before remote writes: render remote issue and child-task bodies through `issueops remote render-template` or their `create-* --template/--body-file` forms. For an accepted supervised handoff, push only the exact accepted `FinalHead`, persist `issueops handoff publish --confirm`, then create the PR/MR through `issueops remote create-pr` with an already-rendered literal `--body`; PR/MR `--body-file` and direct `gh pr create`/`glab mr create` are forbidden. Confirmed writes fail closed on a missing or stale publish receipt, provider/branch/ref drift, critical template validation, missing label, missing assignee, Korean artifact failure, and target/base branch mismatch.
