...
The exact lifecycle ID selects one isolated cycle even when several active cycles share the source checkout. Before an Orca-capable prepare, author the approved child plan in a coordinator temporary file outside the source checkout and stage it with `issueops artifact stage --name plan`; `parent_plan_path` is never child-plan evidence. Run `agent-harness issueops execution prepare --id "$ISSUEOPS_ID" --mode auto` as a preview, then confirm the identical request. A direct holder continues in the canonical worktree. Orca prepare materializes the staged plan inside the canonical child worktree, persists that durable `plan_path`, seals its digest, and only then launches an owner. The owner verifies the sealed issue/context/plan digests and claims the exact generation from its private token file. The active holder implements, verifies, publishes, and calls `issueops execution complete`; merge and destructive cleanup remain separate human-authorized operations. GitHub Orca preserves the #176 ordering: `branch prepare` (base SHA only) → `artifact stage --name plan` → `execution prepare --mode orca` → GraphQL `createLinkedBranch` with `oid=sealed base SHA` → `branch prepare --link-verified`. `gh issue develop` is forbidden on this path because it can link the remote branch at a later base-branch HEAD.
...
- `branch_prepare` / `branch_link_verified`: normally run `issueops branch prepare` only after provider-visible branch evidence exists. GitHub Orca is the exception: record the matching GitHub issue identity and exact base SHA without `--link-verified`, stage the plan, run Orca prepare so its branch remains local-only, then call GraphQL `createLinkedBranch` with the sealed SHA as `oid` and the same branch name. Rerun `branch prepare --link-verified` from the claimed owner before linking a legacy plan or implementing. Do not substitute `gh issue develop`. The branch must start with the issue/task number and a hyphen.
