...
Codex supervised startup requires a fresh per-attempt attestation; it is not an ownership-guard bypass. Observe `codex --help` and require `--dangerously-bypass-hook-trust`, then run `agent-harness issueops handoff codex-hooks-list --id <exact-cycle-id> --json` from the exact source checkout. The bounded adapter owns the persisted worker cwd, exact `codex -C <worker> app-server --stdio` argv, initialize/initialized/hooks-list JSONL, timeout, and output limits. Direct app-server and caller-controlled `write_stdin` remain blocked because Codex does not emit PreToolUse for continuation writes. Proceed only when the exact cwd matches, warnings and errors are both empty, required SessionStart and PreToolUse command hooks are enabled, and every untrusted or modified entry is the exact generated agent-harness command for the installed binary. Any unrelated entry fails closed; never call `config/batchWrite` from this recipe.
...
Completed finish uses the IssueOps automatic best-effort projection. It persists the submitted result and projection intent in the same cycle lock, then performs at most one argv-only `worker_done` call outside the lock. The payload uses the sealed historical worker mailbox—not the refreshable live terminal—as sender, plus the sealed coordinator recipient, exact persisted task and dispatch, committed changed-file list, absolute in-worker report path, final head, host/attempt identity, nonempty subject, and three-sentence body. Success or terminal diagnostic remains durable; crash, timeout, malformed output, and ambiguity are never automatically retried. The manual shell `worker_done` is blocked.

Coordinator accept, publish, and remote-create reconciliation are native-session operations, not mailbox operations: CLI/MCP payload host/session/optional-agent and source cwd must exactly match both the sealed coordinator identity and the live hook event. Publication locks every effective Git config file origin before target resolution/push/readback; reconciliation consumes a bounded non-null exact-project+head candidate array and leaves base/title/body/source matching to the shared durable-claim verifier.

A genuine accepted schema-v5 publication predates the coordinator native-session field. It remains a bounded invalid closed-handoff authority in lifecycle scans and cannot be silently upgraded. Re-attestation requires the exact source coordinator's current native event plus explicit `approve_legacy_coordinator_seal`; a rejected attempt preserves raw bytes, while the successful locked target/local/remote pass atomically seals identity and v6 receipt. Privileged handoff MCP tools use exact bare or `mcp__agent_harness__` names; suffix collisions are never authority.

After verification and immediately before `handoff finish` triggers automatic projection, the worker performs a bounded current-task inbox check. Select only messages with the numeric `sequence`, exact `taskId` and `dispatchId`, and sender and recipient direction; process every newly arrived current-task `status` or `escalation` through the observed maximum sequence. If a message changes the result, repeat any affected verification and commit before finish, then run the fence again. Hooks may only observe, block, or relay this boundary and must never execute the workflow.
