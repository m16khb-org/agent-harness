...
6. Native install, update, and verification are standalone. External tools are not installed or required by agent-harness.
...
| **`engelbart`** | Douglas Engelbart (1925–2013) | Pioneered interactive computing, the mouse, hypertext, collaborative editing, and the "augmentation of human intellect" vision demonstrated in 1968's Mother of All Demos. | Converts noisy meeting records into linked team memory: Korean minutes, Canvas-ready decision/action boards, index rows, correction maps, and transcript audit appendices that help a team act after the meeting. |
...
6. native install, update, verification은 독립 실행 가능합니다. 외부 도구는 agent-harness가 설치하거나 요구하지 않습니다.
...
| **`engelbart`** | 더글러스 엥겔바트 (1925–2013) | 대화형 컴퓨팅, 마우스, 하이퍼텍스트, 공동 편집, "인간 지능 증강" 비전을 개척했고 1968년 Mother of All Demos에서 이를 시연했다. | 시끄러운 회의 기록을 연결된 팀 기억으로 바꾼다: 한국어 회의록, Canvas-ready 결정/액션 보드, 인덱스 row, 보정 맵, 원문 감사 부록을 만들어 회의 이후 팀이 실행할 수 있게 한다. |
