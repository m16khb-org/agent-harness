...
비즈니스 로직의 실제 404/403/409 가능성과 OpenAPI 누락 여부는 정적 테스트만으로 신뢰 있게 판정하지 않는다. 정적 테스트는 후보 파일 선택, `--all` wiring, prompt contract, MCP schema 같은 배선을 검증하고, 실제 API 문서 품질 판정은 `issueops api-doc review`/MCP `api_doc_review`가 렌더한 prompt/schema를 host agent가 수행한 뒤 결과 JSON을 `--result`/`result_file`로 기록해 수행한다.

리뷰 프롬프트는 Business Logic Error Contract Evidence 섹션을 자동 번들한다(service throw 지점, `ClientProxy` → `@MessagePattern` hop, exception filter status 매핑). 증거가 없으면 host agent가 diff만으로 service 계층 에러를 볼 수 없으므로, 이 번들이 마이크로서비스 에러 계약 누락 방지의 핵심이다. `cmd/issueops/apidoc/dogfood` 픽스처가 증거 계약을 회귀 테스트로 고정한다.

### Dogfooding standard

`cmd/issueops/apidoc/dogfood`는 NestJS 마이크로서비스 2개(api-gateway + orders-service)로 구성된 fixture를 materialize하고 시드된 결함 ground truth(S1–S9 정적, E1–E5 리뷰 증거)에 대해 recall을 측정한다. 기준:
