...
Sub-agent 계획은 아래 slug 중 하나, 기대 이득, 알려진 tradeoff, 그리고 그 tradeoff에도 불구하고 net-positive인 이유를 implementation plan 또는 durable evidence(`issueops decision add`)에 함께 기록해야 한다. slug 목록은 이 문서가 정규 소유자다. 전용 결정 명령과 MCP decision tool은 제거됐으므로 문서가 곧 계약이다.
...
- **issueops 적용:** Implementation Planning Phase 1 (Ground)의 explorer/librarian subagent. 대규모 코드베이스 패턴 검색.
...
- **issueops 적용:** IssueOps implement phase의 worktree 기반 작업.
...
- **issueops 적용:** 검토 필요 — 현재 미구현. Claude Code 전용 기능.
...
- **issueops 적용:** Verified Execution Final Quality Gate의 Reviewer. IssueOps pr phase의 reviewer agent.
...
- **issueops 적용:** Implementation Planning Further Review 옵션. 복수 reviewer의 합의.
...
- **issueops 적용:** Implementation Planning Research intent의 "fan out exploration subagents."
...
- **issueops 적용:** 규모가 크고 자연스럽게 분해되는 마이그레이션·audit 작업에만 제한적 사용.
...
- **issueops 적용:** Von Neumann의 Agent Categories (quick/deep/visual-engineering).
...
- **issueops 적용:** Von Neumann의 read-only explorer subagent. Shell·write 권한 없는 탐색 전용 worker.
...
- **issueops 적용:** Implementation Planning(planner)와 Verified Execution(executor)의 분리. 이미 구조적으로 구현됨.
...
- **issueops 적용:** 현재 issueops 범위 밖 (코딩 에이전트 하네스). 미래 multi-domain 확장 시 검토.
...
| 안전·되돌림·정렬 결정 | 메인 에이전트의 판단 책임 | issueops CAUTIONS: "main agent owns safety, reversibility, user-intent alignment" |
| Sub-agent 중첩 (nesting) | 복잡도 폭발, 무한 루프 위험 | Claude Code: Plan subagent prevents nesting |
| 메인·서브 간 중복 작업 | Anti-Duplication Rule 위반 | Implementation Planning SKILL.md: "DO NOT perform the same search yourself" |

---

## Verified Execution 스킬에의 적용
