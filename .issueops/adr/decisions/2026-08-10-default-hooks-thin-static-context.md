...
- upgrade는 알려진 lifecycle event에서 issueops group만 제거하고 co-resident third-party group과 관련 없는 host 설정을 보존한다.
- IssueOps CAS, lease, verification, publication authority는 `issueops ...` CLI/MCP에만 남는다. default hook output을 owner authority나 enforcement evidence로 해석하지 않는다.
