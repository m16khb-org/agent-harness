...
- Kind: `adr`
- Source: `.issueops/issues/_unnumbered/pipe-pressure-and-session-conflict.md`
...
- Decision:
  ...
  - Add `issueops doctor` `pipe_capacity_bytes` plus a `pipe_capacity` check. Capacity below 8192B emits `pipe_capacity_degraded` as a warning with the CAUTIONS 2026-07-09 runbook pointer.
...
- Evidence:
  ...
  - CLI test-helper sweep in `cmd/issueops/*`
  - `internal/core/doctor/checks.go`, `doctor.go`, and `pipe_capacity_test.go`
  - Evidence files under `.issueops/evidence/pipefix-*.txt`
- Alternatives / rejected options:
  ...
  - Immediately folding the existing issueopsapp helper into `internal/testsupport` — rejected because it already uses the safe concurrent-reader pattern and was explicitly out of the sweep scope.
