...
1. On a repo with legacy flat `.issueops` family roots and no modular
...
issueops is an open-source library applied to many projects; its
...
2. Curated AGENTS.md header rule: when AGENTS.md already opens with its
   ...
   template is not prepended. Only the managed `ISSUEOPS` marker block
