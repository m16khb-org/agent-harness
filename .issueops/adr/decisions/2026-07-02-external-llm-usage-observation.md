...
- Consequences: Usage history accumulates as state records for future pattern aggregation (P1-2) and long-loop stability metrics (P2-3). Tests exercising the LLM path through core root must isolate ISSUEOPS_STATE_DIR (llmeval helper updated).
- Evidence:
...
- Alternatives / rejected options:
  ...
  - token_usage compare regression rules now — deferred until real baselines accumulate (design-review trim)
