...
1. **unlazy 게이트 파일 형식을 그대로 채택한다.** `- [ ] G1: outcome` +
   ...
   `.issueops/gates/<scope-slug>.md`다. IssueOps는 병렬 worktree merge
   충돌을 피하고 artifact ownership을 분리하기 위해
   `.issueops/issues/<provider-issue-number>/gates.md`를 명시한다. 기존
   `.issueops/gates/issue-<n>.md`, root `GATES.md`, `gates/*.md`는 읽기
   호환 경로로 남는다.
...
4. **CLI와 MCP가 같은 contract DTO를 공유한다.** `issueops gates
...
5. **IssueOps PR readiness에 게이트를 합성한다(gatesgate).** worktree에서
   canonical `.issueops/issues/<n>/gates.md`와 호환 gate ledger를 찾고,
...
6. **크로스 capability adapter edge는 만들지 않는다.** gates adapter의 policy
   ...
   root(issueopsapp)에서만 배선한다(audit/loopgate 패턴). legacy adapter edge
...
- 서로 다른 IssueOps 사이클은 서로 다른 `.issueops/issues/<number>/gates.md`를 커밋하므로,
