...
- Summary: Add a durable `issueops loop` state machine, keeping the harness as a state/gate recorder rather than a scheduler or verifier.
...
- Decision:
  ...
  - Keep partial-verification policy normative in `.issueops/TESTING.md`; skills only point to that source.
- Consequences: Golden contracts include the loop command/tool list. Loop state lives in user-state `loop/` and is diagnosed by `issueops doctor`.
- Evidence:
  - `internal/core/looprun` state machine and tests
  - `internal/core/issueops_facade.go` strict readiness `loop_incomplete:` gate
  - `cmd/issueops/testdata/{usage.golden.txt,mcp_tools.golden.json,response_contracts.golden.json}` regenerated after CLI/MCP wiring
