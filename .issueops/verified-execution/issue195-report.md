...
- 이슈: https://github.com/m16khb-org/issueops/issues/195
...
- PR: [#216](https://github.com/m16khb-org/issueops/pull/216)
- 원격 CI: [push run 30709143814](https://github.com/m16khb-org/issueops/actions/runs/30709143814), [pull_request run 30709170374](https://github.com/m16khb-org/issueops/actions/runs/30709170374) 모두 성공

## 결과

원격 PR/MR 최초 생성과 `remote_pr_create` 복구를 하나의
`issueopspublication` contract/domain/application/inbound/outbound vertical로
이전했다. `cmd/issueops/issueopsapp`만 provider, raw CAS bridge, live verifier를
...
go test ./internal/core/issueops ./internal/adapter/inbound/issueopspublication ./internal/architecture ./cmd/issueops/issueopscli/executioncmd ./cmd/issueops/mcpcli -count=1
...
go test ./internal/core/lifecycle ./cmd/issueops/hookcli -count=1
go test -race ./internal/core/lifecycle ./cmd/issueops/hookcli -count=1
go vet ./internal/core/lifecycle ./cmd/issueops/hookcli
  (2026-08-01T16:52Z, Codex structured workdir hook regression 포함, 모두 exit 0)
./bin/issueops hook pre-tool-use --host codex --enforce-worktree --json
  (2026-08-01T16:49Z, 실제 source cwd + canonical tool_input.workdir + active lease payload decision=allow)
```

## 문서와 범위

- `.issueops/ARCHITECTURE.md`에 completed publication vertical과
  issueopsapp-only composition/fail-closed routing을 반영했다.
- `.issueops/OPERATIONS.md`에 CLI create와 CLI/MCP reconcile의 shared
  publication handler 운영 계약을 반영했다.
- `.issueops/TESTING.md`에 raw-byte differential, lock-free external call,
  CLI/MCP/AST ratchet 게이트를 반영했다.
- `.issueops/CONVENTIONS.md`를 검토했다. 기존 capability-local package와
