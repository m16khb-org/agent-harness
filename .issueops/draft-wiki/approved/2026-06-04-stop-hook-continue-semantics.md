...
source: "issueops investigation 2026-06-04"
...
Observed 2026-06-04 in issueops: the `--enforce-numbered-next-actions`
Stop hook block branch sent `continue:false`, so the agent halted instead of
presenting the required `선택지:` choices. Fixed in
`cmd/issueops/hook_user_prompt.go` by switching the block branch to
