...
| 9 | golden contract gate exercises none of 4 newest tools + ~14 others | `issueopsapp/response_contract_{mcp,cli}_snapshot_test.go` | P2 | medium | test-gap | Extend both snapshot builders through domain-review/compat/ai-slop/feedback/regress/set_phase/status + cleanup/remote; regen goldens (min: cover the 4 ledger tools) |
...
| 11 | design-review has no standalone routing hint — adversarial design/plan review is a dead trigger outside IssueOps | `hookprompt/rules.go` (no design-review entry); `skills/design-review/SKILL.md` | P2 | quick | routing | Add a `PrioritySecondary` design-review rule keyed on devil's-advocate / over-engineered / second-system / 과설계 / 계획 검토, Reason noting sub-agent dispatch |
| 12 | meeting-notes frontmatter/body breaks pioneer-family contract (no role/"Named after…"/insight/skeleton) | `skills/meeting-notes/SKILL.md:3,12` | P2 | quick | consistency | Add role sentence + "Named after Douglas Engelbart — …" + core insight; optional `<identity>`/Core Rule parity |
| 13 | both audit docs present already-fixed P0/P1s as open (inconsistent RESOLVED marking) | `.issueops/ISSUEOPS_AUDIT.md:10`; `.issueops/PROJECT_AUDIT.md:11` | P2 | quick | docs-drift | Mark ISSUEOPS 1.1 + PROJECT daemon 1.1/1.3 RESOLVED with fixing commits (5536cc8); add "last reconciled against `<sha>`" line; archive as dated snapshots |
...
| 16 | database-design 217-line concurrency section unrouted/undescribed | `skills/database-design/SKILL.md:3,672-887`; `rules.go:129-134` | P3 | quick | routing | Add deadlock/lock-contention/isolation/transaction (+ 데드락/락/트랜잭션) to database-design keywords and "Use when" |
...
| 24 | TECH_STACK frames deps as future candidates; MCP SDK undocumented/no ADR; skill count 10 vs 19 | `.issueops/TECH_STACK.md:49,68-77`; `ADR.md`; `go.mod:11` | P3 | quick | docs-drift | Replace §3 candidates with resolved deps; add ADR for `modelcontextprotocol/go-sdk`; fix count to 19 |
| 25 | CONVENTIONS references nonexistent `adapter/worker`, `adapter/fs` + flat files now split into packages | `.issueops/CONVENTIONS.md:26-30,38-39,58-59` | P3 | quick | docs-drift | Point worker→`internal/core/worker`, drop `adapter/fs`, reflect partitioned core subpackages + `*_facade.go` surface |
...
| 28 | design-review "IssueOps Benchmark Artifact Contract" heading has no consuming scorer signature | `skills/design-review/SKILL.md:36-48`; `benchmark/issueops_pioneer_checks.go` | P3 | quick | docs-drift | Retitle block (e.g. "Devil's-Advocate Verdict Evidence — feeds regress audit") or add a design-review scorer signature; document the 3-set membership policy |
...
| 30 | duplicate `mustJSON` panic helper across two cmd packages | `issueopsapp/misc_facade.go:148-153`; `apidoc/api_doc_review_output.go:7-13` | P3 | quick | consistency | Extract one shared helper; low priority |

> **Verification corrections applied:** audit-survey findings IO-02 (orphan-lock cleanup), IO-06 (non-Unix lock), PS-01 (database-design concurrency routing), and SC-01 (skillcontract pins) were *downgraded* P2→P3 by the adversarial pass (real but lower impact than first surveyed). Finding #13 was *downgraded* P1→P2. These corrected severities are reflected above.
...
- **Routing/consistency, quick:** #6 (structural GitLab host detection), #11 (design-review routing hint), #12 (meeting-notes frontmatter), #16 (database-design concurrency keywords).
...
- Routing/capability parity: #23 (MCP `prune_done`), #18 (classify CLI auth errors or document fallback), #28 (design-review benchmark heading).
