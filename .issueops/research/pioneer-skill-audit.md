...
## TL;DR
**Conclusion**: The issueops pioneer skills follow the Claude Code Agent Skills open standard with valid SKILL.md frontmatter. The namesake-specialist pattern provides clear role identity. archimedes was fully replaced by implementation-planning in the pioneer refactor (6f31c55) — no overlap exists. Cross-reference mesh is well-connected. Language agnosticism was confirmed in this audit.
**Confidence**: High
...
### 2. archimedes Successfully Replaced by implementation-planning (Corrected 2026-06-10)
- **Claim**: The pioneer refactor (commit 6f31c55) fully replaced `archimedes` with `implementation-planning` as the canonical Strategic Planning skill. No file-based `archimedes` skill exists anywhere in the project (`skills/archimedes/`) or global scope (`~/.reasonix/skills/archimedes/`). The `archimedes` that appeared as a slash command is a Reasonix host built-in, not a file-based skill.
- **Sources**:
  - `ls skills/archimedes/` → NOT FOUND (2026-06-10)
  - `ls ~/.reasonix/skills/archimedes/` → NOT FOUND (2026-06-10)
  - `ls skills/implementation-planning/SKILL.md` → EXISTS — canonical project planning skill
  - IssueOps SKILL.md — references `implementation-planning` 6 times in phase assist map
  - Git log: 6f31c55 refactor(skills): generalize pioneer skills — added implementation-planning, web-research, database-design, algorithm-optimization, debugging, prompt-engineering, code-quality-metrics
...
- **Claim**: The cross-reference topology centers on `verified-execution` (9 connections) and `issueops` (10 connections) as hubs, with specialist skills as spokes. This matches Claude Code's recommendation to use skills for "domain knowledge or workflows that are only relevant sometimes."
- **Sources**:
...
| 4 | skills/implementation-planning/SKILL.md | Canonical project planning skill (replaced archimedes) | Repo source | 2026-06-10 | High |
