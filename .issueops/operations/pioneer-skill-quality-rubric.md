...
| `verified-execution` | Whether evidence requirements scale down for low-risk tasks and still reject fake commands. |
| `implementation-planning` | Whether planning activates only when planning is actually the user's goal or the risk warrants it. |
...
| Reproducibility | Save the prompt packet and result summary under `.issueops/evidence/pioneer-skills-quality/`. |
...
|-------|---------------------------|
| `implementation-planning` | `Repo grounding`, `Decision-complete plan`, `Assumptions/defaults`, `Unresolved questions`, `Acceptance criteria` |
| `verified-execution` | `Success criteria`, `Evidence artifact`, `Cleanup receipt`, `Verification mode`, `Skipped checks` |
...
| `verified-execution` | Converts a goal into measurable criteria, captures observable evidence and cleanup receipts, and keeps verification proportionate to risk. |
| `implementation-planning` | Produces a decision-complete plan only when planning is warranted, grounds it in repo evidence, resolves ambiguity, and leaves no implementation judgement calls. |
...
The current scorecard and loop plan live in `.issueops/operations/pioneer-skill-quality-scorecard.md`.
...
code-quality-metrics 명령 결함, algorithm-optimization 게이트 위치, 3개 스킬의 비례성 마찰을 적발).
