...
- `scripts/release-repro-smoke.sh`가 temp `HOME`, `CODEX_HOME`, `ISSUEOPS_STATE_DIR`, fixture `ISSUEOPS_ROOT` 아래에서 통과해야 한다.
...
- `.issueops/operations/release-dogfood-notes.md`에 Codex/Claude MCP 등록과 `inspect/docs/state` dogfood 전사가 있어야 한다.
...
```bash
ISSUEOPS_RELEASE_KEEP_TMP=1 scripts/release-repro-smoke.sh
```

이미 빌드된 바이너리 검증:

```bash
ISSUEOPS_RELEASE_SKIP_BUILD=1 ISSUEOPS_BIN="$PWD/bin/issueops" scripts/release-repro-smoke.sh
```
...
```bash
ISSUEOPS_RELEASE_OUT_DIR="$PWD/dist" scripts/release-build-matrix.sh
```

target override:

```bash
ISSUEOPS_RELEASE_TARGETS="darwin/arm64 linux/amd64" scripts/release-build-matrix.sh
```
...
Decision record: `.issueops/ADR.md` section `2026-06-13 — Distribution decision gate`.

Rollback criteria:

- `inspect --json`, `docs --json`, `state maintain --json`, `scripts/release-repro-smoke.sh`, `scripts/release-build-matrix.sh`, or `issueops self-verify --seed=100 --target-score=95 --json` fails on the release checkout.
...
Current dogfood evidence is tracked in `.issueops/operations/release-dogfood-notes.md`.
