...
최종 갱신: **2026-08-27** — installed hook enforcement는 [ADR 2026-08-27](../adr/decisions/2026-08-27-session-start-owns-compaction-context.md)로 제거됐다. 아래 표에서 ✅ hook이던 행은 `retired`로 표기하며, 해당 실수는 문서 규약과 `issueops` 명령의 자체 검사로만 막는다.

| # | Incident (출처) | 관찰된 실패 | 결정적 강제장치 (hook/test) | 상태 |
|---|------------------|-------------|------------------------------|------|
| 1 | CAUTIONS §15 IssueOps worktree edits | issue 작업이 worktree 밖(소스 repo)에서 편집 | ~~PreToolUse `--enforce-worktree`~~ → 2026-08-27 hook 제거; `issueops execution status` + 절대경로 규율 | retired |
| 2 | CAUTIONS §16 decision replies must have numbered choices | 결정 응답이 번호 선택지/추천 1개 없이 종료 | ~~Stop `--enforce-numbered-next-actions`~~ → 2026-08-27 hook 제거; skill 프롬프트 규약 | retired |
...
| 10 | CAUTIONS §5 Worker lifecycle 문제 (stuck running) | 프로세스 크래시 후 worker가 running으로 고착 | ~~SessionStart 자동 트리거~~ → 2026-08-27 제거; `worker cleanup-stuck` 명시 명령 | retired |
...
| 12 | CP1 command policy override | 첫 workspace root의 policy catalog가 같은 프로세스의 다른 repo 평가로 새는 문제 | `.issueops/policy.json` per-evaluation load + two-root isolation tests (`internal/adapter/policy/policy_catalog_test.go`, `cmd/issueops/policycli/policy_cli_test.go`, `cmd/issueops/mcpcli/mcp_tool_policy_state_test.go`) | ✅ test |
...
| 14 | docs-index content churn | 문서 본문/목록 변화가 response contract golden을 과민하게 깨뜨림 | docs-index projection golden (`cmd/issueops/issueopsapp/response_contract_docs_projection_test.go`) keeps schema/count/required docs while CLI/MCP schema remains fixed | ✅ golden test |
