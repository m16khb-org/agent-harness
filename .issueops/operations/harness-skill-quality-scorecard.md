...
품질 프로그램 Q1 (`.issueops/issues/_unnumbered/harness-quality-improvement-program.md`).
이 스코어카드는 `.issueops/operations/pioneer-skill-quality-rubric.md`를 **그대로 적용**한다
...
- **IO-O**: 게이트 시퀀스(start→intent record→link-issue→branch prepare→link-worktree→design review→link-plan)의 문서화된 CLI 형태가 현 `issueops` CLI와 전부 일치하는가(실행 검증).
...
### project-bootstrap
- **PB-P**: 신규 레포(fixture)를 분석해 AGENTS.md 라우팅 블록 + .issueops 문서 세트를 생성하는가.
...
`.issueops/evidence/harness-skills-quality/sta-p-2026-06-13.md` (gitignored). live failure가 없어
...
`/tmp/issueops-sta-h-run2-fixed-20260613.json`, `/tmp/issueops-sta-h-run3-20260613.json`.
...
`/tmp/issueops-acp-o-preflight-20260613.json`, `/tmp/issueops-acp-o-api-doc-20260613.json`.
...
케이스 결과 원본은 `.issueops/evidence/harness-skills-quality/`(gitignored)에 저장하고,
