...
`~/.codex/hooks.json`, and `~/.omo/extensions/issueops.js`), so they fire
in every repository the agent opens — including repositories the harness does
not own. `ISSUEOPS_DISABLE_HOOKS` turns that registration into a no-op for one
session without editing host settings:

```bash
ISSUEOPS_DISABLE_HOOKS=1 claude    # 이 세션에서만 harness hook 미적용
export ISSUEOPS_DISABLE_HOOKS=1    # 셸 세션 전체에 적용
```
...
- `ISSUEOPS_DISABLE_HOOKS`가 켜지면 두 context command 모두 조용히 exit 0으로 끝나며 telemetry나 state를 남기지 않는다.
