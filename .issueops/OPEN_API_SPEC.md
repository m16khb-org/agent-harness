...
정적 게이트의 route 블록 조립은 괄호/중괄호 밸런스로 decorator 객체 내부(`summary:`/`description:` 줄)까지 포함한다. 다중 줄 `@ApiOperation`을 쓰는 문서화 잘 된 컨트롤러가 오히려 검사를 우회하지 않는지는 `cmd/issueops/apidoc/dogfood` 픽스처가 회귀 테스트로 고정한다.
