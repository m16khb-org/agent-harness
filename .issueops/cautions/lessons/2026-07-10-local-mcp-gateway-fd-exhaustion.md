...
- Triage: (1) `lsof -nP -iTCP:<port> -sTCP:LISTEN`으로 리스너 생존 확인, (2) curl로 MCP initialize POST — 즉시 리셋이면 FD 고갈 의심, (3) `lsof -p <pid> | wc -l`을 limit과 비교, (4) 게이트웨이 stderr 로그에서 `Errno 24` 확인. `issueops doctor`의 `mcp_gateway` 체크가 (2)(3)을 자동화한다 — 도달 불능이면 `mcp_gateway_unreachable`, FD 512 이상이면 `mcp_gateway_fd_pressure` warning.
