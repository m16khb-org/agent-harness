...
- Kind: `caution`
- Source: issueops/issueops/skills 품질 감사 (2026-08-26)
...
- Resolution:
  - `self-verify`에 무조건 실행되는 `gofmt` 단계(`cmd/issueops/validationcli/goformat`)를 추가해
...
- Evidence:
  ...
  - 로컬 `gofmt -l $(git ls-files '*.go')` → 10개 파일. `go test ./... -count=1` →
    `FAIL issueops/cmd/issueops/issueopsapp`(golden `$.cli.self_augment_plan.goals[1]`),
    `FAIL issueops/internal/adapter/skillcontract`(`symlink-not-allowed`).
  - `./bin/issueops quality inspect --json` → `ok:false`, `warnings:["coverage: exit status 1"]`
