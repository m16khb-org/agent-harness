...
- Summary: 사용자에게 반복 표시된 PreToolUse/SessionStart "invalid JSON output" 실패의 원인은 issueops가 아니라, 같은 이벤트에 등록된 claude-mem codex 훅이 stdout이 파이프일 때 JSON을 512바이트에서 잘라 내보낸 것(+ 별건으로 `status` unknown top-level field). issueops 훅은 전 경로에서 스키마 유효했다.
