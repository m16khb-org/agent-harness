...
description: Dated lesson — committing one root GATES.md per IssueOps worktree caused deterministic add/add or content conflicts; task ledgers now use .issueops/gates/*.md paths.
---
...
- Summary: IssueOps의 task-gate 지침이 모든 cycle에 같은 root `GATES.md`를 만들고
  ...
  발생했다.
- Context:
  - `skills/issueops/SKILL.md`가 `issueops gates init --file GATES.md`와
...
- Resolution:
  - `gates init`의 파일 미지정 기본값을 required scope에서 유도한
    `.issueops/gates/<scope-slug>.md`로 변경했다.
  - IssueOps는 더 강한 stable key인
    `.issueops/gates/issue-<provider-issue-number>.md`를 명시하고,
    `gates abandon`에도 같은
    파일을 전달한다.
...
- Evidence:
  ...
  - 같은 repo에서 두 branch가 `.issueops/gates/issue-11.md`와
    `.issueops/gates/issue-12.md`를 각각
