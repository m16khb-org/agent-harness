...
이 문서는 `issueops`에서 코드·문서·설정 변경 시 따라야 할 프로젝트 고유 원칙이다.
...
구체적인 구현 규칙은 `.issueops/CONVENTIONS.md`, 테스트 규칙은 `.issueops/TESTING.md`, 반복 실수 방지는
`.issueops/CAUTIONS.md`, 기술 스택과 명령어는 `.issueops/TECH_STACK.md`를 따른다.
...
5. `.issueops/CAUTIONS.md`의 장애/회귀 방지 규칙
6. `.issueops/CONVENTIONS.md`의 구현 컨벤션
7. `.issueops/TESTING.md`의 테스트 작성·검증 규칙
8. `.issueops/TECH_STACK.md`의 기술 스택·명령어 설명
9. `.issueops/ARCHITECTURE.md`, `.issueops/ADR.md`, README, 과거 계획 문서
...
**자료 접근과 반복 실행되는 hot path는 실용적인 범위에서 평균 또는 상각 `O(1)`을 우선 지향한다.**

- 성능이나 최적화 효과를 유의미하게 훼손하지 않는 선택지 사이에서는 가독성, 기존 프로젝트 컨벤션, API와 구조의 통일성을 우선한다.
- `O(1)`이라는 표기만 얻기 위해 불필요한 cache, index, 중복 상태를 추가하지 않는다. 최적화는 측정된 병목과 시간·공간 복잡도 근거로 정당화한다.
- `O(1)` 지향이 안전성이나 정확성과 충돌하면 이 장의 기존 우선순위에 따라 안전성과 정확성을 먼저 지킨다.
...
- 전체 sub-agent 적합 패턴과 근거는 `.issueops/SUB_AGENT_PATTERNS.md`에 문서화되어 있으며, 12가지 net-positive 패턴으로 정리된다.
- Sub-agent spawning overhead가 직접 작업 비용보다 큰 경우(단일 파일 편집, 전체 컨텍스트 필요, 교차 판단)는 절대 사용하지 않는다.
- Verified Execution 스킬은 "메인 에이전트 직접 수행"을 기본으로 하고, sub-agent dispatch는 12가지 패턴 중 하나에 명시적으로 해당할 때만 허용한다.
...
- Go 코드 변경은 영향 범위에 맞게 `go test ./... -count=1`, `go test -race ./...`, `go build ./cmd/issueops`를 실행한다.
