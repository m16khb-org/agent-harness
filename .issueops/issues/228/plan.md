...
**Architecture:** Establish stable capability contracts and minimal ports first, then migrate independently reviewable child scopes into domain/application/contract/port and inbound/outbound adapters. Concrete wiring remains only in `cmd/issueops/issueopsapp`; each child exact HEAD must pass deterministic tests plus fresh Codex and Claude Code native smoke before parent integration.
...
- `cmd/issueops/installcli/install_native.go` currently calls `BeginLegacyResetActivation` and `SealLegacyResetActivation`; current native activation must be extracted before `internal/core/issueops/reset_legacy*.go` is deleted.
- `scripts/install-native.sh` performs same-directory staged binary replacement; the script remains a repository helper but must call the canonical `issueops install` command after the CLI alias is removed.
- `internal/adapter/hostprobe/codex.go`, `internal/adapter/hostprobe/claude.go`, `internal/adapter/mcp/conformance_probe.go`, and `cmd/issueops/validationcli/installdryrun` already provide isolated MCP and install-dry-run building blocks.
...
- `go list ./...` contains no package/import beginning `issueops/internal/core`.
...
/absolute/canonical/coordinator/worktree/scripts/verify-child-host-smoke.sh \
  --issue 230 \
  --source-root /Users/m16khb/Workspace/issueops \
...
- Every child stores RED and GREEN command/exit evidence under ignored `.issueops/evidence/228/<issue>/`.
...
- API documentation gate: no HTTP endpoint/OpenAPI schema changes are planned. Run `./bin/issueops api-doc static-check --repo "$PWD" --json` at every child preflight and the final gate; define `api_doc_result="$evidence_dir/api-doc-review-result.json"` and, if candidates are detected, require `./bin/issueops api-doc review --repo "$PWD" --result "$api_doc_result" --json` before publication.
...
For each child set `evidence_dir="$child_root/.issueops/evidence/228/$issue"`, create it with mode `0700`, and keep it ignored/untracked. Evidence commands below write only bounded JSON/digests there.

1. Before committing, verify the canonical child worktree contains only task-owned tracked changes and ignored evidence, then run the child-focused, full, race, and golden gates named by the task and build `./bin/issueops`. Unrelated tracked/untracked files are a stop condition; the task-owned implementation diff is expected to be uncommitted here.
2. Stage only the exact task paths, inspect the staged diff, run `./bin/issueops api-doc static-check --repo "$PWD" --json`, and obtain a fresh implementation review with zero unresolved Critical/Important findings. If the API gate reports candidates, complete the agent review/result ingestion before continuing. Brooks is repeated only if the approved plan/design contract changes; it is not the child code reviewer. Any fix returns to step 1.
...
- Modify: `internal/core/policy/*.go`, `internal/core/state/state_types.go`, `cmd/issueops/installcli/install_native.go`
...
go test ./cmd/issueops/installcli -count=1
```
...
Scenario: valid current contracts compile through real consumers
  ...
  Evidence: .issueops/evidence/228/task-1-contract-foundation.json
...
Scenario: forbidden compatibility seam
  ...
  Evidence: .issueops/evidence/228/task-1-contract-foundation-error.json
```
...
- Create: `.issueops/operations/child-host-smoke.md`
...
- Modify: `.issueops/TESTING.md` live-smoke section only if the new runner contract differs from the existing approved text
...
```bash
lock_dir="${ISSUEOPS_STATE_DIR:-$HOME/.local/state/issueops}/child-host-smoke.lock"
...
Build into the disposable child checkout, verify `version`, record SHA-256, then run canonical `install --dry-run --project-local --json` under temporary HOME/CODEX_HOME/ISSUEOPS_ROOT and assert exact two-host/no-write output.
...
Scenario: complete fake two-host pass
  ...
  Evidence: .issueops/evidence/228/task-2-host-smoke.json
...
Scenario: Claude succeeds but restore digest drifts
  ...
  Evidence: .issueops/evidence/228/task-2-host-smoke-error.json
```
...
- Delete: `cmd/issueops/issueopscli/issueops_reset_legacy_cli.go`, `issueops_reset_legacy_cli_test.go`
...
- Modify: `cmd/issueops/statecli/{state_cli_router.go,state_cli_maintenance.go,state_cli_test.go,exports.go}`
- Modify: `cmd/issueops/mcpcli/{mcp_tool_policy_state.go,mcp_tool_policy_state_test.go}`
- Modify: `cmd/issueops/validationcli/stateroundtrip/*`, `cmd/issueops/selfworkflow/rerun/*`, `internal/adapter/cli/usage.go`, contract/response goldens
...
go test ./internal/contract/state ./internal/contract/issueopslease ./cmd/issueops/statecli ./cmd/issueops/issueopscli ./cmd/issueops/mcpcli -count=1
go test -race ./internal/application/nativeactivation ./internal/contract/issueopslease -count=1
scripts/verify-go-test-match.sh --run 'Golden' --expect 'Golden' -- ./cmd/issueops/contractgolden ./cmd/issueops/issueopsapp
```
...
Scenario: current-v1 read/write
  ...
  Evidence: .issueops/evidence/228/230/current-v1.json
...
Scenario: existing invalid versus absent
  ...
  Evidence: .issueops/evidence/228/230/invalid-matrix.json
```
...
- Modify/Delete: `cmd/issueops/mcpcli/{mcp_transport.go,mcp_sdk_server.go,mcp_tools.go,mcp_tool_*.go,mcp_transport_test.go}`
- Modify: `cmd/issueops/issueopsapp/{mcp_facade.go,root_command_facade.go,facade_wrappers_test.go}`
- Modify: `cmd/issueops/daemoncli/{daemon.go,daemon_status.go,daemon_identity.go,daemon_identity_test.go}`
- Modify: `cmd/issueops/daemoncli/daemonpaths/{instance.go,instance_test.go}`
- Delete: `cmd/issueops/installcli/install_native.go` only after canonical install implementation is renamed to `install.go`; remove `runInstallNative`
- Modify: `internal/adapter/cli/usage.go`, `scripts/install-native.sh`, `scripts/release-repro-smoke.sh`
- Modify: `cmd/issueops/updatecli/{update_bootstrap.go,update_bootstrap_test.go,update_bootstrap_edges_test.go}`
- Modify: `cmd/issueops/validationcli/installdryrun/*`, `cmd/issueops/selfworkflow/{rerun,summary,candidateexport,augmentcatalog}/*`
- Modify: `internal/adapter/install_contract_matrix_test.go`, `cmd/issueops/installcli/install_command_test.go`

**Interfaces:**

- Consumes: go-sdk `mcp.IOTransport`, current daemon `daemonInstance`, canonical `issueops install`, and Task 1 native activation.
...
Construct one `mcp.IOTransport` round trip for separate stdin/stdout and one for a single replaying daemon connection. Assert initialization, tools/list, one read-only tool call, cancellation, and EOF behavior; assert `ISSUEOPS_MCP_DIRECT=1` selects the current direct path without legacy parser types.
...
Run: `scripts/verify-go-test-match.sh --run 'SDKTransportOwns' --expect 'SDKTransportOwns' -- ./cmd/issueops/mcpcli`
...
Run: `scripts/verify-go-test-match.sh --run 'LegacyPID|StructuredInstance' --expect 'LegacyPID|StructuredInstance' -- ./cmd/issueops/daemoncli/...`
...
Rename the canonical implementation file/function to `install.go`/`runInstall`. Remove the alias router and usage entry. Keep `scripts/install-native.sh` as the named build-and-install helper, but change its binary invocation and all deterministic validation fixtures to `issueops install`.
...
go test ./cmd/issueops/mcpcli ./cmd/issueops/daemoncli/... ./cmd/issueops/installcli ./cmd/issueops/updatecli -count=1
go test -race ./cmd/issueops/mcpcli ./cmd/issueops/daemoncli/... -count=1
...
Scenario: current SDK MCP through both transports
  Channel: bash
  Steps: run SDK transport table and ISSUEOPS_MCP_DIRECT=1 contract tests
  Expected: initialization and one current tool call pass with no custom JSON-RPC type in production
  Evidence: .issueops/evidence/228/231/mcp-sdk.json

Scenario: retired inputs
  Channel: bash
  Steps: feed integer PID file and invoke issueops install-native --dry-run
  Expected: both fail deterministically; structured daemon file and issueops install succeed
  Evidence: .issueops/evidence/228/231/legacy-rejection.json
```
...
Update CLI/MCP/hook wiring to construct the current service once in `issueopsapp` and call typed inbound handlers. Remove the public two-argument `ReleaseExecution`/`CompleteExecution` compatibility path after all callers use the service contract.
...
Scenario: current execution lifecycle
  ...
  Evidence: .issueops/evidence/228/232/current-lifecycle.json
...
Scenario: legacy oracle and marker rejection
  ...
  Evidence: .issueops/evidence/228/232/legacy-rejection.json
```
...
! go list -json ./... | rg 'issueops/internal/core/(sqlstore|state|webfetch)'
...
Scenario: state/SQL current-v1 parity
  ...
  Evidence: .issueops/evidence/228/237/state-sql.json
...
Scenario: invalid state and bounded HTTP failure
  ...
  Evidence: .issueops/evidence/228/237/failure-paths.json
```
...
- Modify: `cmd/issueops/issueopscli/**`, `cmd/issueops/hookcli/**`, `cmd/issueops/loopcli/**`, `cmd/issueops/workercli/**`, `cmd/issueops/issueopsapp/issueops_*`, lifecycle hook wiring, MCP issueops tool
...
- [ ] **Step 6: Rewire inbound CLI/MCP/hooks and issueopsapp**

CLI/MCP/hook packages consume typed application/contract interfaces. `issueopsapp` constructs outbound implementations and injects them; CLI packages do not instantiate adapters. Preserve command/tool names and required response fields for current surfaces.
...
! go list -json ./... | rg 'issueops/internal/core/(hookfailure|hookmetrics|issueops|lifecycle|looprun|worker)'
```
...
go test ./cmd/issueops/issueopscli ./cmd/issueops/hookcli ./cmd/issueops/loopcli ./cmd/issueops/workercli ./cmd/issueops/mcpcli -count=1
scripts/verify-go-test-match.sh --run 'Golden' --expect 'Golden' -- ./cmd/issueops/contractgolden ./cmd/issueops/issueopsapp
```
...
Scenario: generation-fenced IssueOps lifecycle parity
  ...
  Evidence: .issueops/evidence/228/236/issueops-lifecycle.json
...
Scenario: concurrent persistence/process failure
  ...
  Evidence: .issueops/evidence/228/236/concurrency-failure.json
```
...
- Modify: `cmd/issueops/{basiccli,contractcli,docscli,guardcli,installcli,projectcli,statuscli}/**`
- Modify: `cmd/issueops/issueopsapp/*_facade.go` consumers for audit, docs, doctor, draftwiki, guard, install, policy, project docs, trace, and conformance
...
CLI packages accept application functions/interfaces. `issueopsapp` performs concrete construction. This task may update issueopsapp wiring but must not leave any non-issueopsapp cmd→adapter edge; Task 9 makes that architecture rule unconditional.
...
scripts/verify-go-test-match.sh --run 'Golden' --expect 'Golden' -- ./cmd/issueops/contractgolden
```
...
Scenario: current tooling surface parity
  ...
  Evidence: .issueops/evidence/228/235/tooling-parity.json
...
Scenario: I/O and warning failures
  ...
  Evidence: .issueops/evidence/228/235/tooling-failure.json
```

**Commit boundary:** five commits listed above; the parent merge reverts the child as one capability-migration unit.

### Task 9: Complete #233 cmd-to-adapter inversion through issueopsapp

**Files:**

- Modify: `cmd/issueops/contractcli/**`, `hookcli/**`, `installcli/**`, `issueopscli/**`, `mcpcli/**`, `validationcli/**`
- Modify/Create: `cmd/issueops/issueopsapp/{composition.go,contract_wiring.go,hook_wiring.go,install_wiring.go,issueops_wiring.go,mcp_wiring.go,validation_wiring.go}` using existing file naming where already present
- Modify: `cmd/issueops/issueopsapp/root_command_facade.go` to build dependencies once
- Modify: `internal/architecture/dependency_test.go`
...
- Produces: zero production `cmd/issueops/* -> internal/adapter/*` edges except `cmd/issueops/issueopsapp` composition root.

- [ ] **Step 1: Add the RED zero-edge architecture test**

Replace the cmd→adapter legacy baseline for these packages with an unconditional rule. The diagnostic includes exact importer and imported package; a synthetic issueopsapp edge remains allowed.
...
- [ ] **Step 3: Move concrete construction to issueopsapp**

Build outbound store/process/provider/host/MCP implementations in issueopsapp, create application services, then inject one typed handler into each CLI/MCP/hook entry. Remove package globals where a test can pass dependencies explicitly.
...
go test -race ./cmd/issueops/hookcli ./cmd/issueops/issueopscli ./cmd/issueops/mcpcli -count=1
scripts/verify-go-test-match.sh --run 'Golden' --expect 'Golden' -- ./cmd/issueops/contractgolden
scripts/verify-go-test-match.sh --run '^TestResponseContractsGolden$' --expect '^TestResponseContractsGolden$' -- ./cmd/issueops/issueopsapp
```

- [ ] **Step 6: Commit, push, and execute exact-head child smoke**

Execute the Normative Child Delivery Epilogue for issue 233. Both fresh hosts must traverse the new issueopsapp wiring for SessionStart, PreToolUse, and the MCP call.
...
Scenario: current CLI/MCP/hook parity
  ...
  Expected: current command/tool/field projections pass with zero non-issueopsapp adapter imports
  Evidence: .issueops/evidence/228/233/cmd-parity.json
...
Scenario: missing injected dependency
  ...
  Evidence: .issueops/evidence/228/233/missing-dependency.json
```

**Commit:** `refactor(cli): centralize adapter composition`

Lore records the 18→0 inventory and explicitly names issueopsapp as the only allowed concrete composition root.
...
- Modify: `cmd/issueops/issueopsapp/**` composition files that construct cooperating adapters
- Modify: `internal/architecture/dependency_test.go`
...
- Consumes: Tasks 1, 6, 7, 8, and 9 typed contracts plus the single issueopsapp composition root.
...
Export an exact importer/imported-package table to `.issueops/evidence/228/234/adapter-edges-before.json`. For each edge choose exactly one repair:

1. depend on a narrow existing capability port;
2. introduce a capability-owned port with the minimum required methods;
3. move construction/orchestration to `cmd/issueops/issueopsapp`;
...
Move all concrete pairing into issueopsapp. Keep authentication, command policy, workspace boundaries, secret redaction, and provider-visible side effects at the existing outbound adapter boundary. Preserve request/response types already frozen by Tasks 3–9.
...
go test ./cmd/issueops/issueopsapp ./cmd/issueops/contractgolden -count=1
```

Write `.issueops/evidence/228/234/adapter-edges-after.json` with an empty edge list and the exact tested commit.
...
Scenario: adapter isolation
  ...
  Expected: 20→0 concrete adapter edges; issueopsapp remains the only concrete composition root
  Evidence: .issueops/evidence/228/234/adapter-edges-after.json
...
Scenario: outbound dependency failure
  ...
  Evidence: .issueops/evidence/228/234/adapter-failures.json
```
...
Replace every remaining root import with the Task 1–10 owner. Delete aliases, constructor pass-throughs, error re-exports, and test-only compatibility helpers instead of moving them to another generic package. A caller that needs multiple capabilities receives them independently from issueopsapp.
...
```bash
test ! -d internal/core
! rg -n --glob '*.go' 'issueops/internal/core|internal/core' cmd internal
```
...
Scenario: strict core physical absence
  ...
  Evidence: .issueops/evidence/228/229/strict-core-zero.json
...
Scenario: unused cleanup fidelity
  ...
  Evidence: .issueops/evidence/228/229/unused-zero.json
```
...
- Modify: `.issueops/ADR.md` by appending a superseding clean-break decision; preserve decision history
- Modify: `.issueops/operations/release-dogfood-notes.md`
...
- `cmd/issueops/* -> internal/adapter/*` is zero outside issueopsapp;
...
Append a bounded entry to `.issueops/operations/release-dogfood-notes.md` with parent/child issue and PR links, exact commits, host versions/binary digests, 10 validated receipt paths/digests, restoration results, zero inventories, full gate result, and rollback boundary. Do not include transcripts, credentials, user-home paths beyond tool identity, or private reasoning.

- [ ] **Step 5: Run the strict and full repository gates from one unchanged HEAD**

Run the `Full Repository Gate` below from the integrated child HEAD, preserving separate command results in `.issueops/evidence/228/238/final-gate.json`. Also verify:
...
Scenario: clean checkout cannot regress architecture
  ...
  Evidence: .issueops/evidence/228/238/strict-rules.json
...
Scenario: final current-v1 and two-host release path
  ...
  Evidence: .issueops/evidence/228/238/final-gate.json
```
...
./bin/issueops api-doc static-check --repo "$PWD" --json
