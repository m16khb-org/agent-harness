...
v2: design-review `revise` 반영 — 폴더 비교는 문자열 완전일치(`021`≠`21`), legacy 동번호 문구 정정, T4 삭제, `gates_skipped` 한 줄 집계.

이슈: https://github.com/m16khb-org/issueops/issues/483
...
| 원장 | 판정 |
|---|---|
| `.issueops/issues/<n>/gates.md` (n 일치) | 판정 |
| `.issueops/issues/<다른 숫자>/gates.md` | 제외 → 집계 warning `gates_skipped:<count> (<rel>, …)` 한 줄 |
| `.issueops/issues/<비숫자>/gates.md` (예: `_unnumbered`) | 판정 (소유자 불명 = fail-closed) |
| `.issueops/gates/<파일>.md` — `legacyLedgerIssueNumber`가 n | 판정(pre-#480 사이클의 유일한 원장일 수 있음; canonical이 함께 있을 때만 duplicate 게이트가 fail-closed) |
| `.issueops/gates/<파일>.md` — 다른 번호 | 제외 |
| `.issueops/gates/<파일>.md` — 번호 없음, `GATES.md`, `gates/*.md` | 판정 |
...
| T3 | 문서: `skills/issueops/SKILL.md` gate 문단 한 줄("readiness는 내 이슈 원장과 번호 없는 원장만 본다"), `.issueops/CONVENTIONS.md` 레이아웃 절 한 줄 | 문서 | validate-skill, docs checker |
