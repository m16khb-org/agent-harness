...
- Modify: `cmd/issueops/mcpcli/` IssueOps execution tool schema/mapping
- Modify: `cmd/issueops/issueopsapp/` response mappings and goldens
...
- Test: CLI/MCP/issueopsapp/contract golden suites
...
```bash
go test ./cmd/issueops/issueopscli ./cmd/issueops/mcpcli ./cmd/issueops/issueopsapp ./cmd/issueops/contractgolden -run 'Selection|Preparation|Execution|Golden' -count=1
```
...
- Modify: `.issueops/operations/child-host-smoke.md`
...
- Modify: `.issueops/TESTING.md`
- Create: `.issueops/issueops/248-verified-execution-report.md`
...
- [ ] Record AC-01..AC-08 mapping, RED/GREEN commands, exact-head tests, and separate Orca/native-host evidence in the verification report.
...
- [ ] Run legacy and architecture fitness gates required by `.issueops/TESTING.md`; production legacy count must remain zero and no forbidden dependency edge may be added.
