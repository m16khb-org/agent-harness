...
**Tech Stack:** Go, JSONL state files, existing `corestate.WithKeyLock`, existing `internal/core/lifecycle/docupkeep` store pattern, existing `cmd/issueops/statecli` command router.
...
- Modify `cmd/issueops/statecli/state_cli_router.go`
  - Route `issueops state doc-upkeep archive`.
- Modify `cmd/issueops/statecli/state_cli_maintenance.go`
  - Add CLI flags and text/JSON output for pending archive dry-run/confirm.
- Modify `cmd/issueops/statecli/state_cli_test.go`
  - Cover dry-run default and `--confirm` behavior through the CLI.
- Update `.issueops/issues/_unnumbered/harness-quality-performance-program-v3-measurement-audit.md`
...
import (
	...
	"issueops/internal/core/lifecycle/model"
)
...
func TestCompactPendingEventsMergesDuplicateIntent(t *testing.T) {
	events := []model.DocUpkeepEvent{
		...
		{ID: "new", Kind: "code_change", TargetDocs: []string{".issueops/OPERATIONS.md"}, Summary: " Hook changed ", Evidence: []string{"b.go", "a.go"}, Source: "hook", Status: "pending", CreatedAt: "2026-07-02T00:00:00Z"},
...
import (
	...
	"issueops/internal/core/lifecycle/model"
	"issueops/internal/core/state/statepath"
)
...
Expected: `ok issueops/internal/core/lifecycle/docupkeep`.
...
func TestReadPendingCompactsDuplicatePendingEventsWithoutDroppingIntent(t *testing.T) {
	...
	lines := []string{
		...
		`{"id":"new","kind":"code_change","target_docs":[".issueops/OPERATIONS.md"],"summary":" Hook changed ","evidence":["b.go"],"status":"pending","created_at":"2026-07-02T00:00:00Z"}`,
...
Add tests in `cmd/issueops/statecli/state_cli_test.go`:
...
```bash
go test ./cmd/issueops/statecli -run 'TestStateDocUpkeepArchive' -count=1
```
...
**Files:**
- Modify: `.issueops/issues/_unnumbered/harness-quality-performance-program-v3-measurement-audit.md`
...
```bash
go test ./internal/core/lifecycle/docupkeep ./internal/core/lifecycle ./cmd/issueops/statecli ./cmd/issueops/hookcli -count=1
```
...
git add internal/core/lifecycle/model/types.go \
  ...
  cmd/issueops/statecli/state_cli_router.go \
  cmd/issueops/statecli/state_cli_maintenance.go \
  cmd/issueops/statecli/state_cli_test.go \
  .issueops/issues/_unnumbered/harness-quality-performance-program-v3-measurement-audit.md
git commit -m "fix: bound doc-upkeep pending retention"
```

Commit body should follow `.issueops/COMMIT_POLICY.md` and include the verification commands above.
...
- `go test ./... -count=1`, `go build -o bin/issueops ./cmd/issueops`, and `git diff --check` pass after implementation.
