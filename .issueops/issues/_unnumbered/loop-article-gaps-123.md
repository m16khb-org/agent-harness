...
## TL;DR
> **Summary**: Claude Code "Getting Started with Loops" 아티클 갭 분석에서 승인된 제안 1,2,3을 design-review 악마의 변호인 검토로 축소한 범위로 구현한다. P1은 소비자 게이트(strict PR readiness)를 포함한 4-tool 최소 스키마, P2는 TESTING.md 단일 규범 소유, P3는 명시적 `--pilot` 지정 방식.
> **Deliverables**: `internal/core/looprun` 패키지 + `issueops loop` CLI/MCP 4종 + strict readiness `loop_incomplete:` 게이트, TESTING.md "부분 검증 상태 금지" 규범 + 스킬 2종 포인터, golden/contract/문서 동기화, ADR 기록.
> **Effort**: Large (P2 0.5일 · P3 1–2일 · P1 3–5일, design-review 추정)
...
### Interview Summary
사용자 결정이 이미 명확해 인터뷰는 생략하고 repo 탐색 + design-review sub-agent 악마의 변호인 검토로 대체했다. design-review 판정: P1 TRIM(4 tools, name-키, list/success_criteria 삭제, 소비자 1개 명시 필수), P2 PROCEED(단일 규범 소유), P3 TRIM(명시적 `--pilot` 플래그, pilot-accepted 조건, 버전 스큐 문서화). design-review는 P1에 대해 "1주 state-write 컨벤션 스파이크 후 위반 실측 시 착수"를 권했으나, 사용자가 3종 전부 적용을 명시 지시했으므로 스파이크 대신 **소비자 게이트를 v1 스코프에 포함**하는 것으로 강제력 부재 리스크를 상쇄한다(이 결정은 ADR에 기록).
...
### Core Objective
아티클의 루프 운영 원칙(검증 가능한 정지 조건, 부분 검증 금지, 대규모 실행 전 파일럿)을 issueops의 기존 문법(fail-closed 게이트, durable state, no-shell, actor model)으로 기계 강제한다.

### Deliverables
- `internal/core/looprun` 패키지(types/store/lifecycle) + `internal/core/loop_facade.go`
- `cmd/issueops/loopcli` + `internal/adapter/cli/usage.go` catalog + `internal/adapter/mcp/loop_catalog.go`(tools: `loop_start`/`loop_record_attempt`/`loop_status`/`loop_stop`)
- `IssueOpsStrictPRReadiness`의 `loop_incomplete:<id>` 차단 + doctor active-loop 경고
- TESTING.md "부분 검증 상태 금지" 규범 절 + `skills/self-verify/SKILL.md`·`skills/verified-execution/SKILL.md` 포인터
...
- `go build -o bin/issueops ./cmd/issueops && ./bin/issueops contract check --json` OK
- `go test ./cmd/issueops/contractgolden ./cmd/issueops/issueopsapp -run Golden -count=1` 통과(갱신 후)
- `python3 scripts/validate-skill.py skills/self-verify && python3 scripts/validate-skill.py skills/verified-execution` 통과
- `ISSUEOPS_STATE_DIR=$(mktemp -d) ./bin/issueops self-verify --seed=100 --target-score=95 --json` gate 통과
...
- Test decision: tests-after 아님 — **구현+테스트 동일 task**(repo 컨벤션: table-driven Go test, `t.Setenv("ISSUEOPS_STATE_DIR", t.TempDir())` 격리, TESTING.md §3 기준)
- QA policy: 모든 task에 happy + failure 시나리오, channel은 bash(CLI 직접 실행)
- Evidence: `.issueops/evidence/task-{N}-{slug}.txt`
...
- [ ] 1. P2: "부분 검증 상태 금지" 규범을 TESTING.md에 신설하고 스킬 2종에 포인터 추가
  ...
  **What to do**:
  1. `.issueops/TESTING.md`의 "## 5. 완료 보고 기준" 앞에 새 절 `## 부분 검증 상태 금지 (all-or-nothing verification)` 추가. 규범 전문: (a) 다단계 검증 시나리오에서 한 단계라도 실패하면 이전 단계의 통과를 재사용하지 않고 1단계부터 전체 재실행한다, (b) 완료 보고의 evidence는 마지막 "전 단계 통과" 단일 run에서 나온 것이어야 하며 서로 다른 run의 부분 통과를 조합하지 않는다, (c) 재실행 비용이 큰 경우에도 부분 통과 상태를 "검증됨"으로 승격하지 않는다 — 비용이 문제면 검증 시나리오를 더 작은 독립 시나리오로 분리한다.
  2. `skills/self-verify/SKILL.md` "## Gate" 절 끝에 1문장 + 포인터: "다단계 검증에서 한 단계라도 실패하면 1단계부터 재실행하며 부분 통과 evidence를 재사용하지 않는다 (규범 출처: `.issueops/TESTING.md` 부분 검증 상태 금지 절)."
  3. `skills/verified-execution/SKILL.md` "## Final Quality Gate" 절(316행 부근)에 동일한 1문장 + 포인터 추가.
  ...
  **References**:
  - `.issueops/TESTING.md:184` — "## 5. 완료 보고 기준" 앞 삽입 위치
  - `skills/self-verify/SKILL.md:31-35` — Gate 절 구조
  - `skills/verified-execution/SKILL.md:316` — Final Quality Gate 절
  - 아티클 근거: verify-frontend-change 예시 "단계 실패 시 1단계부터 재실행, 부분 검증 상태 금지"

  **Acceptance Criteria**:
  - [ ] `rg -n "부분 검증 상태 금지" .issueops/TESTING.md skills/self-verify/SKILL.md skills/verified-execution/SKILL.md`가 3파일 모두에서 hit
  - [ ] `python3 scripts/validate-skill.py skills/self-verify && python3 scripts/validate-skill.py skills/verified-execution` exit 0
  ...
  **QA Scenarios**:
  ...
  Scenario: 규범 단일 소유 확인
    Channel: bash
    Steps: rg -c "1단계부터" .issueops/TESTING.md skills/self-verify/SKILL.md skills/verified-execution/SKILL.md
    Expected: TESTING.md는 규범 전문(≥3줄 관련 내용), 스킬 2종은 각 1회만 언급 + "규범 출처" 문자열 포함
    Evidence: .issueops/evidence/task-1-no-partial-verify.txt

  Scenario: 스킬 스키마 검증 실패 회귀 없음
    Channel: bash
    Steps: python3 scripts/validate-skill.py skills/self-verify; python3 scripts/validate-skill.py skills/verified-execution
    Expected: 두 명령 모두 exit 0
    Evidence: .issueops/evidence/task-1-skill-validate.txt
  ```

  **Commit**: YES | `docs(testing): forbid partial verification state with single normative source` | Files: `.issueops/TESTING.md`, `skills/self-verify/SKILL.md`, `skills/verified-execution/SKILL.md`
...
- [ ] 3. P1 core: `internal/core/looprun` 패키지(types/store/lifecycle) + 테스트
  ...
  **What to do**:
  ...
  4. 테스트(`lifecycle_test.go`, `store_test.go`): start-resume 멱등성, terminal 재시작 거부, evidence 필수, auto-exhaust 경계(max번째 fail에서 전이, pass는 미전이), exhausted 후 attempt 거부, success-stop의 pass 요구, stop reason 길이, schema future-version 거부, redaction(`token=abc` → `<redacted>`), `ISSUEOPS_STATE_DIR` temp 격리.
  ...
  **References**:
  ...
  - `.issueops/ARCHITECTURE.md` §5 — state root 격리 원칙
  ...
  **QA Scenarios**:
  ...
  Scenario: durable resume 멱등성 (happy)
    ...
    Evidence: .issueops/evidence/task-3-loop-resume.txt
  ...
  Scenario: evidence 없는 성공 선언 차단 (failure)
    ...
    Evidence: .issueops/evidence/task-3-loop-failclosed.txt
  ```
...
- [ ] 5. P1 wiring: `issueops loop` CLI + MCP 4-tool 배선

  **What to do**:
  1. `internal/core/loop_facade.go`: 기존 core facade 패턴대로 `StartLoopRun`/`RecordLoopAttempt`/`LoopRunStatus`/`StopLoopRun` + type alias.
  2. `cmd/issueops/loopcli/loop.go` 신설: 서브커맨드 `start`(`--repo`, `--name`, `--goal`, `--max-attempts`, verify argv는 `--` 뒤 나머지 인자 — policy check와 동일 문법), `record-attempt`(`--id`, `--verdict pass|fail`, `--evidence TEXT` 반복 허용), `status`(`--id` 또는 `--repo`+`--name`), `stop`(`--id`, `--success` 또는 `--reason TEXT`). 모두 `--json` 지원, 사람이 읽을 text 출력 병행(AGENTS.md §7 계약).
  3. dispatch 등록: `cmd/issueops/issueopsapp/cli_facade.go` + `root_command_facade.go`의 기존 도메인 등록 방식 복제.
  ...
  5. `internal/adapter/mcp/loop_catalog.go` 신설: `loop_start`/`loop_record_attempt`/`loop_status`/`loop_stop` descriptor(각각 쓰기 여부, 필수 인자, 결과 shape 명시). `internal/adapter/mcp/catalog.go`의 집계 목록에 결정적 순서로 추가. MCP handler를 `cmd/issueops` MCP 매핑에 추가.
  6. `cmd/issueops/loopcli/loop_cli_test.go`: flag 파싱, `--` argv 캡처, verdict enum 거부, JSON shape 스모크. golden은 건드리지 않음(T7).
  ...
  **References**:
  - `cmd/issueops/issueopsapp/cli_facade.go`, `cmd/issueops/issueopsapp/root_command_facade.go` — dispatch 등록 지점
  - `internal/adapter/mcp/catalog.go` — MCP catalog 등록·순서 규칙
  - `internal/adapter/cli/usage.go:38,118-126` — catalog entry/usage 라인 형식
  - `./bin/issueops policy check ... --json -- git status` — `--` argv 문법 선례

  **Acceptance Criteria**:
  - [ ] `go test ./cmd/issueops/loopcli ./cmd/issueops/issueopsapp -count=1` 통과(golden 제외 대상은 T7 이후 재확인)
  - [ ] `go build -o bin/issueops ./cmd/issueops` 성공
  - [ ] `./bin/issueops loop start --repo "$PWD" --name smoke --goal "lint clean" --max-attempts 3 --json -- go vet ./...` 정상 JSON 반환

  **QA Scenarios**:
  ```
  Scenario: 루프 전체 수명주기 CLI 스모크 (happy)
    Channel: bash
    Steps: tmp=$(mktemp -d); export ISSUEOPS_STATE_DIR=$tmp
      ./bin/issueops loop start --repo "$PWD" --name qa-loop --goal "tests pass" --max-attempts 2 --json -- go test ./...
      ./bin/issueops loop record-attempt --id <ID> --verdict fail --evidence "go test: 1 failure in core" --json
      ./bin/issueops loop record-attempt --id <ID> --verdict pass --evidence "go test ./... ok all packages" --json
      ./bin/issueops loop stop --id <ID> --success --json
      ./bin/issueops loop status --id <ID> --json
    Expected: status가 "succeeded", attempts 배열 길이 2, verify_argv에 ["go","test","./..."]
    Evidence: .issueops/evidence/task-5-loop-cli-smoke.txt
  ...
  Scenario: auto-exhaust와 성공 종료 거부 (failure)
    ...
    Evidence: .issueops/evidence/task-5-loop-exhaust.txt
  ```

  **Commit**: YES | `feat(loop): wire loop-contract CLI and MCP surfaces` | Files: `internal/core/loop_facade.go`, `cmd/issueops/loopcli/*`, `cmd/issueops/issueopsapp/{cli_facade,root_command_facade}.go`, `internal/adapter/cli/usage.go`, `internal/adapter/mcp/{loop_catalog,catalog}.go`, MCP handler 파일
...
- [ ] 6. P1 소비자: strict PR readiness의 `loop_incomplete:` 차단 + doctor 경고
  ...
  **What to do**:
  ...
  2. `issueops doctor`에 active loop 관측 추가: state root의 loop bucket을 읽어 repo별 active/exhausted loop 수를 진단 항목으로 보고(수정 없음, 보고만 — doctor 계약 유지).
  ...
  **References**:
  ...
  - `.issueops/ARCHITECTURE.md:131` — strict readiness가 요구하는 기존 항목 목록

  **Acceptance Criteria**:
  - [ ] `go test ./internal/core -run 'LoopGate' -count=1` 통과
  - [ ] 기존 `go test ./internal/core -run 'PoolGate' -count=1` 무수정 통과(회귀 0)
  - [ ] `ISSUEOPS_STATE_DIR=$(mktemp -d) ./bin/issueops doctor --repo . --json`에 loop 진단 필드 존재
  ...
  **QA Scenarios**:
  ...
  Scenario: active loop이 PR readiness를 차단 (happy-gate)
    ...
    Evidence: .issueops/evidence/task-6-loop-gate.txt
  ...
  Scenario: 타 repo loop 오차단 없음 (failure/경계)
    ...
    Evidence: .issueops/evidence/task-6-loop-gate-scope.txt
  ```
...
- [ ] 7. golden/contract 일괄 재생성 및 계약 검증
  ...
  **What to do**:
  ...
  3. `./bin/issueops contract check --json` + `go test ./cmd/issueops/contractcli -count=1` 실행.
  ...
  **References**:
  - `.issueops/TESTING.md:105-110` — golden 갱신 명령과 조건
  - `cmd/issueops/testdata/{usage.golden.txt,mcp_tools.golden.json,response_contracts.golden.json}` — 대상 3종(response_contracts는 11,336줄 — diff 검수 필수)
  - `.issueops/AGENTS.md §10` — response-contract golden이 고정하는 범위

  **Acceptance Criteria**:
  - [ ] `go test ./cmd/issueops/contractgolden ./cmd/issueops/issueopsapp -run Golden -count=1` (–update 없이) 통과
  - [ ] `./bin/issueops contract check --json` OK
  ...
  **QA Scenarios**:
  ...
  Scenario: golden 재현성 (happy)
    Channel: bash
    Steps: go test ./cmd/issueops/contractgolden ./cmd/issueops/issueopsapp -run Golden -count=1
    Expected: -update 없이 전부 PASS (재생성 결과가 결정적)
    Evidence: .issueops/evidence/task-7-golden.txt

  Scenario: 기존 계약 불변 (failure 감시)
    Channel: bash
    Steps: git diff cmd/issueops/testdata/ | rg '^-' | rg -v '^---' | head -20
    Expected: 삭제 라인 0건(추가만) — 0건이 아니면 task FAIL로 보고
    Evidence: .issueops/evidence/task-7-golden-additive.txt
  ```

  **Commit**: YES | `test(contract): regenerate goldens for loop surfaces` | Files: `cmd/issueops/testdata/*.golden.*`

- [ ] 8. 문서 동기화 + ADR 기록

  **What to do**:
  1. `.issueops/ARCHITECTURE.md`: §3 실행 모드 표에 `issueops loop` 행(용도: verify-until-done 루프 계약의 durable 상태; 원칙: 하네스는 검증을 실행하지 않고 기록·게이트만), §5 state 절에 loop 위치(`~/.local/state/issueops/loop/`)와 제공 표면(CLI 4종/MCP 4종), strict readiness 요구 항목에 loop 추가.
  2. `AGENTS.md`: §8 디렉토리 맵 `cmd/issueops` 행의 명령 목록에 `loop` 추가, §9 Essential Commands에 loop 스모크 1줄.
  3. `.issueops/OPERATIONS.md` Core Surfaces CLI 목록 + `.issueops/operations/cli-and-mcp.md`에 loop/pilot 사용법.
  5. `.issueops/CAUTIONS.md`: 버전 스큐 1줄 — "pilot_required pool은 구버전 바이너리에서 게이트 없이 claim될 수 있다(additive 필드 무시). shared daemon과 로컬 CLI 버전을 함께 갱신한다."
  6. `.issueops/ADR.md`: 신규 결정 기록 — 아티클 출처, design-review TRIM 결정(4 tools/name-키/소비자 게이트, list·success_criteria 기각, goal-hash 기각, "첫 task=pilot" 기각), 스파이크 대신 소비자 게이트 포함으로 간 근거(사용자 명시 지시), 기각 대안(스케줄러, hook 강제, 토큰 계측), `state write` 컨벤션 대안이 진 이유(전이 강제 불가).
  ...
  **References**:
  - `.issueops/ARCHITECTURE.md:62-73,95-137` — 실행 모드 표·state 절 형식
  - `.issueops/ADR.md:428-437,484-501` — article-insight + design-review TRIM 기록 선례 형식
  - design-review 리뷰 결과(이 계획의 Gap Analysis 절) — ADR에 옮길 기각 근거 원문

  **Acceptance Criteria**:
  - [ ] `./bin/issueops docs --json` 정상(문서 index 파손 없음)
  - [ ] `rg -n "loop" .issueops/ARCHITECTURE.md AGENTS.md .issueops/OPERATIONS.md | rg -i "issueops loop"` hit
  ...
  **QA Scenarios**:
  ...
  Scenario: 문서-구현 일치 (happy)
    Channel: bash
    Steps: ./bin/issueops loop --help 2>&1 | head -10; rg -n 'loop start' .issueops/operations/cli-and-mcp.md
    Expected: 문서의 명령 문법이 실제 usage 출력과 동일 문자열
    Evidence: .issueops/evidence/task-8-docs-parity.txt

  Scenario: docs index 회귀 없음 (failure 감시)
    Channel: bash
    Steps: ./bin/issueops docs --json | head -5
    Expected: 정상 JSON, 에러 0
    Evidence: .issueops/evidence/task-8-docs-index.txt
  ```

  **Commit**: YES | `docs(harness): document loop contract and ADR decisions` | Files: `.issueops/{ARCHITECTURE,ADR,OPERATIONS,CAUTIONS,SUB_AGENT_PATTERNS,TESTING}.md`, `.issueops/operations/cli-and-mcp.md`, `AGENTS.md`

- [ ] 9. 전체 검증 + self-verify 게이트 + 완료 보고

  **What to do**:
  1. TESTING.md §2 전체 검증: `go mod tidy && go test ./... -count=1 && go test -race ./... -count=1 && go vet ./... && go build -o bin/issueops ./cmd/issueops`
  2. 스모크: `./bin/issueops inspect --json`, `docs --json`, `daemon status --json`, T4/T5 QA 시나리오 재실행.
  3. `ISSUEOPS_STATE_DIR=$(mktemp -d) ./bin/issueops self-verify --seed=100 --target-score=95 --json` — 95점 게이트. 실패 시 `lint_diagnose`로 진단 후 수정 반복(부분 검증 금지 규칙 적용: 수정 후 §2 전체부터 재실행).
  ...
  **References**:
  - `.issueops/TESTING.md:85-110,184-195` — 검증 명령·완료 보고 기준·QA gate
  ...
  **QA Scenarios**:
  ...
  Scenario: 95점 게이트 (happy)
    Channel: bash
    Steps: ISSUEOPS_STATE_DIR=$(mktemp -d) ./bin/issueops self-verify --seed=100 --target-score=95 --json
    Expected: 모든 goal_scores > 95, 종료 코드 0
    Evidence: .issueops/evidence/task-9-self-verify.txt

  Scenario: race detector 청정 (failure 감시)
    Channel: bash
    Steps: go test -race ./internal/core/looprun -count=1
    Expected: PASS, DATA RACE 출력 0건
    Evidence: .issueops/evidence/task-9-race.txt
  ```
...
- [ ] F2. Code Quality Review — `oh-my-claudecode:code-reviewer` 또는 code-quality-metrics 측정으로 AI slop/불필요 추상화 부재 확인.
- [ ] F3. Real Manual QA — T1–T9의 모든 QA 시나리오 evidence가 `.issueops/evidence/`에 실재하고 PASS인지 재확인. 특히 T9의 self-verify 95점 게이트.
- [ ] F4. Scope Fidelity Check — 제안 4(이벤트 경계)·스케줄러·hook 자동화가 스며들지 않았는지, 기존 state 동작 회귀가 없는지 `git diff --stat`로 대조.

## Commit Strategy
task별 원자 커밋(각 task의 Commit 라인 참조). 순서: T1 → T2 → T3 → T4 → T5 → T6 → T7 → T8. 형식은 `.issueops/COMMIT_POLICY.md`의 Conventional Commit subject + Lore body. golden 갱신(T7)은 배선 커밋과 분리해 리뷰 가능하게 유지. push는 사용자 지시 시에만.
...
- design-review 지적 사항 3건(소비자 부재, goal-hash footgun, 암묵 pilot 규칙) 모두 해소된 상태로 구현
