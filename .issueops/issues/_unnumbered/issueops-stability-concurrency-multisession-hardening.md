...
- focused 기존 테스트는 통과했다: `internal/core/{policy,state,looprun,sqlstore}`, `cmd/issueops/{daemoncli,updatecli}`. 이는 아래 결함을 잡는 regression이 없다는 뜻이지 결함이 없다는 뜻이 아니다.
...
|---|---|---|---|---|
| R01 daemon admission 복구 실패 | P1 | R | idle socket 64개가 timeout 뒤에도 slot을 점유하며 65번째 연결은 평문 오류와 exit 0을 받는다. status/doctor는 healthy로 오판한다. | `cmd/issueops/daemoncli/daemon_server.go:14-27,101-185`, `daemon_proxy.go:30-70`, `daemon_status.go:10-35` (MCP-1/2) |
| R02 process identity 부재 | P1 | S | PID 파일이 숫자뿐이라 PID reuse 시 무관한 프로세스를 stop/update/cleanup이 종료할 수 있다. live PID+missing socket은 duplicate daemon도 허용한다. | `cmd/issueops/daemoncli/daemonpaths/paths.go:45-66`, `daemon_status.go:38-66` (OWN-1, HI-02) |
...
| R04 cleanup/refresh 범위 과대 | P1 | S | 정상 `issueops mcp` 세션과 dbhub/context7/kordoc 같은 외부 npx MCP까지 signal 후보가 된다. | `cmd/issueops/updatecli/update_bootstrap_mcp.go:42-164,365-370` (CLEAN-1, HI-01) |
...
| R19 self-verify false-positive | P1/P2 | T | 실패 전용 필드가 contract hash에서 빠지고, clean tree에서는 race/vet 미실행도 covered, parallel isolation은 실제 self-verify를 돌리지 않으며 binary drift는 mtime만 본다. | `cmd/issueops/selfworkflow/summary/self_verify_summary_contract.go`, `riskqa/*.go`, `validation_parallel_*.go`, `internal/core/doctor/checks.go:142-170` (QCT-01~04) |
| R20 stability-audit가 시스템을 오염/오판 | P2 | R/T | repo `bin/issueops`를 재빌드해 live symlink 대상을 바꾸고, interruption cleanup이 없어 실제 orphan을 남겼다. proxy/socket FD를 세지 않고 live user daemon RSS를 요구한다. | `skills/stability-audit/scripts/e2e_stability_audit.py:131-139,199-218,321-382` (SA-01~04, QCT-05~07) |
| R21 fuzz·문서·계약 drift | P3 | T | native `Fuzz*`가 0개인데 deterministic battery를 fuzz로 부르고, Go toolchain/audit 문서와 현재 코드가 어긋난다. | `cmd/issueops/selfworkflow/steps/self_verify_steps.go:79`, `.issueops/TECH_STACK.md:28` (QCT-09, DOC-01) |
...
- `./bin/issueops self-verify --full --iterations=10 --seed=100 --target-score=95 --progress=jsonl --json`이 실제 executed coverage를 보고한다.
...
- evidence 경로 `.issueops/evidence/stability-hardening/Txx-*`
...
- **References**: `.issueops/AGENT_WORKFLOW.md`, `SUB_AGENT_PATTERNS.md`, `skills/issueops`.
...
- **References**: `cmd/issueops/daemoncli/daemonpaths/paths.go:45-66`, `daemon_status.go:10-66`, `daemon_start.go:43-76`, `daemon_server.go:72-103`.
...
- **What**: process snapshot에 PID/PPID/start time/executable/instance token/registered endpoint를 포함한다. `mcp cleanup --apply`는 dead-parent인 exact issueops proxy만 signal 직전 재검증해 종료한다. update refresh는 user `issueops` endpoint만 대상으로 하고 외부 npx MCP는 제외한다.
- **Owner**: deep agent. **Depends**: T01.
- **References**: `cmd/issueops/updatecli/update_bootstrap_mcp.go:42-164,365-370`, `issueopsapp/mcp_command_facade.go:21-46`, `.issueops/CAUTIONS.md` orphan proxy contract.
...
- **References**: `internal/core/worker/read_only.go:33-45`, `worker/worker.go:20-35`, `policy/policy_env_redaction.go:55-68`, `cmd/issueops/mcpcli/mcp_tool_policy_state.go`.
...
- **References**: `internal/core/policy/policy_run.go:61-116`, `cmd/issueops/commandstep/run.go:22-37`.
...
- **Commit**: `fix(identity): canonicalize repository ownership across issueops state`.
...
- **References**: `internal/core/install/install.go:80`, `internal/adapter/installutil/install_util.go:40-77`, `cmd/issueops/installcli/install_native_path.go:90`, `scripts/install-native.sh:95-135`.
...
- **Acceptance**: default install은 issueops integration만 생성한다. stale managed link는 dry-run에서 정확히 보고되고 apply 전에는 보존. live no-conflict/conflict/dogfood-name/build mismatch fixture가 구분된다. stateful fake gateway에 doctor를 반복 실행해도 active session과 FD count가 baseline으로 복귀한다.
...
- **References**: `gjc-plugin/hook.ts:19-40`, `internal/adapter/hook/output.go:8-166`, hook CLI, `.issueops/operations/hosts.md`.
...
- **References**: `cmd/issueops/selfworkflow/summary/self_verify_summary_contract.go`, `riskqa/risk_qa*.go`, `validation_parallel_*.go`, candidate catalog, response goldens.
...
- **Must not**: `ROOT/bin/issueops` overwrite; user daemon required; raw `subprocess.run(timeout=...)` 예외로 JSON report 유실.
...
- **References**: `.issueops/TESTING.md`, `PROJECT_AUDIT.md`, `ISSUEOPS_AUDIT.md`, `CAUTIONS.md`, `ADR.md`, response-contract golden.
...
4. **Race/full**
   ...
   - `go build -o <temp>/issueops ./cmd/issueops`
5. **Runtime isolation**
...
- 외부 glab/dbhub/context7/kordoc 기능을 issueops core에 복제
