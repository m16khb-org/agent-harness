...
현재 `issueops`는 테스트와 계약 검증이 강해 동작 안정성은 높다. 반면 다음 구조적 비용이 누적되고 있다.
...
- MCP registry 중복
  ...
  - `cmd/issueops/mcpcli/catalog/core_tools.go`
  - `cmd/issueops/mcpcli/catalog/tools.go`
- installer / hook 중복
...
- global wiring
  ...
  - `cmd/issueops/qualitycli/quality_inspect.go`
- 고분기 router
  - `cmd/issueops/issueopscli/issueops.go`
  - `cmd/issueops/issueopscli/benchmarkcmd/benchmark.go`
...
- `cmd/issueops/issueopsapp/cli_facade.go`
- `cmd/issueops/issueopsapp/root_command_facade.go`
- `cmd/issueops/basiccli/dependencies.go`
- `cmd/issueops/statuscli/dependencies.go`
- `cmd/issueops/qualitycli/dependencies.go`
- `cmd/issueops/selfworkflow/dependencies.go`
- `cmd/issueops/qualitycli/quality_inspect.go`
...
- **리스크:** DI 전환 중 test fixture가 깨질 수 있다.
  - **완화:** P3는 race test와 issueopsapp/CLI targeted test를 함께 돌린다.
