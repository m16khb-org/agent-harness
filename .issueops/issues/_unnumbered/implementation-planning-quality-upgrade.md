...
| 9 | **Spec-Driven Detection** (OpenSpec/Spec Kit) | Low — issueops 생태계에 부재 | **미적용** (미래 검토) |
...
## Work Objectives
### Core Objective
Von Neumann 스킬이 Prometheus 수준의 계획 품질을 내면서도 issueops의 간결함을 유지하도록 6가지 증분 개선을 적용한다.

### Deliverables
- `skills/implementation-planning/SKILL.md` — 6가지 개선이 반영된 최종 스킬 파일
...
- Evidence: `.issueops/evidence/implementation-planning-upgrade/`
...
- [ ] 1. Intent Classification 5종으로 확장
  ...
  **Must NOT do**:
  - Prometheus의 8종 전부를 복사하지 않는다 (Collaborative, Spec-Driven 등은 issueops 생태계에 불필요)
  ...
  **References**:
  - 현재 코드: `skills/implementation-planning/SKILL.md:71-79` — Phase 0: Classify Intent
  ...
  **QA Scenarios**:
  ...
  Scenario: Refactoring intent triggers safety-focused interview
    Channel: bash
    Steps: rg "Refactoring" skills/implementation-planning/SKILL.md 후 전략 텍스트 확인
    Expected: "test coverage" 또는 "behavior preservation" 또는 "risk" 중 2개 이상 포함
    Evidence: .issueops/evidence/implementation-planning-upgrade/task-1-intent.txt

  Scenario: Research intent triggers investigation strategy
    Channel: bash
    Steps: rg "Research" skills/implementation-planning/SKILL.md
    Expected: "parallel" 또는 "probe" 또는 "exit criteria" 중 하나 이상 포함
    Evidence: .issueops/evidence/implementation-planning-upgrade/task-1-research.txt
  ```

  **Commit**: YES | Message: `feat(implementation-planning): expand intent classification to 5 types` | Files: `skills/implementation-planning/SKILL.md`
...
- [ ] 2. Anti-Duplication 규칙 추가
  ...
  **References**:
  - 현재 코드: `skills/implementation-planning/SKILL.md:83-95` — Phase 1: Ground
  ...
  **QA Scenarios**:
  ...
  Scenario: Anti-duplication rule is explicit
    Channel: bash
    Steps: rg "Anti.Duplication|subagent.*재수행|FORBIDDEN.*explor" skills/implementation-planning/SKILL.md
    Expected: 1개 이상 매치
    Evidence: .issueops/evidence/implementation-planning-upgrade/task-2-anti-dup.txt
  ```

  **Commit**: YES | Message: `feat(implementation-planning): add anti-duplication rule` | Files: `skills/implementation-planning/SKILL.md`
...
- [ ] 3. Turn Termination 규칙 강화
  ...
  **References**:
  - 현재 코드: `skills/implementation-planning/SKILL.md:35-42` — Output Discipline
  ...
  **QA Scenarios**:
  ...
  Scenario: Turn termination rules present
    Channel: bash
    Steps: rg "Turn Termination|종료.*규칙|let me know" skills/implementation-planning/SKILL.md
    Expected: 매치 1개 이상
    Evidence: .issueops/evidence/implementation-planning-upgrade/task-3-termination.txt
  ```

  **Commit**: YES | Message: `feat(implementation-planning): enforce turn termination checklist` | Files: `skills/implementation-planning/SKILL.md`
...
- [ ] 4. QA Scenario Anti-Pattern 가드 추가
  ...
  **References**:
  - 현재 코드: `skills/implementation-planning/SKILL.md:300-313` — QA Scenarios 템플릿
  ...
  **QA Scenarios**:
  ...
  Scenario: Anti-pattern examples are concrete
    Channel: bash
    Steps: rg "Verify it works|Check.*API.*returns|should respond with|looks correct" skills/implementation-planning/SKILL.md
    Expected: 3개 이상 매치 (금지 예시로 명시됨)
    Evidence: .issueops/evidence/implementation-planning-upgrade/task-4-anti-pattern.txt
  ```

  **Commit**: YES | Message: `feat(implementation-planning): add QA scenario anti-pattern guard` | Files: `skills/implementation-planning/SKILL.md`
...
- [ ] 5. Agent Profile 추천 추가
  ...
  **References**:
  - 현재 코드: `skills/implementation-planning/SKILL.md:285-315` — Task template
  ...
  **QA Scenarios**:
  ...
  Scenario: Agent recommendation field exists
    Channel: bash
    Steps: rg "Recommended Agent|quick.*deep.*visual" skills/implementation-planning/SKILL.md
    Expected: 매치 1개 이상
    Evidence: .issueops/evidence/implementation-planning-upgrade/task-5-agent.txt
  ```

  **Commit**: YES | Message: `feat(implementation-planning): add agent profile recommendation` | Files: `skills/implementation-planning/SKILL.md`

- [ ] 6. Draft Cleanup 단계 추가

  **What to do**:
  - Phase 3 Step 5(Offer Choice) 뒤에 "Step 6: Draft Cleanup" 추가
  - Plan 완료 후 `.issueops/drafts/<slug>.md` 삭제 지시
  - "Draft was working memory; plan is the single source of truth" 설명
  - `rm .issueops/drafts/<slug>.md` 명령어 예시 포함
  ...
  **References**:
  - 현재 코드: `skills/implementation-planning/SKILL.md:222-227` — Step 5: Offer Choice
  ...
  **QA Scenarios**:
  ...
  Scenario: Draft cleanup step documented
    Channel: bash
    Steps: rg "Draft Cleanup|draft.*삭제|rm.*drafts" skills/implementation-planning/SKILL.md
    Expected: 매치 1개 이상
    Evidence: .issueops/evidence/implementation-planning-upgrade/task-6-cleanup.txt
  ```

  **Commit**: YES | Message: `feat(implementation-planning): add draft cleanup after plan completion` | Files: `skills/implementation-planning/SKILL.md`
...
## Success Criteria
- `wc -c skills/implementation-planning/SKILL.md` < 20000 (20KB 미만)
- 기존 Clearance Checklist(6항목) 변경 없음
- 기존 Plan Template 구조 유지
- 모든 개선 사항이 SKILL.md 내에서 grep으로 확인 가능
- `./bin/issueops docs --json` 에 implementation-planning 정상 출력
