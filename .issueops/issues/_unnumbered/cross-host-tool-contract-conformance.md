...
> **Summary**: Codex·Claude Code·GJC가 같은 MCP tool schema를 실제로 어떻게 호출하는지 먼저 capture-only benchmark로 측정하고, 동일한 drift가 재현된 경우에만 issueops의 단일 canonical validation 계약을 강화한다. 동시에 기존 `failure_class`와 직교하는 증거 기반 `failure_cause` 축을 self-verify/trace/history에 추가하고, 재현된 drift만 behavioral regression fixture로 승격한다.
>
...
다음 네 자료의 통찰을 issueops에 적용할 수 있는지 분석한 선행 결과를, 전부 상세하고 실행 가능한 계획으로 만든다.
...
- `cmd/issueops/contractcli/contract.go:35-55`는 현재 `schema`, `check` 두 subcommand만 제공한다. 새 표면은 새 top-level command가 아니라 `contract conformance` 아래에 둔다.
- `cmd/issueops/mcpcli/mcp_sdk_server.go:33-44`는 raw arguments를 `map[string]any`로 decode한 뒤 handler로 넘기며 uniform schema validation을 하지 않는다.
- `cmd/issueops/mcpcli/mcp_tools.go:23-28,40-70`는 tool-level failure와 JSON-RPC parameter failure를 구분하지만 parse 가능한 schema violation을 공통 판정하는 seam이 없다.
- `cmd/issueops/mcpcli/argmap/args.go`는 string→bool/int, CSV→slice 등의 coercion을 조용히 수행한다.
- `internal/adapter/mcp/catalog.go:3-8` 및 catalog 집계는 advertised schema와 dispatch의 source of truth다.
- `cmd/issueops/testdata/mcp_tools.golden.json`에는 100개 tool이 있으며, top-level `additionalProperties:false`는 0개다.
- `cmd/issueops/selfworkflow/summary/self_verify_summary_failure.go:7-24`의 `failure_class`는 반복 패턴만 분류한다.
- `cmd/issueops/selfworkflow/model/self_verify_summary_types.go:38-60`과 `internal/core/trace/self_verify.go:11-52`에는 causal owner 필드가 없다.
...
- `issueops contract conformance {baseline,live,replay,serve}` CLI.
...
- 세 host runner의 argv/config/temp cleanup을 fake process로 검증하고, live 실행은 명시적 `ISSUEOPS_TOOL_CONFORMANCE_LIVE=1` 없이는 거부한다.
...
issueops contract conformance live --hosts codex,claude,gjc [--model host=value]... [--gjc-auth-env NAME]... [--profile clean|context-pressure] [--only host:fixture] [--resume-report PATH] [--target-completed 1|10|20] [--max-attempts-per-case 1..3] [--evidence-dir PATH] [--json]
issueops contract conformance replay --fixture PATH [--json]
issueops contract conformance serve --fixture-id ID --result-file PATH --run-token TOKEN
```

- `baseline`: network/auth 없이 10 direct cases + tracked regression corpus를 재생한다.
- `live`: `ISSUEOPS_TOOL_CONFORMANCE_LIVE=1`이 없으면 실행 전 거부한다. baseline을 먼저 내부 실행하고 실패 시 host process를 띄우지 않는다.
...
| Claude | temp `.mcp.json`, `-p --output-format stream-json --strict-mcp-config --no-session-persistence --permission-mode dontAsk --tools "" --allowedTools "mcp__issueops_probe__${probe_tool_name}"` | episode별 한 probe tool만 allow | local/user MCP 등록 수정 금지 |
...
이 네 조건 전에는 adoption/retention/impact 수치를 수집하거나 issueops 성공 지표로 주장하지 않는다.
...
- `cmd/issueops/contractcli/conformance.go`
- `cmd/issueops/contractcli/conformance_test.go`

### Existing files to modify unconditionally

- `cmd/issueops/contractcli/contract.go`
- `cmd/issueops/issueopsapp/misc_facade.go`
- `cmd/issueops/commandstep/result.go`
- `cmd/issueops/selfworkflow/model/self_verify_summary_types.go`
- `cmd/issueops/selfworkflow/model/self_augment_compare_types.go`
- `cmd/issueops/selfworkflow/summary/{self_verify_summary,self_verify_summary_contract}.go`
- `cmd/issueops/selfworkflow/historycompare/self_verify_compare_core.go`
- `cmd/issueops/selfworkflow/steps/self_verify_steps.go`
- `internal/core/trace/{analyze,self_verify,fields}.go`
- focused tests beside the files above.
- `cmd/issueops/testdata/{usage.golden.txt,mcp_tools.golden.json,response_contracts.golden.json}`
- `.issueops/{ARCHITECTURE,TESTING,OPERATIONS,CAUTIONS,ADR}.md`
...
- `cmd/issueops/mcpcli/{mcp_sdk_server,mcp_tools}.go`
- `cmd/issueops/mcpcli/argmap/args.go`
- their focused tests.
...
- **Evidence root**: `.issueops/evidence/tool-conformance/` 아래 `BenchmarkReport.RunID`과 같은 이름의 child directory. 이 경로는 ignored이며 파일 mode 0600, directory mode 0700을 assert한다.
...
- Final live evidence: `.issueops/evidence/tool-conformance/20260714T114337.222844000Z/report.json` (`9/9` completed, `0` environment failures, `0` transport failures).
...
- [x] **T1. Preregister the core conformance and failure-cause contracts**
  ...
  **Must not**:
  ...
  - core가 `cmd/issueops` 또는 `internal/adapter`를 import하지 않는다. adapter catalog는 caller가 `ToolDescriptor`로 투영한다.
  ...
  **References**:
  ...
  - `cmd/issueops/testdata/mcp_tools.golden.json` — 현재 100-tool source-backed schema snapshot.
  - `cmd/issueops/mcpcli/argmap/args.go:33-123` — 기존 coercion shape. benchmark classification 설명에만 사용.
  ...
  **QA evidence**:
  ...
  ```bash
  mkdir -p .issueops/evidence
  go test ./internal/core/toolconformance ./internal/core/failurecause -count=1 -v \
    | tee .issueops/evidence/task-1-core-conformance.txt
  ```
...
- [x] **T2. Add a capture-only MCP probe server and deterministic contract CLI**
  ...
  **Implementation**:
  ...
  6. `cmd/issueops/contractcli/conformance.go`에 `baseline`, `live`, `replay`, `serve` dispatch를 추가한다. 이 task에서는 `live`가 port dependency를 호출하도록 wiring만 하고 real runner는 T3에서 주입한다.
  7. `contract.go:40-48` switch에 `conformance`, usage에 네 subcommand를 추가한다. `issueopsapp/misc_facade.go`의 기존 `configureContractCLI` seam으로 catalog/root/process deps를 주입한다.
  ...
  **References**:
  ...
  - `cmd/issueops/mcpcli/mcp_sdk_server.go:16-31,75-84` — official SDK server/register pattern.
  - `cmd/issueops/mcpcli/mcp_sdk_server_test.go:12-42` — raw SDK handler test pattern.
  - `cmd/issueops/contractcli/contract.go:35-93` — nested subcommand/JSON output style.
  - `cmd/issueops/issueopsapp/misc_facade.go:34-45` — contract dependency wiring.

  **Acceptance criteria**:

  - [ ] `go test ./internal/adapter/mcp ./cmd/issueops/contractcli -run 'Conformance|Probe' -count=1` 통과.
  - [ ] `go build -o bin/issueops ./cmd/issueops` 성공.
  - [ ] `./bin/issueops contract conformance baseline --json`은 `ok:true`, `case_count:10`, `gate.decision:"baseline_passed"`.
  - [ ] `ISSUEOPS_TOOL_CONFORMANCE_LIVE` 없이 `live`는 nonzero, `live_opt_in_required`, process spy 0회.

  **QA evidence**:

  ```bash
  go test ./internal/adapter/mcp ./cmd/issueops/contractcli \
    -run 'Conformance|Probe' -count=1 -v \
    | tee .issueops/evidence/task-2-probe-cli.txt
  go build -o bin/issueops ./cmd/issueops
  ./bin/issueops contract conformance baseline --json \
    | tee .issueops/evidence/task-2-baseline.json
  ```
...
- [x] **T3. Implement isolated Codex, Claude, and GJC host runners plus the live gate**
  ...
  **Implementation**:
  ...
  3. Codex adapter:
     ...
     - `-c`의 `mcp_servers.issueops_probe.command`에는 `harness_binary`, `args`에는 `contract conformance serve` argv를 TOML array로 설정.
     - requested model이 `default`가 아니면 `--model` 추가.
  4. Claude adapter:
     - temp `.mcp.json`에 probe server 하나만 작성.
     - `claude -p --output-format stream-json --strict-mcp-config --mcp-config "$mcp_config_file" --no-session-persistence --permission-mode dontAsk --tools "" --allowedTools "mcp__issueops_probe__${probe_tool_name}"`.
     - requested model이 `default`가 아니면 `--model` 추가.
  5. GJC adapter:
  ...
  7. `contract conformance live` flags:
     ...
     - `--evidence-dir` default `.issueops/evidence/tool-conformance`.
  ...
  **Acceptance criteria**:
  ...
  - [ ] `go test ./internal/adapter/hostprobe ./internal/core/toolconformance ./cmd/issueops/contractcli -count=1` 통과.
  ...
  **QA evidence**:
  ...
  ```bash
  go test ./internal/adapter/hostprobe ./internal/core/toolconformance ./cmd/issueops/contractcli \
    -run 'HostProbe|LiveGate|Conformance' -count=1 -v \
    | tee .issueops/evidence/task-3-host-runners.txt
  ```
...
- [x] **T4. Add the orthogonal failure-cause axis to self-verify, trace, and history**
  ...
  **Implementation**:
  ...
  1. `cmd/issueops/commandstep/result.go`의 `StepResult`에 `FailureEvidence []failurecause.Evidence \`json:"failure_evidence,omitempty"\`` 추가. command runner는 기본 nil을 유지하고 새 conformance step만 typed evidence를 채운다.
  ...
  **References**:
  ...
  - `cmd/issueops/selfworkflow/model/self_verify_summary_types.go:38-69` — summary/contract types.
  - `cmd/issueops/selfworkflow/summary/self_verify_summary.go:72-77` — current failure classification insertion point.
  - `cmd/issueops/selfworkflow/summary/self_verify_summary_failure.go:7-24` — 보존할 occurrence classifier.
  - `cmd/issueops/selfworkflow/stateio/self_verify_state_snapshot.go:11-26` — schema-v1 compatibility.
  - `internal/core/trace/analyze.go:25-31`, `self_verify.go:11-52`, `fields.go:116-129` — propagation/dedupe.

  **Acceptance criteria**:

  - [ ] `go test ./cmd/issueops/selfworkflow/... ./internal/core/trace -run 'FailureCause|FailureClass|LegacySnapshot' -count=1` 통과.
  ...
  **QA evidence**:
  ...
  ```bash
  go test ./cmd/issueops/selfworkflow/... ./internal/core/trace \
    -run 'FailureCause|FailureClass|LegacySnapshot' -count=1 -v \
    | tee .issueops/evidence/task-4-failure-cause.txt
  ```
...
- [x] **T5. Run the operator-approved P0 evidence gate and promote only reproduced failures**
  ...
  **Preflight — no external model call**:
  ...
  go build -o bin/issueops ./cmd/issueops
  ./bin/issueops contract conformance baseline --json \
    | tee .issueops/evidence/tool-conformance-baseline.json
  codex --version
  claude --version
  gjc --version
  ```
  ...
  **Initial live batch — explicit approval required**:
  ...
  export ISSUEOPS_TOOL_CONFORMANCE_LIVE=1
  ./bin/issueops contract conformance live \
  ...
  **Reproduction batch — separate approval**:
  ...
  export ISSUEOPS_TOOL_CONFORMANCE_LIVE=1
  ./bin/issueops contract conformance live \
    --resume-report .issueops/evidence/tool-conformance/initial/report.json \
  ...
  **Acceptance criteria**:
  ...
  - [ ] `git status --short`에 `.issueops/evidence/`가 나타나지 않음.

  **Evidence commands**:

  ```bash
  jq '{schema_version,run_id,counts,gate,hosts,warnings}' \
    .issueops/evidence/tool-conformance/*/report.json
  rg -n --hidden -i \
    '(api[_-]?key|authorization|bearer|refresh[_-]?token|/Users/[^/]+)' \
    .issueops/evidence/tool-conformance \
    > .issueops/evidence/task-5-secret-scan.txt || true
  git status --short
  ```
...
- [ ] **T6. Conditionally enforce one canonical MCP argument contract across all tools** — skipped because `gate.decision=defer_hardening` did not satisfy the entry condition.
  ...
  **References**:
  ...
  - `cmd/issueops/mcpcli/mcp_sdk_server.go:33-71` — SDK raw decode/response seam.
  - `cmd/issueops/mcpcli/mcp_tools.go:40-70` — legacy raw call/dispatch seam.
  - `cmd/issueops/mcpcli/argmap/args.go:33-123` — 제거할 silent coercion.
  - `cmd/issueops/mcpcli/mcp_sdk_server_test.go:12-42`, `mcp_tool_project_test.go:135-160` — parity/error tests.

  **Acceptance criteria**:

  - [ ] `go test ./internal/adapter/mcp ./cmd/issueops/mcpcli ./internal/core/toolconformance -count=1` 통과.
  - [ ] `go test -race ./internal/adapter/mcp ./cmd/issueops/mcpcli ./internal/core/toolconformance -count=1` 통과.
  ...
  **QA evidence**:
  ...
  ```bash
  go test ./internal/adapter/mcp ./cmd/issueops/mcpcli ./internal/core/toolconformance \
    -run 'Schema|Arguments|Conformance|Legacy|SDK' -count=1 -v \
    | tee .issueops/evidence/task-6-canonical-validator.txt
  go test -race ./internal/adapter/mcp ./cmd/issueops/mcpcli ./internal/core/toolconformance \
    -count=1 \
    | tee .issueops/evidence/task-6-canonical-validator-race.txt
  ```
...
- [x] **T7. Make deterministic conformance part of self-verify and synchronize public contracts/docs**
  ...
  **Implementation**:
  ...
  3. `BuildCompatibilityContract`:
     ...
     - verification command에 `issueops contract conformance baseline --json` 추가.
  ...
  7. 문서 업데이트:
     - `.issueops/ARCHITECTURE.md`: capture-only P0, shared canonical validator, host adapter 경계.
     - `.issueops/TESTING.md`: 10-case baseline, 9-episode live matrix, reproduction/promotion gate, behavioral assertion, 5% warning의 비규범성.
     - `.issueops/OPERATIONS.md`: baseline/live/replay exact commands, Codex/Claude/GJC isolation, live approval/cost boundary.
     - `.issueops/CAUTIONS.md`: advertised-vs-intent attribution, GJC auth isolation, implicit open map 금지, no silent repair.
     - `.issueops/ADR.md`: evidence-first conditional enforcement, one contract, adoption scorecard activation gate, no scheduler/RL platform.
  ...
  **References**:
  ...
  - `cmd/issueops/selfworkflow/steps/self_verify_steps.go:43-84` — planned step order.
  - `cmd/issueops/selfworkflow/summary/self_verify_summary_contract.go:44-83` — goals/coverage.
  - `cmd/issueops/contractcli/contract.go:95-157` — compatibility contract/hash.
  - `.issueops/TESTING.md:100-110` — golden update procedure.
  ...
  **Acceptance criteria**:
  ...
  - [ ] `./bin/issueops contract check --json`에서 version 2, ok true.
  - [ ] `./bin/issueops contract conformance baseline --json`에서 regression corpus 포함 pass.

  **QA evidence**:

  ```bash
  go test ./cmd/issueops/selfworkflow/... ./cmd/issueops/contractcli \
    ./cmd/issueops/contractgolden ./cmd/issueops/issueopsapp -count=1 \
    | tee .issueops/evidence/task-7-contract-integration.txt
  python3 scripts/validate-skill.py skills/self-verify \
    | tee .issueops/evidence/task-7-self-verify-skill.txt
  ./bin/issueops contract check --json \
    | tee .issueops/evidence/task-7-contract-check.json
  ```
  ...
  **Proposed commits**:
  ...
  각 commit은 `.issueops/COMMIT_POLICY.md`의 Conventional subject + Lore body를 따른다.
...
- [x] **T8. Final all-or-nothing verification, adversarial review, and handoff**
  ...
  **Pre-final diff audit**:
  ...
  git diff --stat
  git diff --check
  git diff -- cmd/issueops/testdata/usage.golden.txt \
  ...
  **Focused verification**:
  ...
  go test ./internal/core/toolconformance ./internal/core/failurecause \
    ...
    ./cmd/issueops/contractcli ./cmd/issueops/mcpcli \
    ./cmd/issueops/selfworkflow/... ./internal/core/trace -count=1
  go test ./cmd/issueops/contractgolden ./cmd/issueops/issueopsapp \
    -run Golden -count=1
  ```
  ...
  ```bash
  rg --files internal/adapter/mcp cmd/issueops/mcpcli -g '*.go' | xargs gofmt -w
  ```
  ...
  **Full final gate — one uninterrupted run**:
  ...
  ./bin/issueops contract check --json
  ./bin/issueops contract conformance baseline --json
  ./bin/issueops install-native --dry-run --json
  tmp_state="$(mktemp -d)"
  ISSUEOPS_STATE_DIR="$tmp_state" \
