...
작성일: 2026-06-11
근거: ① 이 세션의 평가체계 진단 보고서(`.issueops/research/pioneer-skill-evaluation-and-issueops-integration-assessment.md`),
...
| 비-pioneer 스킬 7종 (issueops, atomic-commit-push, self-verify, self-augment, project-bootstrap, prompt-engineering*, draft-wiki-promoter, stability-audit) | **스코어카드/홀드아웃 0** (Go 테스트는 있음: issueops 64파일 ↔ stability-audit 0파일로 편차 극단) | 스킬 인벤토리 |
...
*prompt-engineering는 pioneer 스코어카드에 포함되나 Go contract test 0건.
...
- **[P1, 신규] lifecycle state 동시성 레이스**: `go test ./internal/core/lifecycle ./cmd/issueops/... -count=3` 병렬 부하에서
  ...
  **이것이 stability-audit fast-path의 self-verify(go test 스텝) 간헐 실패의 원인이다**(실측). debugging 진단 → 원자적 쓰기
...
- **P1**: hook failure log 무한 성장(rotation 없음, `hookfailure-*.jsonl`); `ISSUEOPS_EXPECTED_WORKTREE` env가 compaction에 휘발 (ISSUEOPS_AUDIT 2.2).
- **P2**: PIPE_BUF 초과 동시 append interleave; Codex/Claude 렌더링 drift.
- 세션 중 `issueops` MCP 재접속 끊김이 이번 세션에서도 재현됨(도그푸드 관찰) — 안정성 신호.
...
- **무엇**: 기존 `pioneer-skill-quality-rubric.md`(5-dimension, gate flag, A–D evidence, holdout 프로토콜)를 그대로 재사용해
  `.issueops/operations/harness-skill-quality-scorecard.md`(비-pioneer 7종)를 신설. 스킬당 3 visible 케이스 + 1 holdout.
...
- **수용 기준**: `issueops hook stats --json`이 4지표 반환 + Go 테스트; Stop hook p95 latency < 1s(LLM 게이트 제거 후 heuristic이므로 달성 가능해야 정상).
...
- **무엇**: `e2e_stability_audit.py --json`을 (a) 주기 실행 가능한 형태로 두고(예: 주 1회 수동 or cron), (b) 핵심 지표 — 잔존 daemon 수, zombie 수, RSS 추이, MCP 재접속 끊김 횟수 — 를 시계열로 `.issueops/operations/stability-baseline.md`에 누적.
...
- **무엇**: §0의 신규 발견 — InitProjectLifecycleState의 read-while-write 레이스를 debugging 7-step으로 진단 후
...
- **무엇**: 산재한 정량 지표(스킬 스코어카드 2종, 벤치마크 run, hook stats, stability baseline)를 한 문서
  `.issueops/operations/quality-dashboard.md`로 인덱스(링크+최신값+측정일). 자동화는 후순위, 우선 수동 갱신 규약.
...
- **무엇**: contract test 0건인 prompt-engineering(451줄)·draft-wiki-promoter(50줄)·stability-audit(90줄+685줄 스크립트)에 최소 핀 테스트. 단, 계층-C 한계(텍스트 존재≠행동)를 테스트 주석에 명시하는 이번 라운드 관례 유지.
...
- **무엇**: (a) **P0** 세션↔사이클 바인딩: SessionStart hook이 활성 IssueOps 사이클 ID를 state에 기록·복원.
  (b) **P1** `ISSUEOPS_EXPECTED_WORKTREE`를 IssueOps 레코드에 영속화하고 hook이 env+state 양쪽 확인.
...
- **무엇**: verified-execution/prompt-engineering/database-design/code-quality-metrics/implementation-planning에서 검증된 비례 모드를, 고정-무게로 남은 스킬 중 적용 가치가 있는 것(web-research quick-lookup은 일부 반영됨 → issueops 경량 사이클, atomic-commit-push 단일 파일 fast-path 등)에 확장. 단 **measured gap 없으면 EDIT 금지**(keep/discard 규율 유지) — S1 도그푸드 결과로 대상 선별.
