...
### 진단 근거 (요약)
- 시스템 파이프 fd 14,402개, codex 1개 프로세스가 3,112개 점유 → 신규 파이프 100/100이 512B 버퍼로 강등(실측) → write-then-read 캡처 헬퍼가 512B 초과 JSON에서 데드락 → go test 600s FAIL. 상세: `.issueops/CAUTIONS.md` 2026-07-09 파이프 KVA 항목.
- 표준 수정 패턴 실재: `cmd/issueops/issueopsapp/response_contract_runners_test.go:66-85` — `io.ReadAll(r)`을 goroutine으로 **fn 실행 전에** 시작해 파이프 버퍼 크기 의존을 제거(6ee897d).
- 취약 write-then-read 헬퍼 전수(grep `os.Pipe()` + fn 후 `io.ReadAll`): `cmd/issueops/{loopcli,statecli,policycli,projectcli,basiccli,qualitycli,commandstep,issueopscli(2),hookcli(hook_user_prompt, vcsissue/hook_capture, hook_post_tool_use),mcpcli,daemoncli,selfworkflow/{historycompare,candidatescmd},...}` — 고아로 발견된 `.test` 바이너리 목록(statecli/statuscli/workercli/candidatescmd/historycompare/promotecmd/verifycmd)과 일치.
- 다른 세션이 `cmd/issueops/updatecli/update_bootstrap_mcp.go`(+테스트)를 수정 중 — updatecli/mcp proxy 영역은 이 계획의 범위에서 제외한다.
...
- **worktree와 golden**: golden 재생성은 worktree 안에서 실행해도 `cmd/issueops/testdata` 경로가 worktree-로컬이므로 안전.
...
- `rg -l 'os\.Pipe\(\)' cmd --type go` 결과 중 updatecli/제외 목록 외 전부가 `testsupport.CaptureStdout` 사용으로 전환 (`rg -n 'io.ReadAll\(r\)' cmd/issueops/*cli* | wc -l` 감소분으로 확인)
- `ISSUEOPS_STATE_DIR=$(mktemp -d) ./bin/issueops doctor --repo . --json`에 `pipe_capacity` 진단 필드 존재, 강등 시 warning
- worktree 브랜치에서 `go test ./cmd/issueops/... -count=1` 전체 통과 (600s 행 0건)
...
- `cmd/issueops/updatecli/**` 수정 금지(다른 세션 작업 영역)
...
> 구현+테스트 동일 task. Evidence: `.issueops/evidence/pipefix-*.txt`. 전체 스위트는 T4 완료 후에만 실행(그 전에는 행 위험).
...
- [ ] 1. 미커밋 문서 변경을 즉시 원자 커밋 (main tree, 충돌 창 최소화)
  ...
  **What to do**: main 작업 트리에서 `git add .issueops/CAUTIONS.md .issueops/issues/_unnumbered/loop-article-gaps-123.md .issueops/issues/_unnumbered/phase-aware-pioneer-hints.md .issueops/issues/_unnumbered/pipe-pressure-and-session-conflict.md` 후 `docs(cautions): record macOS pipe-KVA hang diagnosis and follow-up plans` Conventional Commit + Lore body로 커밋. **updatecli 2개 파일은 절대 stage하지 않는다**(다른 세션 소유). atomic-commit-push 스킬 절차(preflight, 개별 파일 지정 add) 준수.

  **Must NOT do**: `git add -A`/`git add .` 금지. push는 사용자 지시 시.
  **Recommended Agent**: quick
  **References**: `.issueops/COMMIT_POLICY.md`, `skills/atomic-commit-push/SKILL.md`
  ...
  **QA**:
  ...
  Scenario: 선택적 스테이징 (happy)
    ...
    Evidence: .issueops/evidence/pipefix-commit-docs.txt
  Scenario: 타 세션 파일 불간섭 (failure 감시)
    Channel: bash — git status --short
    Expected: ' M cmd/issueops/updatecli/...' 2줄이 그대로 남아 있음
    Evidence: 동일 파일
  ```
...
- [ ] 2. 격리 worktree 브랜치 생성 및 작업 규약 고정
  ...
  **What to do**: `git worktree add ../issueops-pipefix -b fix/pipe-capture-immunity` (T1 커밋 이후 시점 기준). 이후 T3–T7 전부 이 worktree에서 실행. 착수 시 `git -C /Users/sample/workspace/issueops system-status --short` 스냅샷을 evidence로 남기고, T7 병합 준비 때 재확인해 다른 세션 변경과 파일 교집합 0을 검증한다. 빌드 산출물은 worktree-로컬 `bin/`을 사용(main의 `bin/issueops`는 hook이 사용 중이므로 덮어쓰지 않음).

  **Must NOT do**: main tree에서의 코드 편집·golden 재생성·`go build -o bin/issueops` 금지.
  **Recommended Agent**: quick
  **References**: `git worktree` 표준 절차; `.issueops/AGENT_WORKFLOW.md`의 worktree 계약
  **Acceptance Criteria**: `git worktree list`에 신규 항목; worktree에서 `go build ./...` 성공
  **QA**:
  ```
  Scenario: 격리 확인 (happy)
    Channel: bash — cd ../issueops-pipefix && git status --short
    Expected: clean (updatecli 수정은 main tree에만 존재)
    Evidence: .issueops/evidence/pipefix-worktree.txt
...
- [ ] 3. 공유 캡처 헬퍼 `internal/testsupport` 신설
  ...
  **References**: `cmd/issueops/issueopsapp/response_contract_runners_test.go:66-85`(정확한 복제 원형)
  ...
  **QA**:
  ...
  Scenario: 대용량 출력 무블록 (happy)
    ...
    Evidence: .issueops/evidence/pipefix-helper-large.txt
  Scenario: fn 에러 전파 (failure)
    ...
    Evidence: .issueops/evidence/pipefix-helper-error.txt
  ```
...
- [ ] 4. 취약 캡처 헬퍼 전수 교체
  ...
  **What to do**: grep 목록의 write-then-read 헬퍼를 `testsupport.CaptureStdout` 호출로 교체. 대상(현 시점 확인분): `cmd/issueops/{loopcli,statecli,policycli(2),projectcli(2),basiccli,qualitycli,commandstep,hookcli(hook_user_prompt/vcsissue/hook_post_tool_use),selfworkflow/{historycompare,candidatescmd},issueopscli(2)}` + 착수 시 `rg -n 'io.ReadAll' $(rg -ln 'os\.Pipe\(\)' --type go)`로 재수집해 신규 발생분 포함. **mcpcli/daemoncli는 착수 시 main tree의 다른 세션 변경과 겹치는지 확인 후, 겹치면 보류 목록으로 이월하고 계획 파일에 기록**. 각 파일은 로컬 헬퍼 함수 삭제 + import 추가의 기계적 diff로 제한. 패키지별 targeted test 즉시 실행.

  **Must NOT do**: updatecli 제외. issueopsapp(이미 수정됨)은 testsupport로 옮기지 않음(동작 동일한 중복이지만 다른 세션 활동 영역 — 이월 항목으로만 기록). assertion 로직 변경 금지.
  **Recommended Agent**: deep (12+ 파일 기계 교체, 패키지별 검증)
  **References**: T3 헬퍼; 취약형 원형 `cmd/issueops/loopcli/loop_cli_test.go:88-110`
  **Acceptance Criteria**: 교체 패키지 각각 `go test <pkg> -count=1 -timeout 120s` 통과; `rg -c 'os\.Pipe\(\)' cmd/issueops/loopcli ...` 교체 대상에서 0
  **QA**:
  ```
  Scenario: 행 재발 방지 회귀 (happy — 핵심)
    Channel: bash — go test ./cmd/issueops/loopcli -count=1 -timeout 120s
    Expected: 두 패키지 ok, 120s 내 완료 (교체 전에는 파이프 강등 시 600s 행이던 케이스)
    Evidence: .issueops/evidence/pipefix-sweep-core.txt
  Scenario: 잔존 취약 패턴 없음 (failure 감시)
    ...
    Evidence: .issueops/evidence/pipefix-sweep-audit.txt
  ```
...
- [ ] 5. doctor `pipe_capacity` 진단 체크 추가
  ...
  **QA**:
  ...
  Scenario: 진단 필드 (happy)
    Channel: bash — ISSUEOPS_STATE_DIR=$(mktemp -d) ./bin/issueops doctor --repo . --json | rg pipe_capacity
    Expected: pipe_capacity_bytes 숫자 필드 존재 (현 머신에선 512 관측 예상 → warning 동반)
    Evidence: .issueops/evidence/pipefix-doctor.txt
  Scenario: 정상 머신 무경고 (경계)
    Channel: bash — 단위 테스트에서 임계값 주입으로 8192 이상 케이스 검증
    Expected: warning 부재
    Evidence: .issueops/evidence/pipefix-doctor-ok.txt
  ```
...
- [ ] 7. 전체 검증 + 충돌 체크 + 병합 준비
  ...
  **Recommended Agent**: quick
  **References**: `.issueops/TESTING.md` §2/§5, T2의 충돌 체크 규약
  ...
  **QA**:
  ...
  Scenario: 압력 하 전체 스위트 (happy — 최종 증명)
    ...
    Evidence: .issueops/evidence/pipefix-full-suite.txt
  Scenario: 세션 충돌 0 (failure 감시)
    Channel: bash — comm -12 <(브랜치 변경 파일 sort) <(main 타 세션 변경 파일 sort)
    Expected: 출력 없음
    Evidence: .issueops/evidence/pipefix-conflict-check.txt
  ```
...
- [ ] F4. 스코프: main tree 불간섭 최종 확인, 이월 목록(mcpcli/daemoncli/issueopsapp 통합 여부) 문서화 확인
