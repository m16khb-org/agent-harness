...
- 테스트: **`cmd/issueops/issueopsapp` `TestResponseContractsGolden` 결정적 FAIL** (§2.1) — 나머지 전 패키지 ok
...
| ID | 심각도 | 발견 | 근거 | 처방 |
|----|--------|------|------|------|
| A-1 | **P1** | adapter→cmd 의존 역전: gitlab provider가 `cmd/issueops/issueopscli/remoteparse`를 import | `internal/adapter/provider/gitlab/provider.go:11,629` | `remoteparse`를 `internal/adapter/provider/remoteparse/`로 이동(stdlib-only라 안전). blast 3파일, 로직 무변경 |
...
| A-5 | P3 | `remotecmd/remote.go` 747 LOC 다중 관심사 | `cmd/issueops/issueopscli/remotecmd/remote.go` | 필수 아님. 성장 시 verb 그룹으로 파일 분리 |
...
- **T-1 [P0-운영] `TestResponseContractsGolden` 결정적 FAIL**: 커밋 `6dda6cb`가 `model/types.go`에 `RegressEvents`(`regress_events`, omitempty) 필드를 추가하며 "response goldens unchanged"라 주장했지만, golden fixture의 IssueOps 시나리오가 regress를 유발해 필드가 실제로 채워짐 → `cmd/issueops/testdata/response_contracts.golden.json` 5개 위치 diff. 3회 재실행 모두 FAIL(플레이키 아님). CI가 `go test ./... -count=1`을 돌리므로 **매 push/PR 머지 게이트 차단**. 수정: `-update` 후 diff 검토·커밋(TESTING.md §4 "의도적 계약 변경 시에만 golden 갱신" 절차 준수).
...
| Q-2 | P2 | self-augment lesson 패널티에 최근성 윈도우/감쇠 없음 → severe ≥2 후보는 개선 후에도 매 planning −30점 영구 강등, curriculum rotation에서 배제 | `cmd/issueops/selfworkflow/augmentplan/plan_lesson_penalty.go:32-82` (decay/리셋 grep 0건) | 최근성 윈도우(N일) 또는 감쇠/총량 캡 또는 후보 close 시 lesson 무효화. ※ 영속이 의도된 설계인지 선확인 |
...
| T-5 | P2 | `mcp_sdk_server.go` 9개 함수 전부 0% — 참조는 `mcp_transport.go:27` 1곳뿐이고 그 경로도 테스트 미실행. dead code인지 대기 코드인지 불명 | `cmd/issueops/mcpcli/mcp_sdk_server.go` | 정체 판정 후 삭제 또는 통합 테스트 1개 (PROJECT_AUDIT M1의 "SDK transport 의도적 유지" 결정과 대조 필요) |
...
|------|------|--------|-----------|------|------|
| 0 | **T-1 golden 갱신** — `-update` 후 diff 검토·커밋 | P0-운영 | 高/小 | — (즉시) | `go test ./cmd/issueops/... -count=1` green |
