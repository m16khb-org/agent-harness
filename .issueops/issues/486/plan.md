...
v2: design-review `revise` 반영 — `!run.Executed` 분기 삭제(정책 거부에서 이미 반환, 실행 후 항상 true), #485(`484-gates-check-shell-chain`) 위에 스택(같은 함수·같은 문서 절), 실환경 검증에 clean tree 선행조건.

이슈: https://github.com/m16khb-org/issueops/issues/486
...
| T2 | 문서: `skills/issueops/SKILL.md` 게이트 규칙에 "CHECK는 exit 0이어야 한다; 비영 종료가 정상인 도구(grep no-match 등)는 `python3 -c`로 감싸 0으로 끝낸다" 한 줄, `.issueops/CAUTIONS.md` 한 줄 | 문서 | validate-skill |
