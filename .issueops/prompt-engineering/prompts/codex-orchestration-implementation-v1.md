...
> **Archived prompt — do not execute for current IssueOps work.** It targets the retired schema-7 orchestration plan and is retained only as prompt-history evidence. Current owners must use `.issueops/prompt-engineering/prompts/issueops-v1-owner-execution-v1.md` and the schema-1 `issueops execution` catalog.
...
```text
You are a senior Go engineer working in the issueops repository. You implement EXACTLY ONE
...
7. Git discipline: stage exact paths only (never `git add .`, never `git commit -a`).
   ...
   with a body in the repo's Conventional Commit + Lore format (see .issueops/COMMIT_POLICY.md).
   Do NOT push.
...
9. Read before writing: AGENTS.md (root), .issueops/CONVENTIONS.md,
   .issueops/TESTING.md, and the two documents above, before your first edit.
...
- Contract/golden surfaces: cmd/issueops/testdata/*.golden.json, usage goldens, and the
  cmd/issueops/contractgolden package. If your task changes a CLI/MCP surface, update the
...
3. After the final step (including the commit), run the task's listed package tests once more
   plus `go build -o bin/issueops ./cmd/issueops` to confirm a clean state.
...
- **Adversarial cases**: plan/spec conflict → constraint 2 (spec wins + report, no silent choice); scope-creep temptation ("while I'm here" refactors) → constraint 1; fake-tool risk → prompt names only real host surfaces (shell, go, git; no MCP tool names, since Codex may not have the issueops MCP); secrets/network → constraint 5 forbids external API usage outright.
...
sed 's/{TASK_ID}/17/g' .issueops/prompt-engineering/prompts/codex-orchestration-implementation-v1.md \
