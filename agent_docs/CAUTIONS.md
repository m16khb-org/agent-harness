...
- workspace root 밖 파일 접근을 기본 거부한다.
...
- `configs/codex/skills/<name>`과 `configs/claude/skills/<name>`은 원본으로 연결한다.
- 스킬 수정 후 양쪽 host 경로가 같은 원본을 가리키는지 확인한다.

---

## 9. Self-augmentation drift

자가증강 루프가 실제 native integration을 검증하지 않으면 문서만 통과하는 가짜 안정성이 생긴다.

주의:
- 새 CLI/MCP/native skill 기능은 `harness self-augment`에 smoke 또는 fuzz 단계로 승격한다.
