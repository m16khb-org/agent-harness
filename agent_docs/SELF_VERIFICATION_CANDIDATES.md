...
1. `self-verify-install-dry-run-smoke`: install-native dry-run을 독립 evidence label로 승격한다.
...
다음 자가 증강 cycle에서 실제 구현 후보를 고를 때는 아직 완료되지 않은 후보 중 가장 높은 점수인 `self-verify-install-dry-run-smoke`를 기본 1순위로 본다.
