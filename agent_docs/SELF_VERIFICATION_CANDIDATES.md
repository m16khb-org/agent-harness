...
1. `self-verify-duplicate-mcp-warning`: user/project MCP endpoint 중복 경고를 native integration 위험으로 분류한다.
2. `self-verify-daemon-restart-resilience`: daemon-backed MCP proxy의 stale lock/socket 복구를 다룬다.
...
다음 자가 증강 cycle에서 실제 구현 후보를 고를 때는 아직 완료되지 않은 후보 중 가장 높은 점수인 `self-verify-duplicate-mcp-warning`을 기본 1순위로 본다.
