...
1. `self-verify-step-budget-baseline`: label별 성능 budget/분산을 추적해 느린 회귀를 더 일찍 잡는다.
2. `self-verify-install-dry-run-smoke`: install-native dry-run을 독립 evidence label로 승격한다.
...
다음 자가 증강 cycle에서 실제 구현 후보를 고를 때는 아직 완료되지 않은 후보 중 가장 높은 점수인 `self-verify-step-budget-baseline`을 기본 1순위로 본다.
