...
| 6 | `self-verify-candidate-export` | curriculum | 77 | self-augment 후보가 모두 `already_satisfied`가 된 뒤 다음 self-verify 후보를 별도 명령으로 뽑을 수 있어야 반복 개선이 끊기지 않는다. | 구현됨: `self-verify candidates --json` CLI/MCP golden, state save/read, candidate export self-verify step |
| 7 | `self-verify-step-budget-baseline` | 성능 | 76 | `slowest_steps` top 5는 있으나 label별 budget/분산을 추적하지 않으면 “항상 조금씩 느려지는” 회귀를 놓친다. | 구현됨: `summary.step_duration_stats`, p95 `step_budget:*` regression, slowest top5 밖 label budget fixture |
| 8 | `self-verify-install-dry-run-smoke` | native integration | 76 | `install-native --dry-run`은 구현되었지만 self-verify 단계에 독립 evidence label로 노출되면 no-write 설치 계약이 더 분명해진다. | 구현됨: temp HOME/CODEX_HOME/HARNESS_ROOT dry-run smoke, planned writes/links assertion, no-write path assertion |
...
| 16 | `self-verify-daemon-restart-resilience` | daemon | 68 | daemon-backed MCP proxy가 핵심이 되면 stale lock, socket permission, restart 후 state 재연결을 self-verify가 다뤄야 한다. | 구현됨: daemon temp socket fixture, restart smoke, stale lock recovery test |

## 4. 추천 실행 순서

현재 open 후보 없음. 새 후보가 필요하면 다음 self-verify 실패/운영 병목을 관찰한 뒤 이 문서에 추가한다.
...
다음 자가 증강 cycle에서 실제 구현 후보를 고를 때는 `self-verify candidates --json`의 `open_candidate_ids`를 먼저 확인한다. 현재 목록은 모두 구현 완료 상태다.
