...
8. command policy smoke
9. MCP smoke
10. llm-wiki fixture guard: MCP/CLI smoke가 사용자 durable wiki 대신 temp fixture와 isolated HOME만 사용하는지 확인
11. state roundtrip, prune, doctor, migrate, compare/promote/history and history retention smoke
12. parallel isolation: 동시 실행 중 temp state, daemon dir, llm-wiki fixture, build artifact path가 충돌하지 않는지 확인
13. daemon resilience: stale lock/socket 상태에서 daemon start/status/stop 복구와 socket permission을 확인
14. git preflight fuzz
15. native Codex/Claude integration smoke, including duplicate MCP warning classification fixture
16. redaction audit: docs, skill metadata, golden response artifacts에 unredacted secret-like 문자열이 없는지 확인
17. **QA gate**: `GENIUS_THINK.md`, 루프 문서, skill frontmatter/openai metadata, self-augment skill 존재 여부 확인
...
자기 검증 루프 자체의 다음 개선 후보는 `agent_docs/SELF_VERIFICATION_CANDIDATES.md`에 기록한다. progress heartbeat, secret redaction audit, coverage gap report, failure rerun recipe, policy path fuzz plus, JSON schema contract, flake classifier, output size budget, history retention budget, parallel temp isolation, duplicate MCP warning, daemon restart resilience, llm-wiki fixture guard, candidate export는 구현됐고, 현재 미완료 1순위는 step budget baseline이다.
