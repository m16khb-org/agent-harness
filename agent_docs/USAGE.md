...
`self-verify --json`은 전체 run/step count, 실패 위치, step label coverage, candidate export, contract version/hash, coverage claim matrix/`coverage_gaps`, 실패 시 `failure_class`/`failure_clusters`/`rerun_commands`, bounded stdout/stderr metadata, parallel isolation evidence, llm-wiki fixture guard evidence, duplicate MCP warning classification, daemon resilience evidence, install dry-run no-write smoke, 가장 느린 step 상위 5개, label별 `step_duration_stats` p95 budget, 목표별 `goal_scores`를 `summary`에 포함한다. 실패 triage는 먼저 `summary.failed_*`, `summary.goal_scores`, `summary.slowest_steps`, `summary.step_duration_stats`를 확인한다.
