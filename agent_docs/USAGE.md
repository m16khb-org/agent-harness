...
`self-verify --json`은 전체 run/step count, 실패 위치, step label coverage, 가장 느린 step 상위 5개, 목표별 `goal_scores`를 `summary`에 포함한다. 실패 triage는 먼저 `summary.failed_*`, `summary.goal_scores`, `summary.slowest_steps`를 확인한다.

`--save-state`를 함께 쓰면 전체 runs 로그가 아니라 compact summary snapshot만 `state_key`에 저장한다. `history`, `compare`, `promote`는 저장된 summary checkpoint를 조회·비교·승격한다.
