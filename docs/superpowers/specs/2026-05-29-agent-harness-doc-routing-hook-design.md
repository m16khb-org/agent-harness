...
- Ensure `.agent-harness` documents are considered before work starts when the task implies project rules, architecture, operations, tests, API docs, commits, or durable decisions.
...
- Maintain host-neutral behavior across Codex and Claude Code by keeping routing logic in the shared Go core behind `agent-harness hook user-prompt`.
...
1. **Always-on routing contract**
   - Inject a brief reminder that `.agent-harness` documents are the project operating source of truth.
...
1. Host calls `agent-harness hook user-prompt` on `UserPromptSubmit`.
...
- The hook design makes `.agent-harness` document routing the first-class responsibility.
