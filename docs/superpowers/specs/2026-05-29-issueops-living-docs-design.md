...
`.issueops/*.md` should be living project knowledge, not stale bootstrap output. The harness should notice user decisions, agent-visible workflow outcomes, code changes, and verification signals, then help agents inject or update the right project documents at the right time.
...
- Do not put lifecycle runtime state in `.issueops/` by default.
...
Agent-harness owns lifecycle state schema, validation, and migration logic in its Go core. Target repositories contain only shared knowledge documents under `.issueops/`.

Default runtime state lives outside the repository and is strictly project-scoped:

```text
~/.local/state/issueops/projects/<repo-id>/
```

`<repo-id>` must be derived from stable local repo evidence such as canonical root path, git repository identity, and git origin metadata when available, then hashed so multiple repositories on the same machine do not share lifecycle state. The state directory must contain a small `project.json` fingerprint with the resolved root and git metadata used to create the id. Every hook invocation should re-check that fingerprint before trusting existing state. If the current repo does not match the stored fingerprint, the hook should ignore that state and `issueops doctor` should report a namespace mismatch rather than merging data.
...
.issueops/state/
.issueops/STATE.md
.issueops/state.schema.json
```

If repo-local runtime or schema-looking files are found, `issueops doctor` should warn because team member state can pollute shared history. `STATE.md` should not be universally forbidden; warn only when it appears to be runtime/schema plumbing rather than intentional project documentation.
...
- known `.issueops` docs present in the repo
...
`issueops project bootstrap` should be the normal place to initialize the project-scoped lifecycle namespace, but it must initialize it in user-state, not in the target repo.
...
- Bootstrap still writes target repo `.issueops/*.md` only when `--write` is explicitly set, preserving existing dry-run semantics.
...
- If `project.json` already exists and its fingerprint mismatches the current repo, bootstrap must not merge state. It should return a warning and let `issueops doctor` propose migration or cleanup.
...
4. Select only the relevant `.issueops` docs.
...
- `.issueops` doc changed -> mark related queue entries applied or superseded
...
```bash
issueops doctor
issueops doctor --json
```

This is the user-facing comprehensive diagnostic command. Existing `issueops state doctor` may remain as a narrower internal state-store integrity check, but docs should prefer top-level `doctor` for general troubleshooting.

### Diagnostic scope

`issueops doctor` should check:
...
7. `issueops` MCP server registration
8. daemon status
...
12. `.issueops` docs presence and obvious missing-doc conditions
  ...
  "issues": [
    {
      ...
      "severity": "warning",
      "summary": ".issueops/state contains runtime state in a team repo.",
      "fix": {
        "command": "issueops doctor --fix",
        "destructive": false,
...
- top-level `issueops doctor --json` reports comprehensive issue records
...
6. Add top-level `issueops doctor` with read-only diagnostics and JSON output.
7. Update generated usage, response contracts, docs, and golden tests.
8. Update `.issueops/OPERATIONS.md`, `ARCHITECTURE.md`, `CONVENTIONS.md`, and `TESTING.md` only where implementation evidence requires it.
