...
**Goal:** Make `issueops hook user-prompt` prioritize concise `.issueops` document routing hints while keeping external companion tools secondary.

**Architecture:** Extend the existing shared Go hook analyzer in `internal/core/hook_prompt.go` rather than adding host-specific logic. Tests in `internal/core/hook_prompt_test.go` lock the document-first routing behavior and preserve existing API-doc behavior. The CLI wrapper in `cmd/issueops/hook_user_prompt.go` remains unchanged because it already delegates to the shared core and emits host-compatible hook JSON.
...
func TestBuildUserPromptMCPHintsRoutesArchitectureDocs(t *testing.T) {
	...
	for _, want := range []string{"issueops project-doc routing hint", "ARCHITECTURE.md", "ADR.md", "project_docs_route"} {
...
func TestBuildUserPromptMCPHintsCompanionToolsStaySecondary(t *testing.T) {
	...
	projectIndex := strings.Index(got.AdditionalContext, "issueops project-doc routing hint")
...
Expected: FAIL because the current hook renderer still uses `issueops MCP routing hint` and does not emit the new document-specific or companion-tool sections.
...
```go
add("project_docs_route", "Before acting, decide which AGENTS.md/.issueops documents are required for this task.")
...
- Start with `issueops project-doc routing hint:`.
...
Expected: JSON contains `hookSpecificOutput`, `issueops project-doc routing hint`, `ARCHITECTURE.md`, `ADR.md`, `TESTING.md`, and `AGENT_WORKFLOW.md`.
...
- Modify: `internal/core/hook_prompt.go`
- Modify: `internal/core/hook_prompt_test.go`
- Create: `docs/superpowers/plans/2026-05-29-issueops-doc-routing-hook.md`
...
```bash
git diff -- internal/core/hook_prompt.go internal/core/hook_prompt_test.go docs/superpowers/plans/2026-05-29-issueops-doc-routing-hook.md
```
