...
**Goal:** Add project-scoped lifecycle state initialization and a top-level `agent-harness doctor` that diagnoses harness health and doc-lifecycle issues without storing runtime state in target repos.
...
- Modify `cmd/harness/main.go`: add top-level `doctor`, extend `project bootstrap` output, and pass repo to hook requests.
- Modify `internal/adapter/cli/usage.go` and `cmd/harness/testdata/usage.golden.txt`: document `agent-harness doctor`.
- Modify golden contract snapshots only after tests indicate exact output changes.
- Modify `.agent-harness/OPERATIONS.md`, `.agent-harness/ARCHITECTURE.md`, `.agent-harness/CONVENTIONS.md`, `.agent-harness/TESTING.md`, and README only where implementation evidence changes user-facing behavior.
...
```bash
git add internal/core/project_docs.go internal/core/project_docs_test.go cmd/harness/main.go
...
In `cmd/harness/hook_user_prompt.go`, derive repo from hook JSON if present or `resolveTarget("")` and pass it into `HookUserPromptRequest`.
...
```bash
git add internal/core/hook_prompt.go internal/core/hook_prompt_test.go cmd/harness/hook_user_prompt.go
...
- `.agent-harness/state`, `.agent-harness/state.schema.json`, runtime-looking `.agent-harness/STATE.md`
- docs presence
...
```bash
go test ./internal/core -run 'TestHarnessDoctor' -count=1
go test ./internal/adapter/cli ./cmd/harness -run 'Usage|Golden' -count=1
```
...
```bash
git add internal/core/doctor.go internal/core/doctor_test.go cmd/harness/main.go internal/adapter/cli/usage.go cmd/harness/testdata/usage.golden.txt
git commit -m "feat(cli): add comprehensive harness doctor"
```
...
- `agent-harness doctor` is the general diagnostic command
- `state doctor` remains narrower state-store integrity check
- runtime state/schema files should not be committed under `.agent-harness`
...
```bash
git add .agent-harness README.md cmd/harness/testdata internal cmd/harness docs/superpowers/plans/2026-05-30-agent-harness-living-docs-doctor.md
