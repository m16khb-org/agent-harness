# `io` Command Shim Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `issueops` install/update가 안전한 `io` command shim을 관리하고, symlink로 실행된 CLI가 repo 밖에서도 현재 checkout을 찾아 `io update`를 수행하게 한다.

**Architecture:** `pathutil`이 executable symlink chain을 해석해 harness root 후보를 만든다. 공통 `installcli` path plan은 canonical `issueops` shim을 먼저 관리하고, 고정 target의 `io` shim을 충돌 안전 정책으로 추가한다. Host adapter에는 로직을 복제하지 않는다.

**Tech Stack:** Go 1.26.3, 표준 라이브러리 `os`/`path/filepath`, 기존 `internal/adapter/installutil`, 표준 `testing`, Markdown 운영 문서.

## Global Constraints

- Canonical binary와 public command identity는 계속 `issueops`다.
- `io`는 `~/.local/bin/issueops`를 가리키는 installer-owned convenience symlink다.
- 기존 `io` regular file, directory, unrelated symlink를 덮어쓰지 않는다.
...
- `cmd/issueops/pathutil/path_helpers.go`: executable 원본/resolved path에서 harness root를 찾는 공통 로직.
- `cmd/issueops/pathutil/path_helpers_test.go`: symlink chain root 탐색 regression.
- `cmd/issueops/installcli/install_native_path.go`: canonical/short command shim plan과 `io` 충돌 정책.
- `cmd/issueops/installcli/install_command_test.go`: path mode별 두 shim 및 충돌/no-op 계약.
- `cmd/issueops/installcli/install_command_helpers_test.go`: install result link assertion 재사용.
- `scripts/install-native.sh`: installer help에 `io` command shim 표시.
- `README.md`: 사용자-facing `io update` 사용법.
- `.issueops/operations/install.md`: install/update command shim 운영 계약.
- `.issueops/CONVENTIONS.md`: installer-owned command shim symlink 예외.
- `.issueops/ADR.md`: canonical identity + managed shorthand 결정과 기각 대안.
- `cmd/issueops/testdata/response_contracts.golden.json`: `.issueops` 문서 변경으로 발생한 docs-index projection만 갱신.
...
- Modify: `cmd/issueops/pathutil/path_helpers.go:17-40`
- Test: `cmd/issueops/pathutil/path_helpers_test.go`
...
func TestHarnessRootFromFollowsExecutableSymlink(t *testing.T) {
	...
	canonical := filepath.Join(userBin, "issueops")
	short := filepath.Join(userBin, "io")
...
```bash
go test ./cmd/issueops/pathutil -run TestHarnessRootFromFollowsExecutableSymlink -count=1
```
...
```bash
go test ./cmd/issueops/pathutil -count=1
```
...
### Task 2: 안전한 `io` command shim 설치

**Files:**

- Modify: `cmd/issueops/installcli/install_native_path.go:13-51`
- Test: `cmd/issueops/installcli/install_command_test.go`
...
if !hasInstallLink(result.Links, short, canonical, true) {
	t.Fatalf("%s path mode did not plan io command shim: %+v", mode, result.Links)
}
```
...
```bash
go test ./cmd/issueops/installcli -run 'TestInstallCommandDryRun(Auto|Manual|Skip)PathMode' -count=1
```

Expected: FAIL because no link has path ending in `/.local/bin/io`.
...
```bash
go test ./cmd/issueops/installcli -run 'TestInstallCommandShortShim' -count=1
```

Expected: matching-link test misses the planned link and collision tests do not return the required `io`-specific error.
...
### Task 3: Repo 밖 `io update` smoke를 고정한다

**Files:**

- Modify: `cmd/issueops/pathutil/path_helpers_test.go`
- Test: `cmd/issueops/updatecli/update_bootstrap_test.go`

**Interfaces:**

- Consumes: Task 1의 `harnessRootFrom`
- Consumes: Task 2의 `io -> issueops -> repo binary` link shape
...
```bash
go test ./cmd/issueops/updatecli -run TestRunUpdateUsesResolvedHarnessRootOutsideCheckout -count=1
```
...
- Modify: `.issueops/CONVENTIONS.md:140-145`
- Modify: `.issueops/ADR.md`
- Update generated projection: `cmd/issueops/testdata/response_contracts.golden.json`
...
Expected: FAIL because current help does not mention `~/.local/bin/io`.
...
문서에는 `issueops`가 canonical identity이고 `io` 충돌은 overwrite 없이
실패하며, `io`는 shell alias가 아닌 managed symlink임을 명시한다.

`CONVENTIONS.md`의 symlink 규칙은 다음으로 좁힌다.

```text
기본 symlink는 user skill 원본 연결과 installer-owned command shim
(`~/.local/bin/issueops`, `~/.local/bin/io`)에만 사용한다.
```

`.issueops/ADR.md`에는 날짜 `2026-07-24`, source `user directive`, decision,
...
Expected: PASS and only the `.issueops/ADR.md` / `CONVENTIONS.md` docs-index
...
```bash
gofmt -w cmd/issueops/pathutil/path_helpers.go cmd/issueops/pathutil/path_helpers_test.go cmd/issueops/installcli/install_native_path.go cmd/issueops/installcli/install_command_test.go cmd/issueops/updatecli/update_bootstrap_test.go internal/adapter/install_contract_matrix_test.go
go test ./cmd/issueops/pathutil ./cmd/issueops/installcli ./cmd/issueops/updatecli ./internal/adapter -count=1
```
...
ln -s "$PWD/bin/issueops" "$tmp_root/home/.local/bin/issueops"
ln -s "$tmp_root/home/.local/bin/issueops" "$tmp_root/home/.local/bin/io"
(cd "$tmp_root/outside" && HOME="$tmp_root/home" "$tmp_root/home/.local/bin/io" update --dry-run --path-mode=skip --json)
rm -rf "$tmp_root"
```
...
untracked files. 실제 `~/.local/bin/io`는 생성하지 않았으므로 live `command -v io`
결과는 작업 전 상태와 같다.
