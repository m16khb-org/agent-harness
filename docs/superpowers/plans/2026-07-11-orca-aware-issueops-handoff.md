...
- `StartIssueOpsHandoff` requires a concrete coordinator mailbox recipient, renders/persists it with context version/hash, then executes terminal-create, task-create, and dispatch one at a time.
...
- `TestHandoffStartRecoversRuntimeReissuedTerminalWithoutDuplicateCreate`
- `TestHandoffStartRuntimeRestartRequiresUniqueExactWorktree`
- `TestHandoffRuntimeRestartReconcileResumesToDispatchWithoutDuplicateCreate`
- `TestHandoffStartTerminalCapabilityLossPreservesProvisionedLease`
...
- Completed finish validates bounded HEAD/files/Turing report/command evidence/cleanup receipts and repeats the sealed context-source check. Under one cycle lock it transitions `claimed -> submitted` and persists the deterministic projection intent in the same write. Only after that write is visible may it invoke one external `worker_done` outside the lock. The sender is the sealed historical worker mailbox, never the refreshable live terminal; recipient/task/dispatch/files/report/final-head/host/attempt/subject/body derive only from persisted authority and freshly verified committed evidence. Failure, timeout, malformed output, ambiguity, or crash is terminal for projection and is never automatically retried. Failed finish closes with `worker_failed`. Identical finish repeats return stable evidence; conflicting repeats fail.
...
- `TestHandoffFinishProjectionUsesSealedWorkerMailboxAfterLiveTerminalRollover`
- `TestHandoffFinishPersistsSubmittedAndProjectionIntentAtCrashBoundary`
- `TestHandoffFinishCrashIntentRecoveryNeverRetries`
- `TestHandoffDispatchPreambleRequiresOfficialLabeledIdentityLines`
...
The fresh worker stops only after it has committed the scoped implementation and submitted `issueops handoff finish --outcome completed` with the Turing report and cleanup receipts; that finish performs the bounded automatic Orca `worker_done` projection and returns its durable evidence. It must not send manual completion messages, push, open/merge a PR, accept its own handoff, delete the worktree, or clean the provider branch.
