...
## Context
### Original Request
- "현재 issueops에서 개선되어야 할 점이나 보완되어야 할 점을 수색"
- Explicit skill triggers: `omo:start-work`, `omo:ulw-plan`.

### Research Summary
- `go test ./... -count=1` fails in `cmd/issueops` at `cmd/issueops/response_contract_golden_test.go:200` because `response_contracts.golden.json` is stale relative to current response contracts.
- `self-verify --iterations=10 --seed=100 --target-score=95 --json` fails immediately because the same `go test` gate fails.
- `README.md:135` documents `issueops update`, and `cmd/issueops/main.go:137` dispatches it, but `internal/adapter/cli/usage.go:43` usage text omits it.
...
- `.issueops/ARCHITECTURE.md:154` says command execution is only policy check + fake runner, but `internal/core/policy.go:298` implements real argv-only read-only execution.
- `internal/core/draft_wiki_queue.go:119` reads and rewrites the draft-wiki queue without visible locking around processing.
- Worker CLI exposes `worker run --read-only` (`cmd/issueops/worker.go:95`), but MCP adapter worker tools stop at enqueue/status/list/cancel (`internal/adapter/mcp/catalog.go:25`).
- PreToolUse is currently non-blocking and emits `{}` for host output (`cmd/issueops/hook_user_prompt.go:133`, `cmd/issueops/hook_user_prompt.go:158`), while raw JSON shows `decision: allow`; docs warn blocking requires deterministic policy and host-schema tests (`.issueops/AGENT_WORKFLOW.md:45`, `.issueops/CAUTIONS.md:167`).
...
## Work Objectives
### Core Objective
Make issueops green and reduce operator confusion by aligning tests, command surfaces, docs, and worker/draft-wiki operational contracts.

### Definition of Done
- `go test ./... -count=1` passes.
- `./bin/issueops self-verify --iterations=10 --seed=100 --target-score=95 --json` passes or any remaining failure is a new, named issue with evidence.
- `issueops --help`/usage, README, CLI dispatch, and contract snapshots agree on public commands.
...
- Evidence directory: `evidence/issueops-improvement-scan/`.
...
- [ ] 1. Restore response-contract golden/test baseline
  ...
  **What to do**: Reproduce `TestResponseContractsGolden` failure, inspect the generated diff, decide whether current behavior or the golden is wrong, then either fix the behavior or update `cmd/issueops/testdata/response_contracts.golden.json` with rationale.
  ...
  **References**:
  - Failure: `cmd/issueops/response_contract_golden_test.go:200` - golden assertion.
  - Evidence command: `go test ./... -count=1` currently fails in `cmd/issueops`.

  **Acceptance Criteria**:
  - [ ] A RED reproduction for `TestResponseContractsGolden` is captured before changes.
  - [ ] `go test ./cmd/issueops -run TestResponseContractsGolden -count=1` passes after changes.
  ...
  **QA Scenarios**:
  ...
  Scenario: response contract check is green
    Tool: tmux
    Steps: tmux new-session -d -s qa-contract 'cd /Users/user/Workspace/issueops && go test ./cmd/issueops -run TestResponseContractsGolden -count=1; echo EXIT:$?'; capture pane
    Expected: transcript contains PASS and EXIT:0
    Evidence: evidence/issueops-improvement-scan/task-1-contract.txt

  Scenario: stale golden fails before fix
    Tool: bash
    Steps: go test ./cmd/issueops -run TestResponseContractsGolden -count=1 before implementation
    Expected: failure output includes response_contracts.golden.json mismatch
    Evidence: evidence/issueops-improvement-scan/task-1-red.txt
  ```

  **Commit**: YES | Message: `test(contract): restore response contract golden baseline` | Files: `cmd/issueops/testdata/response_contracts.golden.json` or source fix files

- [ ] 2. Align `update` command across dispatch, help, README, and contracts

  **What to do**: Add or confirm `update` in canonical CLI usage/catalog and contract snapshots, matching `cmd/issueops/main.go:137` and README guidance.
  ...
  **References**:
  - Dispatch: `cmd/issueops/main.go:137`.
  ...
  **Acceptance Criteria**:
  ...
  - [ ] Usage/golden tests include `issueops update`.
  ...
  **QA Scenarios**:
  ...
  Scenario: update appears in help
    Tool: tmux
    Steps: tmux new-session -d -s qa-update-help 'cd /Users/user/Workspace/issueops && ./bin/issueops --help | grep "issueops update"; echo EXIT:$?'; capture pane
    Expected: transcript contains issueops update and EXIT:0
    Evidence: evidence/issueops-improvement-scan/task-2-help.txt

  Scenario: dry-run update is safe
    Tool: bash
    Steps: HOME=$(mktemp -d) CODEX_HOME=$HOME/.codex ./bin/issueops update --dry-run --json
    Expected: JSON reports planned actions without writing real user home
    Evidence: evidence/issueops-improvement-scan/task-2-dry-run.json
  ```
...
- [ ] 3. Refresh stale project docs that still describe the repo as initial or fake-run only
  ...
  **References**:
  ...
  - Stale fake-run-only architecture: `.issueops/ARCHITECTURE.md:154`.
  ...
  **Acceptance Criteria**:
  ...
  - [ ] `./bin/issueops docs --json` includes updated docs without parse errors.

  **QA Scenarios**:
  ```
  Scenario: docs index includes updated source-of-truth text
    Tool: tmux
    Steps: tmux new-session -d -s qa-docs 'cd /Users/user/Workspace/issueops && ./bin/issueops docs --json | python3 -m json.tool >/tmp/issueops-docs.json; grep -q ARCHITECTURE /tmp/issueops-docs.json; echo EXIT:$?'; capture pane
    Expected: EXIT:0
    Evidence: evidence/issueops-improvement-scan/task-3-docs.txt

  Scenario: no stale initial-code claim remains
    Tool: bash
    Steps: rg "아직 애플리케이션 코드가 없는|policy check \+ fake runner" AGENTS.md .issueops README.md
    Expected: no stale false claim remains, or remaining mention is explicitly historical/qualified
    Evidence: evidence/issueops-improvement-scan/task-3-stale-scan.txt
  ```

  **Commit**: YES | Message: `docs(project): align operating docs with current harness implementation` | Files: `AGENTS.md`, `.issueops/ARCHITECTURE.md`, related docs
...
- [ ] 4. Decide and test worker CLI/MCP parity for read-only execution
  ...
  **References**:
  - CLI read-only worker: `cmd/issueops/worker.go:95`.
  ...
  **QA Scenarios**:
  ...
  Scenario: worker read-only command through chosen public surface
    Tool: tmux
    Steps: tmux new-session -d -s qa-worker 'cd /Users/user/Workspace/issueops && tmp=$(mktemp -d) && ISSUEOPS_WORKER_DIR=$tmp ./bin/issueops worker run --read-only --kind qa --workspace-root "$PWD" --cwd "$PWD" --json -- git status --short; code=$?; rm -rf "$tmp"; echo EXIT:$code'; capture pane
    Expected: JSON has status succeeded and EXIT:0
    Evidence: evidence/issueops-improvement-scan/task-4-worker.txt

  Scenario: writable worker command remains denied
    Tool: bash
    Steps: ISSUEOPS_WORKER_DIR=$(mktemp -d) ./bin/issueops worker run --read-only --kind qa --workspace-root "$PWD" --cwd "$PWD" --json -- touch should-not-exist
    Expected: non-zero exit, policy deny reason for write command, file absent
    Evidence: evidence/issueops-improvement-scan/task-4-worker-deny.txt
  ```

  **Commit**: YES | Message: `fix(worker): clarify read-only worker public surface` | Files: `internal/adapter/mcp/catalog.go`, `cmd/issueops/main.go`, docs/tests as chosen
...
- [ ] 5. Harden draft-wiki queue processing against concurrent workers and stale/racy queue state
  ...
  **QA Scenarios**:
  ...
  Scenario: two workers race on one queued draft
    ...
    Evidence: evidence/issueops-improvement-scan/task-5-race.txt
  ...
  Scenario: malformed queue line is reported safely
    ...
    Evidence: evidence/issueops-improvement-scan/task-5-malformed.txt
  ```
...
- [ ] 6. Add a small command-surface consistency guard
  ...
  **References**:
  - Dispatch table: `cmd/issueops/main.go`.
  - Adapter command list: `internal/adapter/cli/usage.go:14`.
  - Response contract failure area: `cmd/issueops/response_contract_golden_test.go:200`.
  ...
  **QA Scenarios**:
  ...
  Scenario: command consistency test passes
    Tool: tmux
    Steps: tmux new-session -d -s qa-command-surface 'cd /Users/user/Workspace/issueops && go test ./cmd/issueops ./internal/adapter/cli -run "Command|Usage|Golden" -count=1; echo EXIT:$?'; capture pane
    Expected: PASS and EXIT:0
    Evidence: evidence/issueops-improvement-scan/task-6-command-surface.txt

  Scenario: help and contract mention same command set
    Tool: bash
    Steps: ./bin/issueops contract schema --json and ./bin/issueops --help, compare top-level command names with a small script
    Expected: no missing command names
    Evidence: evidence/issueops-improvement-scan/task-6-contract-help.txt
  ```
...
- [ ] 7. Enforce CodeGraph-first source search via opt-in PreToolUse policy
  ...
  **References**:
  - Current PreToolUse no-op host output: `cmd/issueops/hook_user_prompt.go:133` and `cmd/issueops/hook_user_prompt.go:158`.
  ...
  - Existing caution against premature blocking: `.issueops/CAUTIONS.md:167`.
  - CodeGraph routing hint already exists for symbol/call-impact lookup: `internal/core/hook_prompt.go` CodeGraph hint branch.

  **Acceptance Criteria**:
  - [ ] Default `issueops hook pre-tool-use` still emits `{}` and does not block normal tool use.
  ...
  - [ ] Opt-in policy allows docs/golden/literal evidence searches such as `rg "response_contracts" cmd/issueops/testdata README.md`.
  ...
  **QA Scenarios**:
  ...
  Scenario: structural source grep is blocked when search-routing enforcement is enabled
    Tool: bash
    Steps: cd /Users/user/Workspace/issueops && printf '%s\n' '{"cwd":"/Users/user/Workspace/issueops","tool_name":"Bash","tool_input":{"command":"rg -n \"func RunReadOnlyCommand\" internal cmd"}}' | ./bin/issueops hook pre-tool-use --enforce-search-routing --json
    Expected: JSON decision is block/deny and reason tells the agent to use CodeGraph first
    Evidence: evidence/issueops-improvement-scan/task-7-search-routing-block.txt

  Scenario: docs/golden literal grep remains allowed
    Tool: bash
    Steps: printf hook JSON for `rg "response_contracts" cmd/issueops/testdata README.md` into `./bin/issueops hook pre-tool-use --enforce-search-routing --json`
    Expected: JSON decision is allow
    Evidence: evidence/issueops-improvement-scan/task-7-search-routing-allow.txt
  ```

  **Commit**: YES | Message: `feat(hook): add opt-in search routing gate` | Files: `cmd/issueops/hook_user_prompt.go`, `internal/core/lifecycle_state.go`, hook tests, install templates/docs only if enabling is requested
...
- [ ] 8. Clarify companion-tool hook timing for claude-mem and LLM Wiki recommendations
  ...
  **References**:
  ...
  - SessionStart/PostCompact catalog timing: `cmd/issueops/hook_user_prompt.go:301`, `cmd/issueops/hook_user_prompt.go:251`.
  - PostToolUse draft-wiki queue capture: `cmd/issueops/hook_user_prompt.go:192`.
  - No-reimplementation policy: `.issueops/CAUTIONS.md:122`.
  ...
  **QA Scenarios**:
  ...
  Scenario: claude-mem hint appears only for memory recall intent
    Tool: bash
    Steps: printf '{"prompt":"지난번에 이미 해결한 memory 찾아줘","cwd":"/Users/user/Workspace/issueops"}' | ./bin/issueops hook user-prompt --json
    Expected: additional_context contains `claude-mem only for previous-session/repeated-work recall`
    Evidence: evidence/issueops-improvement-scan/task-8-claude-mem-hint.json

  Scenario: LLM Wiki hint appears only for explicit wiki/research intent
    Tool: bash
    Steps: printf '{"prompt":"wiki research knowledge base 정리해줘","cwd":"/Users/user/Workspace/issueops"}' | ./bin/issueops hook user-prompt --json
    Expected: additional_context contains `LLM Wiki for explicit wiki/research work`
    Evidence: evidence/issueops-improvement-scan/task-8-llm-wiki-hint.json

  Scenario: generic code task does not over-recommend companion tools
    Tool: tmux
    Steps: tmux new-session -d -s qa-companion-generic 'cd /Users/user/Workspace/issueops && printf "{\"prompt\":\"테스트 실패 고쳐줘\",\"cwd\":\"$PWD\"}" | ./bin/issueops hook user-prompt --json; echo EXIT:$?'; capture pane
    Expected: output does not contain claude-mem or LLM Wiki, and EXIT:0
    Evidence: evidence/issueops-improvement-scan/task-8-generic.txt
  ```

  **Commit**: YES | Message: `docs(hook): clarify companion tool recommendation timing` | Files: `internal/core/hook_prompt_test.go`, `.issueops/AGENT_WORKFLOW.md`, `.issueops/OPERATIONS.md`, `.issueops/CAUTIONS.md` if behavior/docs change
