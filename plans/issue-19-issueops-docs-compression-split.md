# Issue 19: issueops project docs compression and selective split

Issue: provider issue `#19` in the current `issueops` repository
Branch: `chore/19-issueops-docs-compression-split`
Worktree: sibling IssueOps worktree for `chore-19-issueops-docs-compression-split`
...
- `.issueops` docs are agent-facing operating contracts, so path changes must preserve discoverability through required reading, docs index output, and existing routing hints.
...
1. Re-measure current docs and map duplicated sections.
   Verify with `find .issueops -maxdepth 2 -type f -name '*.md' -print | sort | xargs wc -l` and targeted `rg` for duplicated topics.
...
3. Split `OPERATIONS.md`.
   Keep a quick-start and reference map in `OPERATIONS.md`. Move detailed usage into `.issueops/operations/` subdocuments using narrow names such as `install.md`, `cli.md`, `mcp.md`, `hooks.md`, `verification.md`, and `worker.md` if the content boundaries remain clean.
...
5. Enforce IssueOps worktree edit targeting.
   Add a focused PreToolUse guard that blocks mutating tool events outside `ISSUEOPS_EXPECTED_WORKTREE` when IssueOps sets that environment variable. Keep the guard no-op outside IssueOps.
   Verify with focused hook/core tests and native install contract tests.

6. Enforce IssueOps numbered next actions.
   Add a strict-ready Stop hook guard that blocks final responses missing `1.`, `2.`, and `3.` choices only when `ISSUEOPS_EXPECT_NUMBERED_NEXT_ACTIONS=1` and final assistant text is available.
   Verify with focused hook/core tests and native install contract tests.
...
7. Update routing references.
   ...
   Verify with `./bin/issueops docs --json`, `./bin/issueops bootstrap --dry-run`, and `./bin/issueops bootstrap --sync --dry-run`.

8. Final verification.
   Run `git diff --check`, `go test ./cmd/issueops -run Golden -count=1`, and any focused tests required by changed golden/catalog output.
...
```bash
find .issueops -maxdepth 2 -type f | sort
find .issueops -maxdepth 2 -type f -name '*.md' -print | sort | xargs wc -l
rg -n "LLM Wiki|API Documentation|OpenAPI|MCP|worker|hook|IssueOps|self-verify|자기 검증" .issueops
./bin/issueops docs --json
./bin/issueops bootstrap --dry-run
./bin/issueops bootstrap --sync --dry-run
...
go test ./cmd/issueops -run 'TestRunHookPreToolUse|TestRunHookStop' -count=1
