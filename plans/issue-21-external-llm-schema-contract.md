...
Make structured `Z.AI Coding Plan` usage reliable when the provider has no native structured-output API. Every external LLM path that expects machine-readable JSON must inject the expected schema/example into the prompt and parse either strict JSON or fenced `json` JSON, while preserving read-only evaluator constraints.
...
- Add follow-up verification coverage for the actual `Z.AI Coding Plan` invocation mode/model settings.
...
1. Add focused tests for the current gaps.
   ...
   - Verify fake Z.AI fenced `json` output is accepted for IssueOps remote scoring and benchmark judge.
...
4. Wire remaining `Z.AI Coding Plan` paths.
...
5. Verify real-world behavior.
   ...
   - Run full Go tests.
   - Run real production-prompt `zai` checks across the affected commands.
   - Record follow-up evidence for whether print-mode `zai` uses the configured `glm-5-turbo` and fast mode settings.
