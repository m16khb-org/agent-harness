...
선택적 Orca 실행은 `issueops worktree prepare --orchestrator auto|orca|inline`로만 선택한다. Orca가 첫 external mutation 전에 absent/unready이면 `auto`는 inline 결과를 반환하고 ownership state를 쓰지 않는다. Orca가 선택되면 source session이 workspace와 필수 plan/gate를 준비한 뒤 ownership을 dispatch하고, fresh owner가 exact cycle을 claim하고 sealed context를 acknowledge한다. 그 뒤 구현, 검증, publication, PR/MR 생성, `handoff complete`는 canonical worker root의 owner만 수행한다. 외부 create/dispatch 직전에는 exact-worktree terminal과 server-filtered dispatched task를 다시 inventory한다. 호출 전 `pending_operation`을 먼저 저장하며, 모호한 inventory/실패에서는 create를 재시도하거나 inline으로 전환하지 않는다. pre-dispatch worktree-create recovery는 봉인된 source session이 exact epoch로 `issueops worktree reconcile`을 실행해 정확히 하나의 marker candidate만 복구하고, post-dispatch recovery는 `issueops handoff recover`가 소유한다. 완료는 `cleanup_pending_human_decision`에서 멈추며, human merge 뒤 fresh authenticated exact-source session도 승인받아 ordered cleanup receipt를 기록할 수 있다. Orca에는 atomic lease가 없으므로 이 exclusion은 harness-guarded writer에 엄격하며, final snapshot 뒤 외부 actor의 절대적 race 부재를 증명한다고 과장하지 않는다.
...
After `worktree prepare`, complete the required plan and dispatch gates, confirm ownership dispatch, and let the isolated owner claim and acknowledge the exact lifecycle. That owner performs implementation, publication, and completion from the canonical worker root. Before dispatch, an exact-ID prep-only linked worker may update its own IssueOps contract without being captured by another active cycle. The source main worktree remains available before, during, and after handoff: source disengagement applies only to steering or mutation of that transferred cycle.

Do not use source-CWD/session binding as a fence. Select the canonical worker root, exact lifecycle ID, native owner, or persisted Orca resource. Exact-ID routing precedes source-wide inference. Completion becomes `cleanup_pending_human_decision`, not automatic cleanup.
