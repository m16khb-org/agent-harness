...
선택적 Orca 실행은 `issueops worktree prepare --orchestrator auto|orca|inline`로만 선택한다. Orca가 첫 external mutation 전에 absent/unready이면 `auto`는 기존 inline JSON/text를 byte-for-byte 유지하고 `execution_handoff`, warning, state rewrite를 남기지 않는다. Orca가 선택되면 coordinator가 prepare/start, fresh worker가 claim/heartbeat/finish, coordinator가 evidence 재검증/accept를 소유한다. 외부 create/dispatch 및 retry 직전에는 exact-worktree의 모든 terminal과 server-filtered dispatched task를 다시 inventory하고, connected 또는 writable baseline terminal을 possible writer로 간주하며 designated active worker만 둘 다 true여야 한다. 외부 create/dispatch 호출 전 `pending_operation`이 먼저 저장되며, 모호한 inventory/실패에서는 create를 재시도하거나 inline으로 전환하지 않고 `recovery_required`에서 정확히 하나의 identity만 복구한다. Accepted 결과는 exact `FinalHead` push와 durable publish receipt 뒤 literal-body wrapper로만 PR/MR를 만들고, failed/cancelled cleanup은 durable disposition과 ordered receipts가 mutation authority를 소유한다. Orca에는 atomic lease가 없으므로 이 exclusion은 harness-guarded writer에 엄격하며, final snapshot 뒤 외부 actor의 절대적 race 부재를 증명한다고 과장하지 않는다.
