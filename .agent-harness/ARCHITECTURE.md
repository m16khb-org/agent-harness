...
- IssueOps root record는 `schema_version=4`다. missing/zero/v1/v2/v3는 인식 가능한 필드를 보존한 채 read-compatible하고 다음 write에서 v4가 된다. v1 binary는 v2+ handoff row를, v2 binary는 stable terminal tab/leaf가 추가된 v3 row를, v3 binary는 sealed coordinator/worker mailbox와 completion projection intent가 추가된 v4 row를 future schema로 byte-equivalent reject한다. v4보다 큰 row는 bounded identity와 invalid marker만 hook scan에 남겨 fail-closed한다.
