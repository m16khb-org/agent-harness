...
- IssueOps 제공 표면: CLI `issueops start/status/link-issue/link-plan/phase/feedback add/pr-readiness/remote score/benchmark`, MCP `issueops_start/issueops_status/issueops_link_issue/issueops_link_plan/issueops_add_feedback/issueops_set_phase/issueops_pr_readiness/issueops_remote_score`. phase는 problem/grill/plan/implement/feedback/pr/done 7단계이며 `pr`은 issue+plan linkage를 요구한다. feedback은 선택적 classification(contract_change/defect/question/noise)을 기록한다. benchmark run/compare/gate는 CLI 전용 개발/autoresearch 도구다.
