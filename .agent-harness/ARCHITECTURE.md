...
- IssueOps root record의 현재 쓰기 버전은 `schema_version=6`이다. missing/zero/v1/v2/v3/v4와 새 mutation authority가 없는 v5는 인식 가능한 필드를 보존한 채 read-compatible하고 다음 write에서 v6가 된다. v5의 historical publish/cleanup 의미는 그대로 보존하되, v6가 추가한 push-target fingerprint와 `remote_create_claim`을 v5 row에 소급해 만들지 않는다. 따라서 raw v5 row에 claim이 있거나 구형 publish receipt가 있으면 저장 bytes를 바꾸지 않고 재-attest 또는 전용 remote-create reconcile 진단으로 fail-closed한다. Genuine accepted v5 publication은 native coordinator seal 이전 형식이므로 exact source checkout의 현재 native event와 명시적 `--approve-legacy-coordinator-seal`을 함께 검증한 locked re-attestation만 v6로 쓸 수 있다. 거부된 row는 byte-identical하고, invalid closed v5 publication projection은 hook scan에 남아 direct mutation을 계속 막는다. v6보다 큰 row는 bounded identity와 invalid marker만 hook scan에 남긴다.
