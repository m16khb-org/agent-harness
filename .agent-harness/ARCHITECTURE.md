...
- IssueOps root record는 `schema_version=2`다. missing/zero/v1은 필드를 보존한 채 read-compatible하고 다음 write에서 v2가 되며, v1 binary는 v2 handoff row를 future schema로 reject해 ownership lease 삭제를 막는다. v2보다 큰 row는 bounded identity와 invalid marker만 hook scan에 남겨 fail-closed한다.
