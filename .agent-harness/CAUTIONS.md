...
- Resolution: 근본 완화는 파이프를 누수하는 호스트 프로세스(주로 장수 Codex 세션) 재시작이다. `agent-harness mcp cleanup --apply`는 부모가 죽은 고아 프록시만 정리하므로 살아 있는 호스트의 누수에는 효과가 없다. 재발 방지는 캡처 헬퍼를 6ee897d의 harnessapp 방식(동시 reader goroutine)으로 통일하는 것 — 파이프 버퍼 크기 의존이 사라진다. `go test`를 죽일 때는 `pkill -f 'go test'`가 아니라 `.test` 바이너리까지 함께 정리한다.
