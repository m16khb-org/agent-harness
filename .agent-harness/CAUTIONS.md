...
- Known residual gaps (not yet fixed — see multi-session QA): the `(repo,branch)` JSON store has NO per-id lock or `UpdatedAt` CAS, so concurrent `start`/`link`/`set-phase`/`force-release` on the same id is last-writer-wins; hot-path worktree validity is dir-exists-only (`os.Stat`+`IsDir`), so a `git worktree prune`d-but-present directory or a leftover non-git dir still reads as a valid worktree; and stale-reset / force-release rewrite only the JSON record, never cleaning the orphaned worktree dir or its `git worktree` registration. Treat these as the next hardening steps before relying on heavy multi-session parallelism.
