...
- Orca freeform steering은 `orca terminal send --terminal <handle> --text <payload> --enter --json` argv 계약을 사용한다. payload는 한 argv로 전달하거나 POSIX single-quote encoder를 정확히 한 번 적용하며, JSON double-quote·shell/JS template interpolation을 중첩하지 않는다.
