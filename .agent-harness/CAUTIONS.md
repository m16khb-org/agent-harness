...
- Consequence: you cannot both stop-and-wait AND show the user a message via raw Stop output — the only visible channel (`decision:"block"`) continues the agent. So to "notify the user that auto-proceed did not engage and let them choose", return `decision:"block"` + a `reason` that instructs the agent to relay the notice to the user and then STOP (no further work, no new choices). The follow-up Stop carries `stop_hook_active=true`, which disables this notify branch, so the relay turn ends with a no-op `{}` and never loops. Implemented in `runHookStop`.
- So the Stop outcomes are: (1) recover/continue in-turn → `decision:"block"` + `reason`; (2) notify-the-user-then-stop → `decision:"block"` + a relay-and-stop `reason` (relies on the `stop_hook_active` guard to end the follow-up); (3) silent no-op → `{}`. `continue:false` is a hard stop that suppresses the visible feedback, so avoid it for notifications.
- The Stop hook should only treat numbered lines inside an explicit `선택지:`/`Options:`/`Next actions:` section as next-action choices. Explanatory numbered lists can contain words like `추천` and `자동진행` and must not be parsed as auto-proceed choices. When a recommended choice is eligible for continuation, the hook should not claim "자동진행 후보입니다"; it should tell the main agent to re-check its own recommended choice and decide from current task context whether to proceed or ask the user.
- `stop_hook_active` must not suppress agent judgement when the recovery response now includes valid next-action choices. It should suppress only missing-choice recovery loops and notify-and-stop relays. Otherwise the agent can present `선택지:` after a block and then silently stop instead of either proceeding with a safe recommendation or explaining why it needs user confirmation.
