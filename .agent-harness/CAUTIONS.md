...
- Consequence: you cannot both stop-and-wait AND show the user a message via raw Stop output — the only visible channel (`decision:"block"`) continues the agent. So to "notify the user that auto-proceed did not engage and let them choose", return `decision:"block"` + a `reason` that instructs the agent to relay the notice to the user and then STOP (no further work, no new choices). The follow-up Stop carries `stop_hook_active=true`, which disables this branch (and auto-proceed), so the relay turn ends with a no-op `{}` and never loops. Implemented in `runHookStop`.
