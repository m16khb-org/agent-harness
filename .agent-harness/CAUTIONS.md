...
- Installed Codex and Claude PreToolUse hooks include `--enforce-gitops-kubectl`; direct mutating `kubectl` commands such as `apply`, `delete`, `patch`, or `rollout restart` must be represented as manifest changes in git and applied through the repo's GitOps path. Live-access commands such as `kubectl exec` and `kubectl port-forward` require explicit user confirmation because they expose live workloads or local ports. Claude uses native `ask`; Codex first blocks with `승인 AH-XXXXXX`, UserPromptSubmit records that exact token, and the next identical session/workspace/cwd/tool/command is allowed once. Pending/granted state expires after 10 minutes, lives under project-scoped user state, and stores only a SHA-256 request fingerprint rather than the raw command. Expired/lost grants require a new token; do not disable the GitOps gate as routine recovery. Read-only commands and dry-runs such as `kubectl get`, `logs`, `diff`, and `apply --dry-run=server` remain allowed.
