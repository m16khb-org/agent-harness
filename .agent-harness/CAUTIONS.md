...
- 관찰 명령을 세미콜론이나 개행으로 묶었다는 이유만으로 `unsafe_mutation` 처리하면 owner가 상태 확인조차 못 한다. 각 조각을 기존 exact read-only parser로 다시 검증하는 시퀀스와 고정된 `.codegraph` 존재/출력 probe만 정적으로 관찰로 인정한다. 파이프·백그라운드·리다이렉트·치환·미분류 명령·임의 분기 본문은 계속 fail-closed로 둔다.
