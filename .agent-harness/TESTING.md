...
Reversible child-host smoke는 일반 live matrix와 별도다. `scripts/verify-child-host-smoke.sh`는 literal `--confirm-user-activation`, clean local HEAD, exact singleton remote ref가 모두 일치할 때만 user-scope integration을 잠시 활성화한다. Codex `exec --json`은 hook lifecycle notification을 JSONL에 투영하지 않으므로, runner가 임시 child hook command에 private marker 경로를 명시적으로 주입하고 실제 child hook process는 `SessionStart`/`PreToolUse` 이름만 기록한다. Host runner는 이 marker와 native MCP result를 boolean/count/SHA-256/exit/duration projection으로 합친 뒤 원문 stream과 marker를 폐기한다. 어떤 post-activation 실패도 source installer 1회, 원래 네 설정 파일의 private byte snapshot 원자 복원, before/restore raw+semantic digest equality를 모두 통과하지 못하면 `verdict=pass`가 될 수 없다.
