...
tmp_state="$(mktemp -d)" && printf '{"cwd":"%s","tool_name":"Bash","tool_response":"draft wiki smoke"}' "$PWD" | HARNESS_STATE_DIR="$tmp_state" ./bin/agent-harness hook post-tool-use --json && rm -rf "$tmp_state"
