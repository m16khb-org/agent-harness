...
IssueOps root writes use schema v5. Missing/zero/v1/v2/v3/v4 rows upgrade while preserving known fields; legacy mailbox migration runs only through v3, so v4 sealed identities remain unchanged. For the current attempt and every prior attempt, a missing live terminal in legacy rows copies the legacy mailbox, then a no-dispatch attempt clears that mailbox without inventing a coordinator recipient. `DispatchID` and `WorkerMailboxHandle` are either both absent or both present in every current envelope. ContextVersion 1 omits an empty `coordinator_recipient` to preserve legacy v3 context bytes and hashes, while a nonempty sealed recipient remains in both context hashes. v1 rejects v2 ownership, v2 rejects v3 stable terminal identity, v3 rejects v4 sealed mailbox/projection authority, and v4 rejects v5 publication/cleanup authority byte-equivalently. Future rows remain fail-closed with only a bounded ownership projection for hooks.
