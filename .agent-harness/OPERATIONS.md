...
For a resolved Orca path, follow `skills/issueops/references/orca-handoff.md`: coordinator `worktree prepare`/`handoff start --coordinator-recipient <sealed-term>`, fresh worker `handoff claim`/`issueops heartbeat`/`handoff finish`, coordinator `handoff accept`. Completed finish automatically projects `worker_done` once from the sealed worker mailbox; a submitted worker must not run the shell send manually. On `recovery_required`, inspect `issueops status --json` and reconcile explicitly; never repeat create or switch to inline after an external mutation may have run. That reference is the single executable source for runtime-rollover inventory, mailbox projection, bounded terminal monitoring, and cleanup commands; Turing links it and records evidence/receipts rather than copying the commands.
...
IssueOps root writes use schema v4. Missing/zero/v1/v2/v3 rows upgrade while preserving known fields; for the current attempt and every prior attempt, a missing live terminal copies the legacy mailbox, then a no-dispatch attempt clears that mailbox without inventing a coordinator recipient. `DispatchID` and `WorkerMailboxHandle` are either both absent or both present in every v4 envelope. ContextVersion 1 omits an empty `coordinator_recipient` to preserve legacy v3 context bytes and hashes, while a nonempty sealed recipient remains in both context hashes. v1 rejects v2 ownership, v2 rejects v3 stable terminal identity, and v3 rejects v4 sealed mailbox/projection authority byte-equivalently. Future rows remain fail-closed with only a bounded ownership projection for hooks.
