...
IssueOps root writes use schema v3. Missing/zero/v1/v2 rows upgrade while preserving known fields; v1 rejects the v2 ownership lease, v2 rejects v3 stable terminal identity, and future rows remain fail-closed with only a bounded ownership projection for hooks.
