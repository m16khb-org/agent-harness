...
target_type: "notes"
summary: "Decouple agent-harness draft-wiki suggestions by enqueuing them in user state and processing them asynchronously via a worker calling agy -p."
suggester: "agy -p"
...
1. **Enqueueing**: The `agent-harness` `PostToolUse` hooks enqueue draft-wiki suggestions into the user state.
...
3. **Draft Output**: The worker writes the final, reviewable Markdown draft to the following directory:
   ```
   .agent-harness/draft-wiki/draft
   ```
