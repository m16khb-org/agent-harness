...
### Research Findings (oh-my-openagent + agent-harness + academic)
...
### Deliverables
- `skills/shannon/SKILL.md` — Shannon 스킬 본체 (~400줄, 기존 스킬과 유사한 구조)
- `skills/shannon/references/` — SNR 측정 프로토콜, 엔트로피 임계값, 중복도 계산 가이드
...
Phase 1: REGRESSION CHECK — 이전 측정값과 비교
  → agent-harness state에서 이전 Shannon snapshot 로드
...
- [ ] 1. Shannon SKILL.md 본체 작성
  ...
  **References**:
  ...
  - 기존 Turing SKILL.md (`skills/turing/SKILL.md`) — 구조 탬플릿 참조
  ...
  **QA Scenarios**:
  ...
  Scenario: SNR calculation command works
    ...
    Evidence: .agent-harness/evidence/shannon/task-1-snr.txt

  Scenario: Entropy heuristic produces complexity flag
    Channel: bash
    Steps: grep 기반 함수당 라인 수, 중첩 depth 추정; 50줄 초과 함수 플래그
    Expected: 초과 함수 리스트 출력
    Evidence: .agent-harness/evidence/shannon/task-1-entropy.txt
  ```

  **Commit**: YES | Message: `feat(shannon): add Shannon signal-to-noise quality gate skill` | Files: `skills/shannon/SKILL.md`

- [ ] 2. 참조 문서 작성 (SNR protocol + Entropy heuristic + Channel capacity rules)

  **What to do**:
  - `skills/shannon/references/snr-protocol.md`: diff 기반 SNR 계산 단계별 가이드
  - `skills/shannon/references/entropy-heuristic.md`: cyclomatic complexity 근사법 (중첩 if/for/while count, params count)
  - `skills/shannon/references/channel-capacity.md`: 파일·함수 크기 상한 규칙 + 측정 명령어
  ...
  **QA Scenarios**:
  ...
  Scenario: SNR protocol commands are executable
    ...
    Evidence: .agent-harness/evidence/shannon/task-2-protocol.txt
  ```

  **Commit**: YES | Message: `docs(shannon): add SNR protocol, entropy heuristic, channel capacity references` | Files: `skills/shannon/references/*.md`

- [ ] 3. Turing Final Quality Gate에 Shannon metric 연동

  **What to do**:
  - `skills/turing/SKILL.md`의 Final Quality Gate 섹션에 Shannon audit 항목 추가
  ...
  **QA Scenarios**:
  ...
  Scenario: Turing Quality Gate includes Shannon metrics
    Channel: bash
    Steps: rg "shannonAudit\|Shannon.*SNR\|entropyScore" skills/turing/SKILL.md
    Expected: 1개 이상 매치
    Evidence: .agent-harness/evidence/shannon/task-3-turing-gate.txt
  ```

  **Commit**: YES | Message: `feat(turing): integrate Shannon metrics into final quality gate` | Files: `skills/turing/SKILL.md`
...
- [ ] 4. IssueOps Shannon phase 연동
  ...
  **Acceptance Criteria**:
  ...
  - [ ] `.agent-harness/SUB_AGENT_PATTERNS.md` 12패턴 유지 (Shannon은 sub-agent 사용 안 함)

  **QA Scenarios**:
  ```
  Scenario: IssueOps ai-slop-clean mentions Shannon
    Channel: bash
    Steps: rg -A3 "ai-slop-clean" skills/issueops/SKILL.md | grep -i shannon
    Expected: Shannon 언급 1개 이상
    Evidence: .agent-harness/evidence/shannon/task-4-issueops.txt
  ```
...
- [ ] 5. Response contracts golden 갱신 + 검증
  ...
  **What to do**:
  - `go test ./cmd/harness/harnessapp -run TestResponseContractsGolden -count=1 -update`
  ...
  **Acceptance Criteria**:
  ...
  - [ ] `./bin/agent-harness docs --json`에 shannon 포함
  ...
  **QA Scenarios**:
  ...
  Scenario: Full test suite passes
    ...
    Evidence: .agent-harness/evidence/shannon/task-5-tests.txt
  ```

  **Commit**: YES | Message: `chore: update golden snapshots for Shannon skill` | Files: `cmd/harness/testdata/*.golden.json`
...
- 기존 agent-harness 구조·컨벤션·테스트를 깨지 않음
