...
v2: brooks `revise` 반영 — 새 판별 함수 대신 기존 `shelltoken.HasUnquotedControlOperator`(원문·따옴표 인식) 재사용, `-c "…;…"` 원장 회귀 확인 명시, `EXPECT` 존재 시 종료코드 무시 잔여 위험 기록.

이슈: https://github.com/m16khb/agent-harness/issues/484
...
| T4 | 문서: `skills/issueops/SKILL.md` 게이트 규칙 한 줄(단일 명령·복합 검사는 스크립트/python -c 래퍼·EXPECT는 줄 앞 토큰), `.agent-harness/CAUTIONS.md` 한 줄 | 문서 | validate-skill, docs checker |
| T5 | 실환경: 임시 원장(체인 CHECK, `docs-ok: error` 출력)으로 `./bin/agent-harness gates check` → unchecked 확인; 저장소 원장 `issues/477`(8 met), `issues/480`(6 met — G3·G4·G6은 `python3 -c "…;…"` 형태라 따옴표 안 `;`가 거부되지 않아야 함) 판정 불변 | 수동 | 게이트 원장 G4 |
