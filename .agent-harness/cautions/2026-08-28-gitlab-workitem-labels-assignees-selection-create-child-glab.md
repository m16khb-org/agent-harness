...
- Evidence:
  - glab api graphql --hostname gitlab.bubbletap.com 인트로스펙션 → WorkItem 필드에 labels/assignees 없음; WorkItemWidgetLabels: allowsScopedLabels, labels, type / WorkItemWidgetAssignees: allowsMultipleAssignees, assignees, canInviteMembers, type; labels→LabelConnection→Label.title, assignees→UserCoreConnection→UserCore.username
  - glab api version --hostname gitlab.bubbletap.com → 19.2.4-ee
