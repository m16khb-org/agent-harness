...
## 2026-06-03 — Headroom remains explicit opt-in companion

- Kind: `adr`
- Source: codex-cli
- Summary: Headroom is treated as an optional upstream companion whose CLI may be installed by the upstream-tools installer, but runtime token optimization is not auto-enabled.
- Context: User requested verification after local checks showed no headroom CLI, no pipx, no Python module, and no running Headroom process.
- Decision: Keep Headroom integration limited to optional CLI install/update via pipx when upstream tools are explicitly synced; do not route Codex/Claude calls through Headroom proxy, wrap, learn, or MCP automatically.
- Consequences: Operators who want Headroom runtime savings must use the explicit reproducible path `scripts/install-native.sh --with-upstream-tools --enable-headroom-runtime` or `scripts/setup-headroom-runtime.sh`. That path installs/updates `headroom-ai[all]` when possible, runs durable Headroom init for both Codex and Claude Code, merges existing `agent-harness` hooks/settings back into host config, starts profile `init-user`, and verifies the local health endpoint. The agent-harness default installer still must not auto-enable proxy, wrap, learn, or MCP routing without the explicit runtime flag.
- Evidence:
  - `command -v headroom` exited 1; `command -v pipx` exited 1; `python3 -c 'import headroom'` failed with ModuleNotFoundError.
  - `ps aux | rg -i '[h]eadroom|HEADROOM'` found only the verification commands, not a persistent proxy or wrapper process.
  - scripts/install-native.sh:176-186 skips Headroom setup when pipx is missing and otherwise only pipx installs/upgrades `headroom-ai[all]`.
  - scripts/install-native.sh:289-345 runs Headroom only from optional upstream setup and dry-run reports install/update only.
  - .agent-harness/OPERATIONS.md:64 and :70 document that proxy/wrap/learn are not automatic and runtime use is manual.
  - internal/adapter/install_contract_matrix_test.go:160-182 asserts the installer includes Headroom install strings and excludes `headroom wrap`, `headroom proxy --port`, and `headroom learn`.
  - `go test ./internal/adapter -run 'TestInstallNativeUpstreamToolsUseHeadroom' -count=1` passed.
- Alternatives / rejected options:
  - Auto-run `headroom proxy` or `headroom wrap` during native install; rejected because it changes host runtime request flow and should require explicit operator intent.
  - Run `headroom learn` during install; rejected because it can modify AGENTS/CLAUDE files and needs separate approval.
