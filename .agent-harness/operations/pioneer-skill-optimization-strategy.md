...
### shannon — firsthand: measured `ae5832a..a859773` (this session's commits)
...
2. **Domain scope gap.** shannon's four metrics (SNR=code noise, entropy=cyclomatic, redundancy=code blocks,
...
### hopper — firsthand: diagnosed the real pre-existing `TestResponseContractsGolden` failure

Outcome (the method works): hopper's 7-step flow drove straight to a confirmed root cause. REPRODUCE → ISOLATE
...
`$HARNESS_ROOT/.agent-harness/` docs index, which includes the **gitignored `evidence/` subtree**. The committed
golden (regenerated in ae5832a) baked in references to local-only evidence files present at that moment
(`reruns/berners-lee/fixture-request.md`, `codd/analysis.md`, …); any other working tree indexes different
...
1. **No snapshot/golden-test isolation pattern.** Step 3 offers Bisect / Divide&Conquer / Trace-Diff, but the
   ...
   `-update` into a scratch copy, `git diff`, then restore." hopper doesn't mention this; I had to know it.
2. **Step 2 lint_diagnose assumes the daemon/MCP is up.** The `agent_harness` MCP disconnected mid-session, so
...
### torvalds — firsthand: dated the golden pollution via history archaeology

Outcome (the method works): used torvalds' History Analysis toolset directly.
`git log -S'.agent-harness/evidence/pioneer-skills-quality' -- <golden>` pinpointed the polluting commit instantly
= **ae5832a** (whose own message says "Regenerate the response contract golden after the docs index changed").
Confirmed the current committed golden carries **120 lines** of gitignored `agent-harness/evidence` paths, and
`git check-ignore -v` confirmed `.gitignore:16:evidence`. This independently corroborates and dates the hopper
root cause.
...
Friction observed firsthand:
1. **No read-only "investigation/archaeology" entry point.** torvalds is organized around mutating ops (rebase,
...
Dogfooding hopper + torvalds surfaced a real harness bug: `TestResponseContractsGolden` is non-hermetic. The
response-contract snapshot's docs index scans the live `$HARNESS_ROOT/.agent-harness/` tree **including the
...
### dijkstra — firsthand: decided no-change on the bounded startup loop
...
1. **The "should I optimize at all?" gate is buried at the very bottom (lines ~498/516).** A reader must load ~500
   ...
   late. Same friction class as torvalds: the key decision rule is buried.
...
### codd — firsthand: applied to the events-table task; confirmed a structural verification gap

Outcome: invoked codd and applied Step 4. The multi-index CRITICAL rule I added earlier is present and well-placed
...
Friction observed firsthand:
1. **Verification gold standard is structurally unreachable in the common agent context.** codd's NEVER rule
   ("Recommend DDL without the full before/after EXPLAIN ANALYZE evidence") and all of Step 7 assume a live DB
   connection. In agent-harness — and most agent sessions — there is **no DB**, so every codd recommendation can
   ...
   permanent tension every codd run hits.
2. **Applicability gate missing.** codd is 1000+ lines (longest skill) yet this repo has no RDBMS at all — codd is
...
- **EDIT (medium)**: add an explicit "Advisory mode (no live DB)" note at Step 1/Step 7 — when no DB connection is
  available, codd outputs the recommendation + the exact EXPLAIN/row-count commands for the operator + marks it
...
- **EDIT (low)**: add an early applicability gate ("no relational DB / pure file-state repo → codd does not apply").

### karpathy — firsthand: critiqued my own holdout sub-agent prompt; it caught a real bug
...
conflicting location signals, so the sub-agent measured the wrong repo. karpathy's "ambiguous → single source of
...
Friction observed firsthand:
1. **No lightweight / one-shot mode.** karpathy mandates a 5-case TEST SUITE, A/B testing, and versioned storage
   under `.agent-harness/karpathy/prompts/` ("NEVER publish a prompt without a test suite"). That is right for a
...
### turing — firsthand: applied to a 2-file validate + build; confirmed the proportionality gap
...
1. **Ceremony does not scale down.** Applying turing's *full* method to a 2-file validate still demands
...
### von-neumann — firsthand: correctly routed the golden fix to direct execution (no over-planning)

Outcome (activation discipline works): asked von-neumann to decide the altitude for the golden-bug fix. Phase 0
...
Strengths (HOLD): unlike dijkstra/torvalds/codd, von-neumann puts its decision gate at the TOP (identity para +
...
1. **No structured output for the no-plan decision.** All the machinery (Phase 1 Ground → Phase 2 Interview →
   clearance checks → draft → plan template) assumes you ARE planning. When von-neumann correctly decides NOT to
...
### berners-lee — firsthand: researched Go golden-test hermeticity (and it confirmed the fix direction)
...
fix: the harness normalizes *paths* ($HARNESS_ROOT, $STATE_DIR) but not the *set of files indexed*, so regenerating
...
1. **Quick-lookup class vs. absolute Critical Rules.** Phase 0 has a "Quick lookup → don't over-engineer" intent
   class, but the Critical Rules still say ALWAYS write the report to `.agent-harness/research/<slug>.md` and NEVER
   single-source. For a tight 2-fetch lookup like this one, writing a full report file is disproportionate. The
   skill names the lightweight case but its absolute rules don't honor it. (Better than karpathy/turing, which lack
...
- **EDIT (low)**: reconcile the Quick-lookup class with the Critical Rules — explicitly allow quick lookups to use
  inline citations and skip the `.agent-harness/research/` report file, so Phase 0 and the ALWAYS/NEVER rules agree.
...
1. **Buried decision gate** (dijkstra, codd, torvalds; shannon's non-code case). The "should I even act / which mode
   / does this apply?" gate sits at the *bottom* of long skills, so a reader internalizes the heavy machinery before
   reaching the rule that says "don't." **von-neumann is the positive model** — its activation boundary is at the
   ...
   variance (dijkstra/torvalds optimized/acted because the gate came too late).

2. **No proportionality / lightweight mode** (karpathy, turing, berners-lee; codd's advisory case). Skills calibrated
   for high-stakes reuse over-ceremony simple/low-risk tasks (test suites, reviewer gates, report files, EXPLAIN).
   **Strategy: add an explicit lightweight mode to each** — one-shot prompt (karpathy), low-risk proportionate
   evidence (turing), quick lookup (berners-lee), advisory-no-DB (codd).

3. **shannon portability defect (unique, highest individual value)**: the skill's own measurement commands silently
   mis-measure on a host where `grep` is aliased to `ugrep`/`rg`. Real, reproduced twice. **Strategy: ugrep-safe
   commands + alias warning.**

4. **Harness defect (not a skill issue)**: the non-hermetic golden, diagnosed via hopper + torvalds and confirmed by
   berners-lee research. **Strategy (Go, separate): exclude the gitignored `evidence/` subtree from the docs index;
   regenerate the golden.** von-neumann routed this to direct execution (not a plan).

### Recommended edit priority (all are skill-body/doc edits, reversible)

| Priority | Skill | Edit | Why |
|----------|-------|------|-----|
| 1 | shannon | ugrep-safe commands + alias note | Skill's own commands fail on this host (reproduced) |
| 2 | dijkstra | hoist "optimize-at-all?" gate to top | Attacks measured executor variance |
| 3 | turing | proportionate low-risk mode | Confirmed TURING-H1 gap firsthand |
| 4 | hopper | add Snapshot/Golden isolation strategy | Most common Go failure class, missing |
| 5 | codd | advisory-mode (no live DB) note | Resolves structural EXPLAIN tension |
| 6 | karpathy | one-shot/orchestration prompt mode | Over-heavy for inline prompts |
| 7 | berners-lee | reconcile quick-lookup vs Critical Rules | Minor proportionality |
| 8 | von-neumann | routing-record for no-plan case | Closes VON-NEUMANN-H1 0.2 gap |
| 9 | torvalds | read-only archaeology pointer | Low; mostly HOLD (variance-confirmed correct) |
