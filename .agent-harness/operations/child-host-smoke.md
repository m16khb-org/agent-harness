...
The runner builds and dry-runs the child binary, records bounded digests plus a private byte-for-byte snapshot of the currently installed source surfaces, activates the child checkout, verifies unchanged host versions, and runs one `empty_object` MCP episode per host. It instruments only the temporary child `SessionStart`/`PreToolUse` commands with the private marker path, so observation does not depend on either host inheriting arbitrary parent environment variables. The child hook processes append only those two event names because Codex does not project hook lifecycle notifications into `exec --json`; the host probes combine the markers with native MCP results. The receipt exposes only hook booleans, MCP call count, response SHA-256, exit code, and duration, and never persists native event transcripts.
