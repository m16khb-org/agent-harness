...
Scoring rubric: `.agent-harness/operations/pioneer-skill-quality-rubric.md`.
...
Request: Use `$berners-lee` to research whether a library supports a specific capability, verify against official docs and source/package metadata, and return cited findings.
...
Request: Use `$berners-lee` to fetch a login-only, paywalled, CAPTCHA, or bot-protected source.
...
Request: Use `$berners-lee` inside an IssueOps-style workflow and write a reusable research report.
Expected: Writes report only under `.agent-harness/research/`, records IssueOps feedback only when a valid IssueOps cycle exists.
...
Request: Use `$codd` to optimize a slow query with row counts, plan evidence, index choice, write-penalty note, and before/after proof.
...
Request: Use `$codd` to denormalize a schema because "we might need it later."
...
Request: Use `$codd` to recommend pool size, isolation level, or live DDL safety.
...
Request: Use `$dijkstra` to improve an algorithm's complexity.
...
Request: Use `$dijkstra` to replace simple startup O(n) code with a complex data structure.
...
Request: Use `$dijkstra` to classify empirical complexity from scaled inputs.
...
Request: Use `$hopper` to debug a deterministic failing command.
...
Request: Use `$hopper` on a missing package path, syntax error, or exact-line missing import.
...
Expected: Uses `agent-harness project lint-diagnose --json -- <command...>` or labels MCP `lint_diagnose(command_argv)` separately.
...
Request: Use `$karpathy` to turn an ambiguous prompt into a testable prompt with acceptance criteria.
...
Request: Use `$karpathy` to make a prompt "show all chain-of-thought."
...
Request: Use `$karpathy` to design a tool-calling prompt for the current host.
...
Request: Use `$shannon` to measure the quality of current uncommitted work.
...
Request: Use `$shannon` when there are zero changed lines.
...
Request: Use `$shannon` to run pre-PR quality checks.
...
Request: Use `$torvalds` before a rebase, bisect, cherry-pick, or recovery.
...
Request: Use `$torvalds` to recover from a bad rewrite with `reset --hard`.
...
Request: Use `$torvalds` for worktree management or advanced git handoff to atomic commit flow.
...
Request: Use `$turing` to complete a goal with measurable criteria and evidence.
...
Request: Use `$turing` for a small documentation-only task.
...
Request: Use `$turing` with state checkpoints or IssueOps feedback.
...
Request: Use `$von-neumann` to plan an ambiguous multi-step implementation.
...
Expected: Uses existing IssueOps `link-plan`; does not reference nonexistent `agent-harness von-neumann plan`.
