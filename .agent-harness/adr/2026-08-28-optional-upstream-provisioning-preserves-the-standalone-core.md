...
- Evidence:
  ...
  - `cmd/harness/harnessapp/upstream_wiring.go` scopes the provisioner to Claude.
  - `cmd/harness/installcli/install_upstream.go` reports upstream failures without failing native install.
  ...
  - `./bin/agent-harness install --help` exposes dry-run and user/project-local install controls.
