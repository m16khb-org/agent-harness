...
- 소유 범위: `skills/von-neumann/SKILL.md`, `skills/berners-lee/SKILL.md`, `skills/karpathy/SKILL.md`, 이 보고서
...
- 확인된 계약: `issueops status`는 `cmd/harness/issueopscli/issueops_subcommands.go:30`에서 `--id`를 받고, 빈 값은 `internal/core/issueops/issueops_state.go:261`에서 `id is required`로 거부한다. `link-plan` 명령은 `cmd/harness/testdata/usage.golden.txt:40`과 일치하고 `.agent-harness/plans/` 경로도 repo에 존재한다.
- 결함: IssueOps 감지 예시가 `agent-harness issueops status --json`으로 되어 있어 실제로는 실패한다.
- 수정: `agent-harness issueops status --id "$ISSUEOPS_ID" --json`으로 좁혔다. activation boundary, routing record, plan/draft lifecycle은 현재 품질 계약과 일치해 변경하지 않았다.

### berners-lee

- 확인된 계약: `web_fetch_resilient`는 `internal/adapter/mcp/local_assistant_catalog.go:29`, CLI fallback은 `cmd/harness/testdata/usage.golden.txt:87`, `issueops feedback add`는 같은 golden의 46행에 존재한다. `skills/berners-lee/references/report-template.md`도 존재한다.
- 결함 1: access-control 신호 뒤 TLS/browser impersonation, cookie warming, browser escalation을 권하고 `archive.today` 제출까지 포함했다. 이는 skill 자체의 read-only/access-control 금지와 `.agent-harness/operations/pioneer-skill-quality-cases.md:59-63`의 bot boundary 계약에 어긋난다.
...
- 확인된 계약: privacy/tool-truth guardrail, 실제 host tool 확인 규칙, `issueops feedback add` 예시는 현재 golden과 일치한다. `.agent-harness/karpathy/prompts/`도 존재한다.
...
Evidence artifact: 이 보고서, owned diff, `cmd/harness/testdata/usage.golden.txt`, `internal/adapter/mcp/local_assistant_catalog.go`, `cmd/harness/issueopscli/issueops_subcommands.go`, `internal/core/issueops/issueops_state.go`, `.agent-harness/operations/pioneer-skill-quality-cases.md`.
...
| `python3 scripts/validate-skill.py skills/berners-lee` | exit 0, `Skill is valid!` |
| `python3 scripts/validate-skill.py skills/karpathy` | exit 0, `Skill is valid!` |
| `python3 scripts/validate-skill.py skills/von-neumann` | exit 0, `Skill is valid!` |
