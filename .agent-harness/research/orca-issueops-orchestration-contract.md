...
- Current harness parser: `cmd/harness/hookcli/hookinput/hook_input.go:8-28`.
...
The resulting V1 rule is: invoke each create mutation once, never retry an ambiguous response automatically, reconcile only one exact ownership-epoch marker (or one terminal PTY delta), and leave zero/multiple candidates fail-closed. Full evidence and cleanup receipts are in `.agent-harness/research/orca-live-handoff-spike-2026-07-11.md`.
