...
- Produces: strict semantic rejection of any installed agent-harness hook group whose executable differs from the stable expected runtime, including `.worktrees` paths.

- [x] **Step 1: Add explicit failing stale-worktree readback cases**

  After canonical install, replace only one host hook command with `/source.worktrees/completed/bin/agent-harness hook ...`. Assert `VerifyActivation` fails and names the host hook readback surface. Keep formatting-only and third-party co-resident hook cases accepted.
...
- Modify: `cmd/harness/hookcli/dependencies.go`
- Modify: `cmd/harness/hookcli/hook_pre_tool_use.go`
- Modify: `cmd/harness/hookcli/hook_pre_tool_use_test.go`
- Modify: `cmd/harness/hookcli/hookcatalog/catalog.go`
- Modify: `cmd/harness/hookcli/hookcatalog/catalog_test.go`
- Modify: `cmd/harness/harnessapp/host_facade.go`
...
- [x] **Step 3: Run the focused RED tests**
  ...
  Run: `go test ./internal/core/install ./cmd/harness/hookcli ./cmd/harness/hookcli/hookcatalog -run 'NativeRuntime|CachedWorktreeRuntime' -count=1`
...
- [x] **Step 4: Implement one host-neutral diagnosis and thin projections**
  ...
  Resolve symlinks on the observed executable, derive its checkout root only for the installer-owned `bin/agent-harness` layout, resolve that root through Task 1, and compare against the canonical source `bin/agent-harness`. Return a typed diagnostic; do not read or rewrite host config here. Inject the executable provider and diagnostic into hook CLI dependencies. PreToolUse emits the existing host-native block response and SessionStart emits host-native context; both exit normally so a cached executable produces actionable evidence instead of a missing status.

- [x] **Step 5: Run focused GREEN and hook contract regression tests**

  Run: `go test ./internal/core/install ./cmd/harness/hookcli ./cmd/harness/hookcli/hookcatalog ./cmd/harness/harnessapp -count=1`
...
- [x] **Step 5: Run focused GREEN**
  ...
  Run: `go test ./internal/adapter ./cmd/harness/installcli -count=1`
...
- [x] **Step 1: Run formatting and focused verification**
  ...
  Run: `go test ./internal/core/install ./internal/adapter/installutil ./internal/adapter/codex ./internal/adapter/claude ./internal/adapter ./cmd/harness/hookcli ./cmd/harness/hookcli/hookcatalog ./cmd/harness/installcli -count=1`
...
- [x] **Step 2: Run full verification sequentially**
  ...
  Run: `go build -o /tmp/agent-harness-282 ./cmd/harness`

  Run: `go test ./cmd/harness/contractgolden -run Golden -count=1`

  Run: `go test ./cmd/harness/harnessapp -run TestResponseContractsGolden -count=1`
...
- [ ] **Step 4: Publish and merge through IssueOps**
  ...
  Commit with the repository Conventional Commit + Lore policy, create a Korean GitHub PR linked to #282, wait for both push and PR CI runs, record completion with the final head and Turing report, merge only after all checks pass, and fast-forward source `main`.
