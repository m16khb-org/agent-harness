...
`cmd/harness/hookcli/hookinput/hook_input.go:43` `EffectiveCWDFromHookInput`은 shell tool일 때
...
- upstream `openai/codex` 저장소 수정 (#278) — agent-harness 범위 밖. 추적만 유지한다.
- IssueOps 가드의 일반 shell admission 확대. 모든 허용은 좁은 exact grammar로만 한다.
- Orca relay 자체 수정. agent-harness 쪽 계약/복구 경로만 다룬다.
- OpenWiki 자동 갱신.
...
`cleanup_candidate` lifecycle 3건, active lease 2건, 미커밋 turing 상태.
