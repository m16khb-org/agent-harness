...
이슈: https://github.com/m16khb/agent-harness/issues/100

## 재평가 결론 (brooks devil's-advocate revise 반영)

#95의 재평가 조건 충족. 근거 체인(brooks 지적으로 PreToolUse 직접 근거 보강):
...
- top-level 문자열 추출 의존은 합성 fixture 1건(`hook_input_test.go` top-level `note`) — brooks 전수 조사로 깨지는 테스트가 이 1개뿐임을 확인.

전환한다.

## 변경

1. `hook_paths.go` walk의 string 케이스(`*** Begin Patch` 스캔 + `.go`/`.agent-harness`/`testdata/` 내용 heuristic)를 **`insideToolInput` 또는 부모 키가 `command`/`cmd`일 때만** 수행하도록 게이트.
...
2. 키 기반 추출(path·`_path`·file·filename)·filesystem alias·tool_response/tool_result/transcript 계열 subtree 제외는 불변. 갱신되는 fixture에 **키 기반 추출 보존이 의도**라는 주석을 남긴다(다음 재평가 반복 방지, brooks 지적).
