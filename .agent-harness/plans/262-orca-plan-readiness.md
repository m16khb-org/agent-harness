...
- Modify: `cmd/harness/harnessapp/issueops_preparation_wiring.go`
- Create: `internal/core/issueops/issueops_artifact_stage_test.go`
- Test: `internal/core/issueops/execution_owner_packet_test.go`
- Test: `cmd/harness/harnessapp/issueops_preparation_wiring_test.go`
...
```bash
go test ./internal/core/issueops ./cmd/harness/harnessapp -run 'PlanArtifact|Preparation.*Plan' -count=1
```
...
- Test: `cmd/harness/harnessapp/issueops_resume_wiring_test.go`
...
```bash
go test ./internal/core/issueops ./cmd/harness/harnessapp -run 'Resume.*Plan' -count=1
```
...
- Modify: `.agent-harness/TESTING.md` if the plan-ready regression command is not already covered
- Modify: `cmd/harness/testdata/response_contracts.golden.json` only if the structured error changes the frozen surface
- Create: `.agent-harness/issueops/262-turing-report.md`
...
```bash
go test ./internal/core/issueops ./internal/application/issueopspreparation ./internal/adapter/inbound/issueopspreparation ./internal/adapter/outbound/issueopspreparation ./cmd/harness/harnessapp ./cmd/harness/issueopscli ./cmd/harness/mcpcli -count=1
