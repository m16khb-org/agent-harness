...
### 배경 (갭 분석)
- 스킬 계층(`skills/issueops/SKILL.md:246-283`)에는 phase별 pioneer 스킬 매핑이 강하게 존재하고 brooks는 fail-closed 게이트로 기계 강제됨.
- hook 계층(`internal/core/hookprompt/rules.go`)은 **키워드 기반** secondary hint만 있음 — 사용자가 plan phase에서 계획 키워드 없이 말하면 von-neumann 힌트가 뜨지 않는다.
...
- bound cycle(phase=plan)에서 `printf '{"prompt":"이거 진행해줘","repo":"<repo>"}' | ./bin/agent-harness hook user-prompt` 출력에 `issueops-phase` 힌트 라인과 von-neumann이 포함
- unbound repo/비 IssueOps repo에서는 힌트 없음(기존 출력 불변)
- `go test ./internal/core/hookprompt ./internal/core/issueops ./cmd/harness/hookcli -count=1` 통과
- `go test ./cmd/harness/contractgolden ./cmd/harness/harnessapp -run Golden -count=1` 통과(force 시 갱신 검수)
...
> 구현+테스트 동일 task. hookcli 스모크는 실제 바이너리 파이프 실행. Evidence: `.agent-harness/evidence/phase-hint-*.txt`
...
- [ ] 1. canonical phase→기대 스킬 테이블 신설
  ...
  **What to do**: `internal/core/issueops/routing_expectations.go` 신설 — `func ExpectedSkillRoutings() []SkillRouting`과 `func ExpectedSkillsForPhase(phase string) []string`. 내용은 SKILL.md 273-283 표 그대로: problem/grill/issue/plan→von-neumann(+grill: berners-lee, issue/plan: codd, plan: karpathy·dijkstra·brooks), compatibility-review→brooks, implement→turing·dijkstra·codd·hopper·torvalds, ai-slop-clean→shannon·turing·karpathy·dijkstra, feedback→turing·hopper·berners-lee, pr→turing·karpathy·torvalds, cleanup→turing·torvalds. karpathy는 cross-cutting이므로 plan/ai-slop-clean/pr에만 명시(SKILL.md 표 기준). 테스트: (a) 모든 phase 키가 유효 phase enum인지, (b) `skills/issueops/SKILL.md`의 요약 표(273-283행)를 파싱해 스킬 집합 일치 고정(드리프트 가드).
  ...
  **QA**:
  ...
  Scenario: 테이블-문서 일치 (happy)
    ...
    Evidence: .agent-harness/evidence/phase-hint-table.txt
...
- [ ] 2. hookprompt phase-인지 secondary 힌트 주입
  ...
  **QA**:
  ...
  Scenario: plan phase 힌트 주입 (happy)
    Channel: bash — temp HARNESS_STATE_DIR에 cycle+binding 시드 후 printf '{"prompt":"진행해줘","repo":"<repo>"}' | ./bin/agent-harness hook user-prompt
    Expected: additional_context에 "issueops-phase"와 "von-neumann" 문자열 포함
    Evidence: .agent-harness/evidence/phase-hint-inject.txt
  Scenario: unbound repo 불변 (failure/경계)
    Channel: bash — binding 없는 repo로 동일 실행
    Expected: 출력에 "issueops-phase" 부재, 기존 힌트 구조 불변
    Evidence: .agent-harness/evidence/phase-hint-absent.txt
  ```
...
- [ ] 3. routing score의 기대값 기본 채움
  ...
  **What to do**: `cmd/harness/issueopscli/issueops_subcommands.go:237-248` — `--expect` 생략 시 `parseExpectedRouting` 대신 T1의 `ExpectedSkillRoutings()`를 기본값으로 사용. usage 문구에 "(기본: canonical 테이블)" 반영. 테스트: --expect 없이 실행 시 canonical 기대로 채점되는지.

  **Recommended Agent**: quick
  **References**: `cmd/harness/issueopscli/issueops_subcommands.go:237-256`
  **Acceptance Criteria**: `go test ./cmd/harness/issueopscli -count=1` 통과
  **QA**:
  ```
  Scenario: 기본 기대 채점 (happy)
    Channel: bash — temp state에 routing 기록 후 ./bin/agent-harness issueops routing score --id <ID> --json (--expect 없음)
    Expected: missing 목록이 canonical 테이블 기준으로 계산됨
    Evidence: .agent-harness/evidence/phase-hint-score-default.txt
  Scenario: --expect 명시 우선 (경계)
    Channel: bash — --expect 지정 실행
    Expected: 명시값이 기본값을 대체
    Evidence: 동일 파일
  ```
  **Commit**: YES | `feat(issueops): default routing score expectations to canonical table` | Files: `cmd/harness/issueopscli/issueops_subcommands.go`, 테스트

- [ ] 4. golden/문서/전체 검증

  **What to do**: (1) usage 문구 변경이 있으면 `go test ./cmd/harness/contractgolden ./cmd/harness/harnessapp -run Golden -update -count=1` 후 diff 추가분만 검수. (2) `.agent-harness/ARCHITECTURE.md` hook 절과 `skills/issueops/SKILL.md`의 hook 설명(24행 부근)에 phase-인지 힌트 1-2줄 반영, ADR에 결정 기록(관측 전용/게이트 기각/단일 테이블 근거). (3) TESTING.md §2 기본 검증 + hookcli 스모크. **주의**: 테스트 실행 전 CAUTIONS의 파이프 KVA 항목 확인 — stdout-capture 테스트가 512B 강등 상태에서 행할 수 있으니 실행 전 `.test` 고아/파이프 압력을 점검한다.

  **Recommended Agent**: quick
  **References**: `.agent-harness/TESTING.md:105-110`, `.agent-harness/CAUTIONS.md`의 2026-07-09 파이프 KVA 항목
  **Acceptance Criteria**: 전체 `go test ./... -count=1`(파이프 압력 정상 상태에서) + golden 통과 + `./bin/agent-harness docs --json` 정상
  **QA**:
  ```
  Scenario: E2E 스모크 (happy)
    Channel: bash — Definition of Done의 hook user-prompt 파이프 실행 2종
    Expected: bound→힌트 있음 / unbound→없음
    Evidence: .agent-harness/evidence/phase-hint-e2e.txt
  Scenario: golden additive (failure 감시)
    Channel: bash — git diff cmd/harness/testdata | rg '^-' | rg -v '^---'
    Expected: 삭제 라인 0건
    Evidence: .agent-harness/evidence/phase-hint-golden.txt
  ```
