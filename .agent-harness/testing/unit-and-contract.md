...
- `agent-harness state write/read/list/prune/doctor/maintain` output shape
...
Golden file은 읽기 쉽게 작게 유지하고, schema를 변경하면 의도와 migration을 문서화한다. 실제 CLI/MCP JSON response golden은 timestamp, temp path, audit id, git sha, home/harness path를 `$TIMESTAMP`, `$STATE_DIR`, `$WORKSPACE`, `$GIT_REPO`, `$GIT_SHA`, `$HOME`, `$HARNESS_ROOT`, `$AUDIT_ID` 같은 placeholder로 정규화한 뒤 비교한다.

## Contract/audit/worker verification

CLI/MCP DTO를 변경하면 `agent-harness contract check --json`과 golden test를 실행해 command name, MCP tool name, required response field가 기계가 읽을 수 있는 형태로 유지되는지 확인한다. policy audit 동작을 변경하면 JSONL record가 append-only인지, secret-like argument가 redacted되는지 검증한다. generic worker를 변경할 때는 no-shell MVP 범위인 enqueue/status/list/cancel을 테스트한다.
