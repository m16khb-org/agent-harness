...
const issueOpsUsageCatalog = `  agent-harness issueops start --repo PATH [--branch NAME] [--json]
  ...
  agent-harness issueops branch prepare --id ID --provider github|gitlab --issue-url URL --branch NAME --base-branch REF [--base-sha SHA] [--parent-worktree PATH] [--remote-branch-url URL] [--link-verified] RECORD_ACTOR_FLAGS [--json]
