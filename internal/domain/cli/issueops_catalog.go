...
const issueOpsUsageCatalog = `  agent-harness issueops start --repo PATH [--branch NAME] [--json]
  ...
  agent-harness issueops branch prepare --id ID --provider github|gitlab --issue-url URL --branch NAME --base-branch REF [--base-sha SHA] [--parent-worktree PATH] [--remote-branch-url URL] [--code-project-key HOST/PROJECT] [--link-verified] RECORD_ACTOR_FLAGS [--json]
  agent-harness issueops branch await-link --id ID [--timeout DURATION] [--json]
  agent-harness issueops branch retarget --id ID --base-branch REF --reason TEXT RECORD_ACTOR_FLAGS [--json]
  ...
  agent-harness issueops devils-advocate review --id ID --verdict pass|revise|stop --reviewer-context subagent|inline [--finding TEXT]... [--waive --waiver-rationale TEXT] RECORD_ACTOR_FLAGS [--json]
  ...
  agent-harness issueops execution prepare --id ID --mode auto|direct|orca --owner-host codex|claude|omo [--owner-model MODEL] [--owner-effort EFFORT] [--direct-reason REASON] [--expected-readiness-fingerprint SHA256] [--issue-snapshot-file PATH] ACTOR_FLAGS [--confirm] [--json]
  agent-harness issueops execution status --id ID [--json]
  agent-harness issueops execution whoami [--json]
  agent-harness issueops execution claim --id ID --generation N (--claim-current-token|--claim-token-file PATH) [--issue-body-sha256 SHA256 --context-packet-sha256 SHA256] [--issue-snapshot-file PATH] [ACTOR_FLAGS] [--json]
  ...
  agent-harness issueops cleanup abandon --id ID --reason TEXT [--close-pr] [--close-issue] [--delete-remote-branch] (--preview | --apply --confirm --fingerprint SHA256) [--json]
