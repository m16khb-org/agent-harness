...
func validateFetchURL(ctx context.Context, raw string, allowPrivate bool, resolver webfetchport.Resolver) (*url.URL, error) {
	...
	if net.ParseIP(strings.Trim(host, "[]")) == nil {
		...
		for _, addr := range addrs {
			if isUnsafeHost(addr, allowPrivate) {
				return nil, fmt.Errorf("unsafe resolved host %q -> %s", host, addr)
			}
		}
	}
	return u, nil
...
func (policy urlPolicy) Validate(ctx context.Context, raw string, allowPrivate bool) (string, error) {
	parsed, err := validateFetchURL(ctx, raw, allowPrivate, policy.resolver)
	if err != nil {
		return "", err
	}
