...
func TestActiveLeaseReadOnlyReaderAdmission(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"full self-verify", "./bin/agent-harness self-verify --full --iterations=10 --seed=100 --target-score=95 --llm-eval=false --progress=jsonl --json", "#439"},
...
func TestActiveLeaseReadOnlyReaderDenyMatrix(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"self-verify unknown write", "./bin/agent-harness self-verify --full --write --json"},
