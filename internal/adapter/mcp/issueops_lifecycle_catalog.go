	...
	return []Tool{
		...
		},
		{
			...
			InputSchema: map[string]any{"type": "object", "required": []string{"id", "provider", "kind", "url", "labels", "assignees"}, "properties": map[string]any{
				...
				"target_branch": map[string]any{"type": "string", "description": "Verified PR/MR target branch; must match the IssueOps linked base branch for strict PR readiness."},
		...
		},
		{
			...
			InputSchema: map[string]any{"type": "object", "required": []string{"id", "title"}, "properties": map[string]any{
				...
				"host":          map[string]any{"type": "string", "description": "Native owner host for ownership-transfer cycles."},
				"session_id":    map[string]any{"type": "string", "description": "Native owner session for ownership-transfer cycles."},
				"agent_id":      map[string]any{"type": "string", "description": "Optional native owner agent identity."},
				"cwd":           map[string]any{"type": "string", "description": "Canonical owner worker root for ownership-transfer cycles."},
		...
		},
		{
			...
			Description: "Create a remote pull request / merge request for the IssueOps cycle. Supervised execution requires the full explicit provider, title, body, head, base, labels, assignees, and confirm payload.",
			InputSchema: map[string]any{"type": "object", "required": []string{"id", "provider", "title", "body", "head", "base", "labels", "assignees", "confirm"}, "properties": map[string]any{
				"id":            map[string]any{"type": "string", "description": "IssueOps id."},
				"title":         map[string]any{"type": "string", "description": "PR/MR title."},
				"body":          map[string]any{"type": "string", "description": "PR/MR body (markdown)."},
				"head":          map[string]any{"type": "string", "description": "Explicit source branch."},
				"base":          map[string]any{"type": "string", "description": "Explicit target branch."},
				"provider":      map[string]any{"type": "string", "description": "Explicit remote provider.", "enum": []string{"github", "gitlab"}},
		...
		{
			Name:        "issueops_remote_reconcile_create",
			Description: "Coordinator-only reconciliation for a durable remote_create_claim. Exactly one live verified candidate is adopted; zero requires explicit approval plus authoritative proof; ambiguity retains unknown.",
			InputSchema: map[string]any{"type": "object", "required": []string{"id", "claim_id", "coordinator_recipient", "host", "session_id", "source_cwd", "confirm"}, "properties": map[string]any{
				"id":                    map[string]any{"type": "string"},
				"claim_id":              map[string]any{"type": "string"},
				"coordinator_recipient": map[string]any{"type": "string"},
				"host":                  map[string]any{"type": "string"},
				"session_id":            map[string]any{"type": "string"},
				"agent_id":              map[string]any{"type": "string"},
				"source_cwd":            map[string]any{"type": "string"},
				"approve_zero_clear":    map[string]any{"type": "boolean"},
		...
		},
		{
			...
			InputSchema: map[string]any{"type": "object", "required": []string{"action", "id"}, "properties": map[string]any{
				...
				"source_cwd":                    map[string]any{"type": "string"},
				"workspace_epoch":               map[string]any{"type": "string", "description": "Exact ready-workspace epoch required for ownership dispatch."},
				"confirm":                       map[string]any{"type": "boolean"},
				...
				"worker_scope":                  map[string]any{"type": "string"},
				"verification_commands":         map[string]any{"type": "array", "items": map[string]any{"type": "string"}},
				"heartbeat_cadence":             map[string]any{"type": "string"},
				"stop_conditions":               map[string]any{"type": "array", "items": map[string]any{"type": "string"}},
				"result_format":                 map[string]any{"type": "string"},
				"allow_codex_hook_trust_bypass": map[string]any{"type": "boolean", "description": "Explicit attestation that nonsecret Codex hook evidence was reviewed before supervised startup."},
				...
				"recovery_action":               map[string]any{"type": "string", "enum": []string{"reconcile", "abandon", "cancel", "finalize-cancel"}},
