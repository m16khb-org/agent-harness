...
func Write(root, draftWikiDir, title, targetWiki, targetType, agyModel, output string) (string, error) {
	body := strings.TrimSpace(StripAgyOutputPreamble(StripMarkdownFence(output)))
	if body == "" {
		return "", fmt.Errorf("agy output was empty")
	}
	if !strings.HasPrefix(body, "---\n") {
		body = Frontmatter(title, targetWiki, targetType, agyModel) + "\n" + body + "\n"
	}
...
func Frontmatter(title, targetWiki, targetType, agyModel string) string {
	if title == "" {
