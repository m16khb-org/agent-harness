...
func Write(root, draftWikiDir, title, targetWiki, targetType, model, output string) (string, error) {
	body := strings.TrimSpace(StripLLMOutputPreamble(StripMarkdownFence(output)))
	if body == "" {
		return "", fmt.Errorf("LLM output was empty")
	}
	if !strings.HasPrefix(body, "---\n") {
		body = Frontmatter(title, targetWiki, targetType, model) + "\n" + body + "\n"
	}
...
func Frontmatter(title, targetWiki, targetType, model string) string {
	if title == "" {
...
target_wiki: %q
target_type: %q
summary: "Draft generated by Z.AI Coding Plan for human review."
suggester: "zai:%s"
model: %q
