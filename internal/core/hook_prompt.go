...
const nextActionPolicyHint = "next-action: end only user-decision turns with a '선택지:' block — exactly 3 numbered options and exactly one '(추천)'. Mark '(추천)' only when the main agent judges that option safe, reversible, and aligned with user intent from current context. The Stop hook may re-enter the main agent to re-check that recommendation; proceed only if that judgement still holds, otherwise ask for user confirmation. The main agent must state its auto-proceed or no-auto-proceed rationale when re-entered from Stop hook choices. Stop-hook auto-proceed result reports must still end with choices. Do not mark destructive, irreversible, or uncertain actions as '(추천)'."
