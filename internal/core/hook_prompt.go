...
const nextActionPolicyHint = "next-action: end any turn that needs a user decision with a '선택지:' block — exactly 3 numbered options and exactly one '(추천)'. The Stop hook only parses choices inside that section and, when the recommended option has a forward/verify verb (진행·계속·구현·추가·작성·검증·테스트·빌드·린트·확인·점검·실행·수정·반영·적용 / proceed·continue·implement·add·write·verify·test·build·check·inspect·run·apply·fix·update), no destructive/irreversible word (삭제·제거·되돌리·초기화·병합·배포·릴리즈·게시·전송·결제·환불·강제·푸시·운영 / delete·remove·drop·truncate·reset·revert·rollback·merge·rebase·push·deploy·release·publish·send·payment·prod), and no hedge (아마·확실치·미확인·maybe·might·unsure), it re-enters the main agent to re-check that same recommendation. Main agent: use current context to proceed only if the action is safe, reversible, and aligned with user intent; otherwise ask for user confirmation. For destructive/irreversible/uncertain actions, present the option but do NOT mark it '(추천)' — it will stop for explicit user confirmation."
