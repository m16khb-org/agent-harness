...
func SuggestCommit(req CommitSuggestRequest) (CommitSuggestResult, error) {
	...
	if err := DecodeExternalLLMStructuredJSONObject("agy commit suggest", llm.Output, &response); err != nil {
...
func buildCommitSuggestPrompt(diff string) string {
	return BuildStructuredPrompt(StructuredPromptSpec{
		...
		Data: []PromptDataSection{
			BuildExternalLLMJSONSchemaSection(commitSuggestResponseSchemaExample(), []string{
