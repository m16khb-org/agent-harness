...
func DeterministicFixtures() []BenchmarkFixture {
	return []BenchmarkFixture{
		{ID: "article_basic", StatusCode: http.StatusOK, Headers: map[string]string{"Content-Type": "text/html"}, Body: "<main>" + strings.Repeat("article body ", 60) + "</main>", Expected: []string{CategoryStrongOK}, MinBodyChars: 500},
		{ID: "json_api", StatusCode: http.StatusOK, Headers: map[string]string{"Content-Type": "application/json"}, Body: `{"ok":true,"items":[{"id":1}]}`, Expected: []string{CategoryStrongOK}},
		{ID: "rss_feed", StatusCode: http.StatusOK, Headers: map[string]string{"Content-Type": "application/rss+xml"}, Body: `<rss><channel><item><title>one</title></item></channel></rss>`, Expected: []string{CategoryStrongOK, CategoryWeakOK}},
		{ID: "empty_spa", StatusCode: http.StatusOK, Headers: map[string]string{"Content-Type": "text/html"}, Body: `<html><body><div id="root"></div><script src="/app.js"></script></body></html>`, Expected: []string{CategorySuspectOK}},
		{ID: "waf_challenge", StatusCode: http.StatusOK, Headers: map[string]string{"Content-Type": "text/html"}, Body: `Just a moment... captcha check your browser`, Expected: []string{CategoryChallenge, CategoryBlocked}},
		{ID: "login_wall", StatusCode: http.StatusOK, Headers: map[string]string{"Content-Type": "text/html"}, Body: `Sign in to continue. Log in to view this page.`, Expected: []string{CategoryAuthRequired}},
		{ID: "paywall_shell", StatusCode: http.StatusOK, Headers: map[string]string{"Content-Type": "text/html"}, Body: `<meta property="og:title" content="Headline"><body>Subscribe to read this member-only article.</body>`, Expected: []string{CategoryPaywalled}},
		{ID: "rate_limit", StatusCode: http.StatusTooManyRequests, Headers: map[string]string{"Content-Type": "text/plain", "Retry-After": "1"}, Body: `slow down`, Expected: []string{CategoryRateLimited}},
		{ID: "redirect_loop", RedirectLoop: true, Expected: []string{CategoryUnknown}},
		{ID: "unsafe_redirect", UnsafeRedirect: true, Expected: []string{CategoryBlocked}},
		{ID: "gzip_charset", StatusCode: http.StatusOK, Headers: map[string]string{"Content-Type": "text/html; charset=shift_jis", "Content-Encoding": "gzip"}, Body: gzipString("<main>" + strings.Repeat("decoded body ", 60) + "</main>"), Expected: []string{CategoryStrongOK}},
		{ID: "malformed_content", StatusCode: http.StatusOK, Headers: map[string]string{"Content-Type": "application/json"}, Body: `{"broken":`, Expected: []string{CategoryUnknown, CategorySuspectOK}},
	}
}

func RunBenchmark(ctx context.Context, req BenchmarkRequest) (BenchmarkResult, error) {
	if req.Live {
	...
	result := BenchmarkResult{
		...
		LiveParityReport:    LiveParityReport{Warnings: []string{}},
	...
	for _, fixture := range fixtures {
		...
		if err != nil {
			...
			result.FixtureResults = append(result.FixtureResults, BenchmarkFixtureRun{ID: fixture.ID, OK: false, Failure: err.Error()})
			continue
		}
		unsafeRedirectFetches += finalFetches
		run := BenchmarkFixtureRun{ID: fixture.ID, Category: fetchResult.Category, StopReason: fetchResult.StopReason}
		...
		if isFalseStrongOKFixture(fixture.ID) && fetchResult.Category == CategoryStrongOK {
			falseStrongOK++
			run.OK = false
...
func runLiveBenchmark(ctx context.Context, req BenchmarkRequest) (BenchmarkResult, error) {
	result := BenchmarkResult{
	...
	for _, fixture := range req.Fixtures {
		if strings.TrimSpace(fixture.URL) == "" {
			...
			result.FixtureResults = append(result.FixtureResults, BenchmarkFixtureRun{ID: fixture.ID, OK: false, Failure: msg})
			continue
		}
		started := time.Now()
		fetchResult, err := Fetch(ctx, Request{
			URL:                 fixture.URL,
			Timeout:             timeout,
		...
		run := BenchmarkFixtureRun{ID: fixture.ID, Category: fetchResult.Category, StopReason: fetchResult.StopReason, LatencyMS: latencyMS}
		if err != nil {
...
func compareBaseline(ctx context.Context, command string, fixtures []BenchmarkFixture, candidateCategories map[string]string, timeout time.Duration, result *BenchmarkResult) {
...
func runFixture(ctx context.Context, fixture BenchmarkFixture, timeout time.Duration) (Result, int, error) {
	...
	fetchResult, err := Fetch(ctx, Request{URL: server.URL, Timeout: timeout, AllowPrivateNetwork: true})
...
func scoreBenchmark(result BenchmarkResult) float64 {
	score := 100.0
