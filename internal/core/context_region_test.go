...
func TestContextSerializationStableDistinguishesDriftFromVolatile(t *testing.T) {
	...
	stable, _, err := ContextSerializationStable(func() any {
	...
	volatileCalls := 0
	volatileStable, _, err := ContextSerializationStable(func() any {
		volatileCalls++
	...
	driftCalls := 0
	drifted, _, err := ContextSerializationStable(func() any {
		driftCalls++
...
func TestDocsIndexImmutablePrefixIsByteDeterministic(t *testing.T) {
	...
	first := contextMarshal(t, StableProjection(contextToAny(t, DocsIndex(root, "0.1.0"))))
	second := contextMarshal(t, StableProjection(contextToAny(t, DocsIndex(root, "0.1.0"))))
	if first != second {
		t.Fatalf("docs_index immutable prefix drifted across builds:\nfirst=%s\nsecond=%s", first, second)
	}

	raw, ok := contextToAny(t, DocsIndex(root, "0.1.0")).(map[string]any)
	if !ok {
	...
	if _, present := StableProjection(raw).(map[string]any)["generated_at"]; present {
