...
func TestExecutionMutationRequiresCurrentLeaseHolderInCanonicalWorktree(t *testing.T) {
	...
	record := IssueOpsRecord{Execution: &model.ExecutionV1{
		...
		Lease: model.WriteLeaseV1{Generation: 3, Status: model.LeaseStatusActive, Holder: &holder, ClaimedAt: "2026-07-22T00:00:01Z"},
	}}

	exact := IssueOpsActor{Host: "codex", SessionID: "session-1", AgentID: "agent-1", CWD: root, NativeProcessAncestry: []model.NativeProcessReceiptV1{*holder.SessionProcess}}
	...
	for name, actor := range map[string]*IssueOpsActor{
		...
		"reused process":     {Host: "codex", SessionID: "session-1", AgentID: "agent-1", CWD: root, NativeProcessAncestry: []model.NativeProcessReceiptV1{{PID: 42, StartedAt: "2026-07-22T00:00:01Z", Executable: "/usr/bin/codex"}}},
...
func TestExecutionMutationAllowsPreExecutionPlanningButFencesNonActiveLease(t *testing.T) {
	...
	record := IssueOpsRecord{Execution: &model.ExecutionV1{
		...
		Lease: model.WriteLeaseV1{Generation: 1, Status: model.LeaseStatusReleased, ReleasedAt: "2026-07-22T00:00:01Z"},
	}}
