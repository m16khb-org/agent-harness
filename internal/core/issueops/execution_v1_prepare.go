...
type ExecutionPrepareRequestV1 struct {
	ID          string              `json:"id"`
...
type ExecutionPrepareDependenciesV1 struct {
...
type ExecutionPrepareResultV1 struct {
	OK                  bool               `json:"ok"`
	ID                  string             `json:"id"`
	...
	Workspace           model.WorkspaceV1  `json:"workspace"`
	Execution           *model.ExecutionV1 `json:"execution,omitempty"`
...
func PrepareExecutionV1(ctx context.Context, stateRoot string, req ExecutionPrepareRequestV1, deps ExecutionPrepareDependenciesV1) (ExecutionPrepareResultV1, error) {
	if req.Confirm {
		if err := RequireIssueOpsV1MutationAllowed(stateRoot); err != nil {
			return ExecutionPrepareResultV1{OK: false, ID: req.ID}, err
		}
	}
	...
	if record.Execution != nil {
		if record.Execution.Pending != nil {
			...
			result.OK = false
			result.NextCommand = executionReconcileCommandV1(record.ID, true)
	...
	if err != nil {
		return ExecutionPrepareResultV1{OK: false, ID: req.ID, RequestedMode: requested}, err
	}
	resolved, fallback, probe, err := resolveExecutionPrepareMode(ctx, record, req, requested, deps.Orca)
	if err != nil {
		return ExecutionPrepareResultV1{OK: false, ID: req.ID, RequestedMode: requested}, err
	}
	result := ExecutionPrepareResultV1{
...
func prepareDirectExecution(ctx context.Context, stateRoot string, record IssueOpsRecord, req ExecutionPrepareRequestV1, deps ExecutionPrepareDependenciesV1, workspaceReq port.ExecutionWorkspaceRequest, result ExecutionPrepareResultV1) (ExecutionPrepareResultV1, error) {
	if deps.Direct == nil {
		return ExecutionPrepareResultV1{OK: false, ID: record.ID}, fmt.Errorf("direct Git worktree provisioner is unavailable")
	}
	actor, err := normalizeNativeActorV1(req.Actor)
	if err != nil {
		return ExecutionPrepareResultV1{OK: false, ID: record.ID}, err
	}
	if !samePath(req.CWD, record.Repo) && !samePath(req.CWD, workspaceReq.Root) {
		return ExecutionPrepareResultV1{OK: false, ID: record.ID}, fmt.Errorf("direct prepare cwd must be source_root or the canonical worktree")
	}
	if req.Confirm {
		prober, ok := deps.Direct.(port.ExecutionWorkspaceAccessProber)
		if !ok {
			return ExecutionPrepareResultV1{OK: false, ID: record.ID}, fmt.Errorf("direct provisioner cannot verify canonical worktree base access")
		}
		...
		if !access.Allowed {
			return ExecutionPrepareResultV1{
...
func prepareOrcaExecution(ctx context.Context, stateRoot string, record IssueOpsRecord, req ExecutionPrepareRequestV1, deps ExecutionPrepareDependenciesV1, workspaceReq port.ExecutionWorkspaceRequest, probe port.ExecutionOrcaProbeRequest, result ExecutionPrepareResultV1) (ExecutionPrepareResultV1, error) {
	if deps.Orca == nil {
		return ExecutionPrepareResultV1{OK: false, ID: record.ID}, fmt.Errorf("Orca provisioner is unavailable")
	}
	if !req.Confirm {
		return result, nil
	}
	...
	req.Actor = actor
	if !samePath(req.CWD, record.Repo) && !samePath(req.CWD, workspaceReq.Root) {
		return ExecutionPrepareResultV1{OK: false, ID: record.ID}, fmt.Errorf("Orca prepare cwd must be source_root or the canonical worktree")
	}
	snapshot, err := readExecutionOwnerSnapshotV1(ctx, record, deps.ReadIssue)
	if err != nil {
		return ExecutionPrepareResultV1{OK: false, ID: record.ID}, err
	}
	pending, payload, err := beginOrcaExecutionIntentV1(stateRoot, record, workspaceReq, probe, req, snapshot, deps.Now)
	if err != nil {
		return ExecutionPrepareResultV1{OK: false, ID: record.ID}, err
	}
	for step := 0; pending.Execution != nil && pending.Execution.Pending != nil; step++ {
		if step >= 4 {
			return ExecutionPrepareResultV1{OK: false, ID: record.ID}, fmt.Errorf("Orca prepare exceeded the fixed external intent stage count")
		}
		pending, payload, err = executeOrcaIntentStageV1(ctx, stateRoot, pending, payload, deps.Orca, deps.ReadIssue, deps.Now)
		if err != nil {
			return ExecutionPrepareResultV1{OK: false, ID: record.ID}, err
		}
	}
...
func validateExecutionOrcaReceiptV1(workspace port.ExecutionWorkspaceRequest, receipt port.ExecutionOrcaReceipt) error {
...
func validateExecutionOrcaWorkspaceReceiptV1(workspace port.ExecutionWorkspaceRequest, receipt port.ExecutionOrcaWorkspaceReceipt) error {
	got := receipt.Workspace
	if !samePath(got.SourceRoot, workspace.SourceRoot) || !samePath(got.Root, workspace.Root) || got.Branch != workspace.Branch || got.BaseHead != workspace.BaseHead || got.Driver != "orca" ||
...
func newExecutionOperationIDV1() (string, error) {
	var raw [16]byte
...
func resolveExecutionPrepareMode(ctx context.Context, record IssueOpsRecord, req ExecutionPrepareRequestV1, requested string, orca port.ExecutionOrcaProvisioner) (string, string, port.ExecutionOrcaProbeRequest, error) {
...
var issueOpsV1OwnerReportLabels = []string{
	"Status",
	"Lifecycle",
	"Mode/host/model",
...
func renderExecutionOwnerReportContractV1(record IssueOpsRecord, req ExecutionPrepareRequestV1) string {
	...
	for index, label := range issueOpsV1OwnerReportLabels {
		lines = append(lines, "- "+label+": "+values[index])
	}
	return strings.Join(lines, "\n")
}

func executionReconcileCommandV1(id string, preview bool) string {
	action := "--confirm"
	if preview {
		action = "--preview"
	}
...
func workspaceFromReceipt(receipt port.ExecutionWorkspaceReceipt, linkedAt string) model.WorkspaceV1 {
	return model.WorkspaceV1{
...
func preparedExecutionResult(record IssueOpsRecord, requested string) ExecutionPrepareResultV1 {
	return preparedExecutionResultWithModes(record, requested, "")
}

func preparedExecutionResultWithModes(record IssueOpsRecord, requested, fallback string) ExecutionPrepareResultV1 {
	return ExecutionPrepareResultV1{
