...
func TestExecutionV1OrcaCrashAfterMutationReconcilesExactlyOneCandidateWithoutDuplicate(t *testing.T) {
	tests := []struct {
		name      string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			payload, err := readExternalOrcaIntentPayloadV1(stateRoot, pending.Execution.Pending.OperationID)
			if err != nil {
				t.Fatal(err)
			}
			if payload.Stage != test.stage || payload.InvocationState != orcaIntentUnknownV1 || calls[test.stage] != 1 {
			...
			raw, ok, err := db.Get(externalIntentV1Bucket, payload.OperationID)
			if err != nil || !ok {
			...
			result, err := ReconcileExecutionV1WithDependencies(context.Background(), stateRoot, ExecutionReconcileRequestV1{
				ID: record.ID, Confirm: true, Actor: executionActorV1("codex", "fresh-reconciler"), CWD: record.Repo,
			}, ExecutionReconcileDependenciesV1{Orca: fake, ReadIssue: executionIssueSnapshotReaderV1})
			if err != nil {
				t.Fatal(err)
			}
			...
			if test.completed {
				...
				if _, err := readExternalOrcaIntentPayloadV1(stateRoot, payload.OperationID); err == nil {
...
func TestExecutionV1OrcaReconcileZeroMultipleAndTransportAmbiguityNeverMutate(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			_, _ = PrepareExecutionV1(context.Background(), stateRoot, req, ExecutionPrepareDependenciesV1{Orca: fake, ReadIssue: executionIssueSnapshotReaderV1})
			fake.inspect = test.inspect
			if _, err := ReconcileExecutionV1WithDependencies(context.Background(), stateRoot, ExecutionReconcileRequestV1{
				ID: record.ID, Confirm: true, Actor: executionActorV1("claude", "fresh"), CWD: record.Repo,
			}, ExecutionReconcileDependenciesV1{Orca: fake, ReadIssue: executionIssueSnapshotReaderV1}); err == nil {
...
func TestExecutionV1OrcaReconcileRetriesOnlyProvenNotInvokedAndOnlyOnce(t *testing.T) {
	stateRoot, record := executionPrepareRecord(t)
	invokeCalls := 0
	fake := &executionOrcaFake{probe: port.ExecutionOrcaProbeResult{Available: true, Ready: true}}
	fake.invoke = func(request port.ExecutionOrcaIntentRequest) (port.ExecutionOrcaIntentReceipt, error) {
	...
	reconcile := func() error {
		_, err := ReconcileExecutionV1WithDependencies(context.Background(), stateRoot, ExecutionReconcileRequestV1{
			ID: record.ID, Confirm: true, Actor: executionActorV1("codex", "fresh"), CWD: record.Repo,
		}, ExecutionReconcileDependenciesV1{Orca: fake, ReadIssue: executionIssueSnapshotReaderV1})
		return err
	}
...
func TestExecutionV1OrcaReceiptCASRejectsConcurrentIntentChange(t *testing.T) {
	stateRoot, record := executionPrepareRecord(t)
	fake := &executionOrcaFake{probe: port.ExecutionOrcaProbeResult{Available: true, Ready: true}}
