...
type claimableExecutionFixtureV1 struct {
	record    IssueOpsRecord
	worktree  string
	tokenPath string
}

func TestExecutionV1ConcurrentClaimsChooseOneLifecyclePerSession(t *testing.T) {
	stateRoot := t.TempDir()
	first := newClaimableExecutionFixtureV1(t, stateRoot, "69-first")
	second := newClaimableExecutionFixtureV1(t, stateRoot, "70-second")
	actor := executionActorV1("codex", "one-native-session")
	start := make(chan struct{})

	requests := []ExecutionClaimRequestV1{
	...
	for _, request := range requests {
		...
		go func() {
			ready.Done()
			<-start
			_, err := claimExecutionV1(stateRoot, request)
			errs <- err
		}()
	}
	ready.Wait()
	...
	active := 0
	for _, fixture := range []claimableExecutionFixtureV1{first, second} {
...
func TestExecutionV1CompetingClaimsChooseOneHolder(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-competing")
	start := make(chan struct{})
	errs := make(chan error, 2)
	for _, actor := range []model.NativeActorV1{
		executionActorV1("codex", "session-a"),
		executionActorV1("claude", "session-b"),
	} {
		actor := actor
		go func() {
			<-start
			_, err := claimExecutionV1(stateRoot, ExecutionClaimRequestV1{
...
func TestExecutionV1ClaimRetryIsIdempotentAfterTokenConsumption(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-idempotent")
	actor := executionActorV1("codex", "idempotent-session")
	request := ExecutionClaimRequestV1{
		ID: fixture.record.ID, Generation: 1, Actor: actor,
		CWD: fixture.worktree, TokenFile: fixture.tokenPath,
	}
	if _, err := claimExecutionV1(stateRoot, request); err != nil {
		t.Fatal(err)
	}
...
func TestExecutionV1ClaimRejectsInsecureOrSymlinkToken(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			stateRoot := t.TempDir()
			fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-token-"+testCase.name)
			testCase.mutate(t, fixture.tokenPath)
			_, err := claimExecutionV1(stateRoot, ExecutionClaimRequestV1{
				ID: fixture.record.ID, Generation: 1,
				Actor: executionActorV1("codex", "token-session"),
				CWD:   fixture.worktree, TokenFile: fixture.tokenPath,
			})
			if err == nil {
...
func TestExecutionV1ReseedInvalidatesPriorGenerationToken(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-reseed")
	requester := executionActorV1("claude", "reseed-requester")
	...
	reseeded, err := ReplaceExecutionV1(stateRoot, ExecutionReplaceRequestV1{
	...
	actor := executionActorV1("claude", "reseed-session")
...
func TestExecutionV1RevokeRejectsStaleInventoryAndFencesOldGeneration(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-revoke")
	old := executionActorV1("codex", "old-session")
	if _, err := claimExecutionV1(stateRoot, ExecutionClaimRequestV1{
	...
	requester := executionActorV1("claude", "revoke-requester")
	preview, err := ReplaceExecutionV1(stateRoot, ExecutionReplaceRequestV1{
	...
	if _, err := ReplaceExecutionV1(stateRoot, ExecutionReplaceRequestV1{
	...
	if _, err := ReleaseExecutionV1(stateRoot, ExecutionReleaseRequestV1{
...
func TestExecutionV1FinalizeRejectsLiveOwnerProcess(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-live")
	old := executionActorV1("codex", "live-session")
	if _, err := claimExecutionV1(stateRoot, ExecutionClaimRequestV1{
		ID: fixture.record.ID, Generation: 1, Actor: old,
		CWD: fixture.worktree, TokenFile: fixture.tokenPath,
	}); err != nil {
	...
	requester := executionActorV1("claude", "live-revoke-requester")
	preview, err := ReplaceExecutionV1(stateRoot, ExecutionReplaceRequestV1{
		ID: fixture.record.ID, Action: ExecutionReplacePreview, ExpectedGeneration: 1, Actor: requester, CWD: fixture.record.Repo,
	})
	if err != nil {
...
func TestExecutionV1ClaimRejectsForgedLiveProcessIdentity(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-forged-process")
	actor := executionActorV1("codex", "forged-process-session")
...
func TestExecutionV1ReplaceRejectsForgedRequesterAndForeignCWD(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-forged-replacer")
	actor := executionActorV1("claude", "forged-replacer")
	actor.SessionProcess.StartedAt = "1970-01-01T00:00:00Z"
	if _, err := ReplaceExecutionV1(stateRoot, ExecutionReplaceRequestV1{
		ID: fixture.record.ID, Action: ExecutionReplacePreview, ExpectedGeneration: 1,
	...
	actor = executionActorV1("claude", "foreign-cwd-replacer")
	if _, err := ReplaceExecutionV1(stateRoot, ExecutionReplaceRequestV1{
		ID: fixture.record.ID, Action: ExecutionReplacePreview, ExpectedGeneration: 1,
...
func TestExecutionV1FinalizeRejectsPIDReuseIdentityMismatch(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-pid-reuse")
	actor := executionActorV1("codex", "pid-reuse-session")
	if _, err := claimExecutionV1(stateRoot, ExecutionClaimRequestV1{
		ID: fixture.record.ID, Generation: 1, Actor: actor,
		CWD: fixture.worktree, TokenFile: fixture.tokenPath,
	}); err != nil {
	...
	requester := executionActorV1("claude", "pid-reuse-requester")
	preview, err := ReplaceExecutionV1(stateRoot, ExecutionReplaceRequestV1{
		ID: fixture.record.ID, Action: ExecutionReplacePreview, ExpectedGeneration: 1, Actor: requester, CWD: fixture.record.Repo,
	})
	if err != nil {
...
func TestExecutionV1FinalizeRejectsWorkspaceCWDOrWritableFileProcess(t *testing.T) {
	for _, testCase := range []struct {
		name  string
		start func(t *testing.T, fixture claimableExecutionFixtureV1) *exec.Cmd
	}{
		{name: "cwd", start: func(t *testing.T, fixture claimableExecutionFixtureV1) *exec.Cmd {
			cmd := exec.Command("sleep", "30")
			cmd.Dir = fixture.worktree
			startExecutionProcessFixtureV1(t, cmd)
			return cmd
		}},
		{name: "writable-file", start: func(t *testing.T, fixture claimableExecutionFixtureV1) *exec.Cmd {
		...
		t.Run(testCase.name, func(t *testing.T) {
			stateRoot := t.TempDir()
			fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-process-"+testCase.name)
			activateExecutionFixtureWithDeadHolderV1(t, stateRoot, &fixture)
			cmd := testCase.start(t, fixture)
			t.Cleanup(func() {
				_ = cmd.Process.Kill()
				_ = cmd.Wait()
			})
			requester := executionActorV1("claude", "process-audit-requester-"+testCase.name)
			preview, err := ReplaceExecutionV1(stateRoot, ExecutionReplaceRequestV1{
				ID: fixture.record.ID, Action: ExecutionReplacePreview, ExpectedGeneration: 1, Actor: requester, CWD: fixture.record.Repo,
			})
			if err != nil {
...
func TestExecutionV1OrcaFinalizeRequiresTerminalAndTaskQuiescence(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-orca-quiescence")
	activateExecutionFixtureWithDeadHolderV1(t, stateRoot, &fixture)
	record, err := ReadIssueOps(stateRoot, fixture.record.ID)
	if err != nil {
		t.Fatal(err)
	...
	record.Execution.Orca = &model.OrcaBindingV1{
	...
	deps := ExecutionReplaceDependenciesV1{OrcaOwner: inspector}
	requester := executionActorV1("codex", "orca-replacement-requester")
	preview, err := ReplaceExecutionV1WithDependencies(context.Background(), stateRoot, ExecutionReplaceRequestV1{
		ID: record.ID, Action: ExecutionReplacePreview, ExpectedGeneration: 1, Actor: requester, CWD: record.Repo,
	}, deps)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := ReplaceExecutionV1WithDependencies(context.Background(), stateRoot, ExecutionReplaceRequestV1{
...
func newClaimableExecutionFixtureV1(t *testing.T, stateRoot, branch string) claimableExecutionFixtureV1 {
	t.Helper()
	...
	return claimableExecutionFixtureV1{record: record, worktree: worktree, tokenPath: tokenPath}
}

func executionActorV1(host, sessionID string) model.NativeActorV1 {
...
func activateExecutionFixtureWithDeadHolderV1(t *testing.T, stateRoot string, fixture *claimableExecutionFixtureV1) {
	t.Helper()
	_ = os.Remove(fixture.tokenPath)
	fixture.record.Execution.Lease = model.WriteLeaseV1{
		...
		Holder: &model.NativeActorV1{
			...
			SessionProcess: &model.NativeProcessReceiptV1{
...
func startExecutionProcessFixtureV1(t *testing.T, cmd *exec.Cmd) {
	t.Helper()
	if err := cmd.Start(); err != nil {
		t.Fatal(err)
	}
	if _, err := ObserveNativeProcessReceiptV1(cmd.Process.Pid); err != nil {
		_ = cmd.Process.Kill()
		_ = cmd.Wait()
