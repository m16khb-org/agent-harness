...
func TestExecutionIssueSnapshotEvidenceSealsPrepareAndClaimWithoutFallback(t *testing.T) {
	...
	}, ExecutionActionDependencies{ReadIssue: fallback, Claim: executionSnapshotClaimHandler(evidence)})
	if err != nil {
		t.Fatal(err)
	}
...
func TestExecutionIssueSnapshotEvidenceSealsFinalizeAndReplacementClaimWithoutFallback(t *testing.T) {
	...
	raw, err = ExecuteExecution(context.Background(), fixture.stateRoot, ExecutionActionRequest{
		...
		IssueSnapshot: evidence,
	}, ExecutionActionDependencies{Claim: executionSnapshotClaimHandler(evidence)})
	if err != nil {
...
func executionSnapshotClaimHandler(_ *port.ExecutionIssueSnapshotEvidence) ExecutionClaimHandler {
	return func(ctx context.Context, stateRoot string, request ExecutionClaimRequest) (ExecutionResult, error) {
		return claimViaVerticalWithDeps(ctx, stateRoot, request, ExecutionClaimDependencies{ReadIssue: request.ReadIssue})
	}
}
