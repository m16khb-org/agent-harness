...
func TestSupervisedHandoffCyclesRetainsOnlyCurrentNonterminalMissingWorktreeAuthority(t *testing.T) {
	...
	store.writeRecord(t, model.IssueOpsRecord{
		...
		Ownership:  &model.IssueOpsOwnershipLedger{ActiveAttempt: 1, Attempts: []model.IssueOpsOwnershipAttempt{{Number: 1, Handoff: &model.IssueOpsExecutionHandoff{State: "owner_active", WorkerRoot: missing}}}},
	})
	...
	store.writeRecord(t, model.IssueOpsRecord{
		...
		Ownership:  &model.IssueOpsOwnershipLedger{Attempts: []model.IssueOpsOwnershipAttempt{{Number: 1, Handoff: &model.IssueOpsExecutionHandoff{State: "closed", WorkerRoot: filepath.Join(t.TempDir(), "closed-worker")}}}},
	})
...
func TestOwnershipLedgerHistoricalAttemptNeverBecomesCurrentAuthority(t *testing.T) {
	store := newActiveTestStore(t)
	repo := t.TempDir()
	historicalWorker := filepath.Join(t.TempDir(), "historical-worker")
	currentWorker := filepath.Join(t.TempDir(), "current-worker")
	store.writeRecord(t, model.IssueOpsRecord{
		SchemaVersion: model.IssueOpsCurrentSchemaVersion,
		ID:            "io-active",
		Repo:          repo,
		Branch:        "68-ledger-authority",
		Phase:         model.IssueOpsPhaseImplement,
		CycleState:    model.IssueOpsCycleActive,
		Ownership: &model.IssueOpsOwnershipLedger{ActiveAttempt: 2, Attempts: []model.IssueOpsOwnershipAttempt{
			{Number: 1, Workspace: &model.IssueOpsExecutionWorkspace{WorkerRoot: historicalWorker}, Handoff: &model.IssueOpsExecutionHandoff{State: "closed", WorkerRoot: historicalWorker}, ClosedAt: "2026-07-22T00:30:00Z"},
			{Number: 2, Workspace: &model.IssueOpsExecutionWorkspace{WorkerRoot: currentWorker}, Handoff: &model.IssueOpsExecutionHandoff{State: "owner_active", WorkerRoot: currentWorker}},
		}},
	})

	current := SupervisedHandoffCyclesForRepo(store.issueOpsStore(), currentWorker)
	if len(current) != 1 || current[0].ID != "io-active" {
		t.Fatalf("current attempt did not retain authority: %+v", current)
	}
	if historical := SupervisedHandoffCyclesForRepo(store.issueOpsStore(), historicalWorker); len(historical) != 0 {
		t.Fatalf("historical attempt regained live authority: %+v", historical)
	}
}

func TestPausedCycleReleasesOrdinaryActiveCycleSelection(t *testing.T) {
	store := newActiveTestStore(t)
	repo := t.TempDir()
	store.writeRecord(t, model.IssueOpsRecord{
		SchemaVersion: model.IssueOpsCurrentSchemaVersion,
	...
	if record, ok := CycleForBranch(store.issueOpsStore(), repo, "68-paused"); ok {
		t.Fatalf("paused cycle retained ordinary source fence: %+v", record)
	}
}
