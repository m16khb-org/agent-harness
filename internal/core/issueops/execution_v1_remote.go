...
const (
	externalIntentV1Bucket       = "external_intent_v1"
	externalIntentRemotePRV1     = "remote_pr_create"
	remoteInvocationUnknownV1    = "unknown"
	remoteInvocationNotInvokedV1 = "not_invoked_proven"
)

type RemotePullRequestCreateFuncV1 func(string, port.IssueProviderCreatePullRequestRequest) (port.IssueProviderCreatePullRequestResult, error)
type RemotePullRequestReconcileFuncV1 func(string, port.IssueProviderReconcilePullRequestRequest) (port.IssueProviderReconcilePullRequestResult, error)
type RemoteArtifactVerifyFuncV1 func(model.IssueOpsRemoteArtifactVerificationRequest) error

type RemotePullRequestRequestV1 struct {
	ID                 string              `json:"id"`
...
type RemotePullRequestDependenciesV1 struct {
	Create    RemotePullRequestCreateFuncV1
	Reconcile RemotePullRequestReconcileFuncV1
	Verify    RemoteArtifactVerifyFuncV1
	Now       func() time.Time
}

type externalRemotePRPayloadV1 struct {
...
// CreateRemotePullRequestV1 is the only IssueOps v1 PR/MR creation path. It
// persists one exact operation intent before invoking the provider and never
// retries an ambiguous call.
func CreateRemotePullRequestV1(ctx context.Context, stateRoot string, req RemotePullRequestRequestV1, deps RemotePullRequestDependenciesV1) (port.IssueProviderCreatePullRequestResult, error) {
	if req.Confirm {
	...
	record, providerReq, kind, err := prepareRemotePullRequestV1(stateRoot, req)
	if err != nil {
		return port.IssueProviderCreatePullRequestResult{}, err
	}
	if !req.Confirm {
	...
	pending, payload, err := beginRemotePullRequestIntentV1(stateRoot, record, req.Actor, req.CWD, req.ExpectedGeneration, providerReq, req.Provider, kind, deps.Now)
	if err != nil {
		return port.IssueProviderCreatePullRequestResult{}, err
	}
	result, callErr := deps.Create(payload.Provider, payload.Request)
	if callErr != nil {
		invocation := remoteInvocationUnknownV1
		var typed *port.IssueProviderCreateError
		if errors.As(callErr, &typed) && !typed.Invoked {
			invocation = remoteInvocationNotInvokedV1
		}
		_ = recordRemotePullRequestFailureV1(stateRoot, pending.ID, payload.OperationID, invocation, payload.RetryCount, result.URL, callErr, deps.Now)
	...
	if strings.TrimSpace(result.URL) == "" {
		...
		_ = recordRemotePullRequestFailureV1(stateRoot, pending.ID, payload.OperationID, remoteInvocationUnknownV1, payload.RetryCount, "", err, deps.Now)
		return result, err
	}
	if err := verifyRemotePullRequestResultV1(record, payload, result.URL, deps.Verify); err != nil {
		_ = recordRemotePullRequestFailureV1(stateRoot, pending.ID, payload.OperationID, remoteInvocationUnknownV1, payload.RetryCount, result.URL, err, deps.Now)
		return result, fmt.Errorf("provider returned a URL but durable verification requires execution reconcile: %w", err)
	}
	if _, err := finishRemotePullRequestIntentV1(stateRoot, pending.ID, payload, result.URL, true, deps.Now); err != nil {
		_ = recordRemotePullRequestFailureV1(stateRoot, pending.ID, payload.OperationID, remoteInvocationUnknownV1, payload.RetryCount, result.URL, err, deps.Now)
...
func prepareRemotePullRequestV1(stateRoot string, req RemotePullRequestRequestV1) (IssueOpsRecord, port.IssueProviderCreatePullRequestRequest, string, error) {
	...
	if req.Confirm && !validExecutionHeadV1(headSHA) {
...
func beginRemotePullRequestIntentV1(stateRoot string, expected IssueOpsRecord, actor model.NativeActorV1, cwd string, expectedGeneration uint64, providerReq port.IssueProviderCreatePullRequestRequest, provider, kind string, now func() time.Time) (IssueOpsRecord, externalRemotePRPayloadV1, error) {
	operationID, err := newExecutionOperationIDV1()
	if err != nil {
		return IssueOpsRecord{}, externalRemotePRPayloadV1{}, err
	}
	marker := "<!-- agent-harness:issueops-v1 operation=" + operationID + " -->"
	providerReq.Body = strings.TrimSpace(providerReq.Body) + "\n\n" + marker
	payload := externalRemotePRPayloadV1{
		SchemaVersion: model.IssueOpsSchemaVersion, OperationID: operationID, Provider: provider, Kind: kind,
		Request: providerReq, InvocationState: remoteInvocationUnknownV1,
	}
	...
	err = withIssueOpsLock(context.Background(), stateRoot, expected.ID, func(context.Context) error {
		...
		current.Execution.Pending = &model.ExternalIntentV1{OperationID: operationID, Kind: externalIntentRemotePRV1, Marker: marker, StartedAt: executionNow(now)}
...
func recordRemotePullRequestFailureV1(stateRoot, id, operationID, invocation string, retryCount int, knownURL string, cause error, now func() time.Time) error {
	return withIssueOpsLock(context.Background(), stateRoot, id, func(context.Context) error {
		...
		payload, err := readExternalRemotePRPayloadV1(stateRoot, operationID)
		if err != nil {
			return err
		}
		...
		record.Execution.Failure = &model.ExecutionFailureV1{OperationID: operationID, Code: "external_operation_ambiguous", Message: message, At: executionNow(now)}
		_, err = persistExecutionTransitionWithMutations(stateRoot, record, nil, []sqlstore.Mutation{{Bucket: externalIntentV1Bucket, ID: operationID, Data: data}})
		return err
	})
}

func finishRemotePullRequestIntentV1(stateRoot, id string, payload externalRemotePRPayloadV1, url string, enforceOriginalGeneration bool, now func() time.Time) (IssueOpsRecord, error) {
	var persisted IssueOpsRecord
	err := withIssueOpsLock(context.Background(), stateRoot, id, func(context.Context) error {
		...
		stored, err := readExternalRemotePRPayloadV1(stateRoot, payload.OperationID)
		if err != nil {
			return err
		}
		...
		current.Execution.Failure = nil
		persisted, err = persistExecutionTransitionWithMutations(stateRoot, current, nil, []sqlstore.Mutation{{Bucket: externalIntentV1Bucket, ID: payload.OperationID, Delete: true}})
		return err
	})
	return persisted, err
}

func readExternalRemotePRPayloadV1(stateRoot, operationID string) (externalRemotePRPayloadV1, error) {
	...
	if !ok {
		return externalRemotePRPayloadV1{}, fmt.Errorf("external intent payload is missing")
	}
	var payload externalRemotePRPayloadV1
	if err := json.Unmarshal(data, &payload); err != nil {
		return externalRemotePRPayloadV1{}, fmt.Errorf("decode external intent payload: %w", err)
	}
	if payload.SchemaVersion != model.IssueOpsSchemaVersion || payload.OperationID != operationID || payload.Generation == 0 || payload.Provider == "" || payload.Kind == "" {
		return externalRemotePRPayloadV1{}, fmt.Errorf("external intent payload is invalid")
	}
	return payload, nil
}

func remotePullRequestReconcileRequestV1(payload externalRemotePRPayloadV1) port.IssueProviderReconcilePullRequestRequest {
	req := payload.Request
...
func validateRemotePullRequestCandidateV1(record IssueOpsRecord, payload externalRemotePRPayloadV1, candidate port.IssueProviderReconcilePullRequestCandidate) error {
	expected := remotePullRequestReconcileRequestV1(payload)
	if strings.TrimSpace(candidate.ProjectKey) != expected.ProjectKey || strings.TrimSpace(candidate.SourceProjectKey) != expected.ProjectKey ||
		...
		!sameCanonicalRemoteSetV1(candidate.Labels, expected.Labels) || !sameCanonicalRemoteSetV1(candidate.Assignees, expected.Assignees) {
...
func verifyRemotePullRequestResultV1(record IssueOpsRecord, payload externalRemotePRPayloadV1, url string, verify RemoteArtifactVerifyFuncV1) error {
...
func sameCanonicalRemoteSetV1(left, right []string) bool {
...
func validExecutionHeadV1(value string) bool {
...
func boundedExecutionRemoteDiagnosticV1(err error) string {
	if err == nil {
