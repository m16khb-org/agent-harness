...
func TestIssueOpsV1UsesOnlySchemaOneAndDedicatedNamespace(t *testing.T) {
	stateRoot := t.TempDir()
...
func TestIssueOpsV1ReaderIgnoresLegacyBucketAndFailsClosedOnOtherSchemas(t *testing.T) {
	stateRoot := t.TempDir()
	db, err := sqlstore.Open(stateRoot)
	if err != nil {
		t.Fatal(err)
...
func TestIssueOpsV1RejectsLegacyExecutionAuthorityPayload(t *testing.T) {
	stateRoot := t.TempDir()
	db, err := sqlstore.Open(stateRoot)
	if err != nil {
		t.Fatal(err)
...
func TestIssueOpsDefaultStateRootIsV1Specific(t *testing.T) {
