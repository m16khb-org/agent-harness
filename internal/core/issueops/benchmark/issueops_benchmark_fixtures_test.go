...
func completeBenchmarkArtifactForTest() IssueOpsBenchmarkArtifact {
	return IssueOpsBenchmarkArtifact{
		...
		TaskBreakdown:          "Worker A owns internal/core/issueops_benchmark.go. Worker B owns cmd/harness/issueops.go. Large issue is unsafe as one work item because one issue would hide risky behavior changes. It uses provider-native child work items and records them with issueops link-child.",
