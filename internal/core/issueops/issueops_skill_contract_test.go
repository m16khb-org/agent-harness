...
func TestIssueOpsSkillDocumentsOptionalOrcaHandoffContract(t *testing.T) {
	...
	for _, want := range []string{
		...
		"unsealed retry may explicitly replace those defaults before the next preview",
		"owner may push only the exact branch and create a draft PR/MR",
		"published final head, changed files, Turing report, verification, and durable draft PR/MR artifact",
