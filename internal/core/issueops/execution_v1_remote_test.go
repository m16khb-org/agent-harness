...
type remoteExecutionFixtureV1 struct {
	claimableExecutionFixtureV1
	actor model.NativeActorV1
}

func TestRemotePullRequestV1PersistsIntentBeforeSingleProviderCallWithActiveLease(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-remote-intent")
	...
	actor := executionActorV1("codex", "remote-intent-session")
	...
	providerCalls := 0
	result, err := CreateRemotePullRequestV1(context.Background(), stateRoot, RemotePullRequestRequestV1{
	...
	}, RemotePullRequestDependenciesV1{Create: func(provider string, request port.IssueProviderCreatePullRequestRequest) (port.IssueProviderCreatePullRequestResult, error) {
		providerCalls++
...
func TestRemotePullRequestV1AuthoritativeZeroRetriesOnlyProvenNotInvokedOnce(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newRemoteExecutionFixtureV1(t, stateRoot, "69-remote-zero-retry")
	initialCalls := 0
	_, err := CreateRemotePullRequestV1(context.Background(), stateRoot, fixture.request("retry once"), RemotePullRequestDependenciesV1{
	...
	retryCalls := 0
	result, err := ReconcileExecutionV1WithDependencies(context.Background(), stateRoot, ExecutionReconcileRequestV1{
		ID: fixture.record.ID, Confirm: true, Actor: fixture.actor, CWD: fixture.worktree,
	}, ExecutionReconcileDependenciesV1{RemotePR: RemotePullRequestDependenciesV1{
...
func TestRemotePullRequestV1ZeroUnprovenAndMultipleCandidatesRetainIntentWithoutRetry(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			stateRoot := t.TempDir()
			fixture := newRemoteExecutionFixtureV1(t, stateRoot, "69-remote-"+tc.name)
			_, err := CreateRemotePullRequestV1(context.Background(), stateRoot, fixture.request(tc.name), RemotePullRequestDependenciesV1{
				Create: func(string, port.IssueProviderCreatePullRequestRequest) (port.IssueProviderCreatePullRequestResult, error) {
			...
			retryCalls := 0
			result, reconcileErr := ReconcileExecutionV1WithDependencies(context.Background(), stateRoot, ExecutionReconcileRequestV1{
				ID: fixture.record.ID, Confirm: true, Actor: fixture.actor, CWD: fixture.worktree,
			}, ExecutionReconcileDependenciesV1{RemotePR: RemotePullRequestDependenciesV1{
				Reconcile: func(string, port.IssueProviderReconcilePullRequestRequest) (port.IssueProviderReconcilePullRequestResult, error) {
...
func TestRemotePullRequestV1OneExactCandidateIsAdopted(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newRemoteExecutionFixtureV1(t, stateRoot, "69-remote-adopt")
	_, err := CreateRemotePullRequestV1(context.Background(), stateRoot, fixture.request("adopt exact"), RemotePullRequestDependenciesV1{
		Create: func(string, port.IssueProviderCreatePullRequestRequest) (port.IssueProviderCreatePullRequestResult, error) {
			return port.IssueProviderCreatePullRequestResult{}, errors.New("ambiguous transport")
		},
	})
	...
	payload, err := readExternalRemotePRPayloadV1(stateRoot, pending.Execution.Pending.OperationID)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemotePullRequestV1BlocksGenerationReplacementWhileIntentIsPending(t *testing.T) {
	stateRoot := t.TempDir()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, "69-remote-race")
	record := fixture.record
	record.IssueURL = "https://github.com/example/agent-harness/issues/69"
	record.Phase = model.IssueOpsPhasePR
	if _, err := writeIssueOps(stateRoot, record); err != nil {
	...
	owner := executionActorV1("codex", "remote-race-owner")
	if _, err := claimExecutionV1(stateRoot, ExecutionClaimRequestV1{
	...
	go func() {
		_, err := CreateRemotePullRequestV1(context.Background(), stateRoot, RemotePullRequestRequestV1{
	...
	replacer := executionActorV1("claude", "remote-race-replacer")
	...
	_, err = ReplaceExecutionV1(stateRoot, ExecutionReplaceRequestV1{
...
func newRemoteExecutionFixtureV1(t *testing.T, stateRoot, branch string) remoteExecutionFixtureV1 {
	t.Helper()
	fixture := newClaimableExecutionFixtureV1(t, stateRoot, branch)
	record := fixture.record
	record.IssueURL = "https://github.com/example/agent-harness/issues/69"
	record.Phase = model.IssueOpsPhasePR
	if _, err := writeIssueOps(stateRoot, record); err != nil {
	...
	actor := executionActorV1("codex", branch+"-session")
	if _, err := claimExecutionV1(stateRoot, ExecutionClaimRequestV1{
		ID: record.ID, Generation: 1, Actor: actor, CWD: fixture.worktree, TokenFile: fixture.tokenPath,
	}); err != nil {
		t.Fatal(err)
	...
	return remoteExecutionFixtureV1{claimableExecutionFixtureV1: fixture, actor: actor}
}

func (fixture remoteExecutionFixtureV1) request(title string) RemotePullRequestRequestV1 {
	return RemotePullRequestRequestV1{
