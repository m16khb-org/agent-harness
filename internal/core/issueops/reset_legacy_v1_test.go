...
func TestPreviewLegacyResetV1BuildsStableExactManifest(t *testing.T) {
	stateDir := t.TempDir()
	...
	v1Root := filepath.Join(stateDir, "issueops_v1")
	v1db, err := sqlstore.Open(v1Root)
	if err != nil {
		t.Fatal(err)
	}
	if err := v1db.Put("issueops_v1", "io-bbbbbbbbbbbb", []byte(`{"schema_version":1}`)); err != nil {
		t.Fatal(err)
	}
	...
	first, err := PreviewLegacyResetV1(stateDir, 1)
	if err != nil {
		t.Fatal(err)
	}
	second, err := PreviewLegacyResetV1(stateDir, 1)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, ok, err := v1db.Get("issueops_v1", "io-bbbbbbbbbbbb"); err != nil || !ok {
		t.Fatalf("v1 row changed: ok=%v err=%v", ok, err)
	}
}

func TestPreviewLegacyResetV1RejectsUnknownEntriesAndAuthority(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
	db, err := sqlstore.Open(legacyRoot)
	if err != nil {
...
func TestPreviewLegacyResetV1RejectsOrphanSQLiteFamily(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
...
func TestPreviewLegacyResetV1ProjectsOnlyTaskBearingOrcaAuthority(t *testing.T) {
...
func TestPreviewLegacyResetV1ProjectsSchemaAwareOrcaAuthority(t *testing.T) {
	tests := []struct {
		name        string
	...
	}{
		{
			...
			record: map[string]any{
				...
				"execution_handoff": legacyResetExecutionAuthorityFixtureV1("closed", "task-v7", "dispatch-v7"),
			},
		...
		},
		{
			...
			record: map[string]any{
				...
				"execution_handoff": legacyResetExecutionAuthorityFixtureV1("closed", "task-v8", "dispatch-v8"),
			},
				...
				"ownership": map[string]any{
					...
					"attempts": []any{
						map[string]any{
							...
							"workspace": legacyResetExecutionAuthorityFixtureV1("ready", "", ""),
							"handoff":   legacyResetExecutionAuthorityFixtureV1("closed", "task-old", "dispatch-old"),
						},
						map[string]any{
							"number": 2, "started_at": "2026-07-03T00:00:00Z",
							"workspace": legacyResetExecutionAuthorityFixtureV1("ready", "", ""),
							"handoff":   legacyResetExecutionAuthorityFixtureV1("owner_active", "task-current", "dispatch-current"),
						},
					},
				},
			},
		...
		},
		{
			...
			record: map[string]any{
				"schema_version": 9, "id": "io-aaaaaaaaaaaa", "phase": "implement", "cycle_state": "active",
				"execution_handoff": legacyResetExecutionAuthorityFixtureV1("closed", "task-top", "dispatch-top"),
				"ownership": map[string]any{
					"active_attempt": 1,
					"attempts": []any{map[string]any{
						"number": 1, "started_at": "2026-07-01T00:00:00Z",
						"workspace": legacyResetExecutionAuthorityFixtureV1("ready", "", ""),
						"handoff":   legacyResetExecutionAuthorityFixtureV1("owner_active", "task-nested", "dispatch-nested"),
					}},
				},
			},
		...
		},
		{
			...
			record: map[string]any{
				...
				"execution_handoff": legacyResetExecutionAuthorityFixtureV1("closed", "task-v10", "dispatch-v10"),
			},
...
func TestPreviewLegacyResetV1ClassifiesOnlyKnownPreSchemaRecords(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "external authority field is blocked", mutate: func(record map[string]any) {
			record["execution_handoff"] = legacyResetExecutionAuthorityFixtureV1("closed", "task-hidden", "dispatch-hidden")
...
func TestPreSchemaActiveCycleHasFiniteDrainPath(t *testing.T) {
	...
	if _, err := DrainLegacyCycleV1(stateDir, LegacyResetDrainCycleRequestV1{
		TargetSchema: 1, ExpectedFingerprint: preview.Fingerprint, LifecycleID: "io-aaaaaaaaaaaa", Confirm: true,
	}); err != nil {
		t.Fatal(err)
	}
	after, err := PreviewLegacyResetV1(stateDir, 1)
...
func legacyResetExecutionAuthorityFixtureV1(state, taskID, dispatchID string) map[string]any {
...
func TestConfirmLegacyResetV1UsesFingerprintAndPreservesUnrelatedState(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
	db, err := sqlstore.Open(legacyRoot)
	if err != nil {
	...
	existingV1 := []byte(`{"schema_version":1,"id":"io-bbbbbbbbbbbb"}`)
	if err := v1db.Put("issueops_v1", "io-bbbbbbbbbbbb", existingV1); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := confirmLegacyResetV1(stateDir, 1, strings.Repeat("0", 64), deps); err == nil || !strings.Contains(err.Error(), "stale") {
	...
	result, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, deps)
	if err != nil {
		t.Fatal(err)
	}
	...
	gotV1, ok, err := v1db.Get("issueops_v1", "io-bbbbbbbbbbbb")
	if err != nil || !ok || string(gotV1) != string(existingV1) {
		t.Fatalf("existing v1 row changed: ok=%v data=%q err=%v", ok, gotV1, err)
	}
	marker, ok, err := v1db.Get(issueOpsMetaV1Bucket, issueOpsSchemaMarkerV1ID)
	...
	retry, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, deps)
	if err != nil {
...
func TestConfirmLegacyResetV1ResumesAfterRowsDeleted(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
	db, err := sqlstore.Open(legacyRoot)
	if err != nil {
	...
	if _, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, deps); !errors.Is(err, crash) {
	...
	result, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, resetLegacyV1TestDeps())
	if err != nil {
...
func TestConfirmLegacyResetV1ResumesAfterEveryCrashBoundary(t *testing.T) {
	discovery := newLegacyResetCrashFixtureV1(t)
	...
	for _, target := range stages {
		t.Run(strings.ReplaceAll(target, ":", "_"), func(t *testing.T) {
			fixture := newLegacyResetCrashFixtureV1(t)
			...
			if _, err := confirmLegacyResetV1(fixture.stateDir, 1, fixture.preview.Fingerprint, deps); !errors.Is(err, crash) {
			...
			resume := resetLegacyV1TestDeps()
			resume.AfterStep = func(step string) error {
				stageCalls[step]++
				return nil
			}
			result, err := confirmLegacyResetV1(fixture.stateDir, 1, fixture.preview.Fingerprint, resume)
			...
			after, err := PreviewLegacyResetV1(fixture.stateDir, 1)
			...
			v1db, err := sqlstore.Open(filepath.Join(fixture.stateDir, issueOpsV1Directory))
			if err != nil {
				t.Fatal(err)
			}
			if got, ok, err := v1db.Get("issueops_v1", "io-bbbbbbbbbbbb"); err != nil || !ok || !slices.Equal(got, fixture.v1Bytes) {
				t.Fatalf("unrelated v1 row changed: got=%q ok=%v err=%v", got, ok, err)
			}
			metaIDs, err := v1db.List(issueOpsMetaV1Bucket)
			if err != nil || !slices.Equal(metaIDs, []string{issueOpsSchemaMarkerV1ID}) {
				t.Fatalf("schema marker rows=%#v err=%v", metaIDs, err)
			}
			control, err := sqlstore.Open(filepath.Join(fixture.stateDir, issueOpsResetV1Directory))
			if err != nil {
				t.Fatal(err)
			}
			controlIDs, err := control.List(issueOpsResetV1Bucket)
			if err != nil || !slices.Equal(controlIDs, []string{issueOpsResetV1ReceiptID}) {
...
type legacyResetCrashFixtureV1 struct {
	stateDir       string
...
func newLegacyResetCrashFixtureV1(t *testing.T) legacyResetCrashFixtureV1 {
	t.Helper()
	stateDir := t.TempDir()
	...
	v1Bytes := []byte(`{"schema_version":1,"id":"io-bbbbbbbbbbbb"}`)
	v1db, err := sqlstore.Open(filepath.Join(stateDir, issueOpsV1Directory))
	if err != nil {
		t.Fatal(err)
	}
	if err := v1db.Put("issueops_v1", "io-bbbbbbbbbbbb", v1Bytes); err != nil {
		t.Fatal(err)
	}
	preview, err := PreviewLegacyResetV1(stateDir, 1)
	if err != nil {
	...
	manifest, exists, err := buildLegacyResetManifestV1(stateDir, 1)
	if err != nil || !exists {
		t.Fatalf("crash fixture manifest exists=%v err=%v", exists, err)
	}
	return legacyResetCrashFixtureV1{
		stateDir: stateDir, preview: preview, entryCount: len(orderLegacyResetDeletionEntriesV1(manifest.Entries)),
		unrelatedPath: unrelatedPath, unrelatedBytes: unrelatedBytes, v1Bytes: v1Bytes,
	}
}

func TestConfirmLegacyResetV1NeverWidensPreviewFileManifest(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
	db, err := sqlstore.Open(legacyRoot)
	if err != nil {
	...
	if _, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, deps); err == nil || !strings.Contains(err.Error(), "not in the sealed preview manifest") {
...
func TestConfirmLegacyResetV1RejectsRootReplacementDuringResume(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
	db, err := sqlstore.Open(legacyRoot)
	if err != nil {
	...
	if _, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, resetLegacyV1TestDeps()); err == nil || !strings.Contains(err.Error(), "identity") {
...
func TestCompletedLegacyResetReceiptDoesNotMaskReappearedState(t *testing.T) {
	...
	if _, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, resetLegacyV1TestDeps()); err != nil {
		t.Fatal(err)
	}
	writeResetTestFile(t, legacyRoot, "io-bbbbbbbbbbbb.json", []byte(`{"schema_version":9,"id":"io-bbbbbbbbbbbb","phase":"done","cycle_state":"closed"}`))

	if _, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, resetLegacyV1TestDeps()); err == nil || !strings.Contains(err.Error(), "reappeared") {
...
func TestConfirmLegacyResetV1RechecksProcessesInsideControlLock(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
	db, err := sqlstore.Open(legacyRoot)
	if err != nil {
	...
	calls := 0
	deps := resetLegacyV1TestDeps()
	deps.LiveProcesses = func(string) ([]resetLegacyV1Process, error) {
		calls++
		if calls == 1 {
			return nil, nil
		}
		return []resetLegacyV1Process{{PID: 42, StartedAt: "start", Executable: "agent-harness"}}, nil
	}
	if _, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, deps); err == nil || !strings.Contains(err.Error(), "pid=42") {
...
func TestConfirmLegacyResetV1RechecksProcessesImmediatelyBeforeMutation(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
	db, err := sqlstore.Open(legacyRoot)
	if err != nil {
	...
	deps.LiveProcesses = func(string) ([]resetLegacyV1Process, error) {
		...
		return []resetLegacyV1Process{{PID: 84, StartedAt: "start", Executable: "/opt/agent-harness", Kind: "worker"}}, nil
	}
	if _, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, deps); err == nil || !strings.Contains(err.Error(), "pid=84") {
...
func TestLiveHarnessProcessesV1FindsDirectAndResidentMCPWithoutSubstringFalsePositives(t *testing.T) {
	...
	processes, err := liveHarnessProcessesFromSnapshotV1(stateDir, []byte(output), 105)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLiveHarnessProcessesV1UsesRegisteredDaemonPIDStartExecutableIdentity(t *testing.T) {
	t.Setenv("HARNESS_DAEMON_DIR", "")
	stateDir := t.TempDir()
	...
	processes, err := liveHarnessProcessesFromSnapshotV1(stateDir, output, 999)
	if err != nil {
		t.Fatal(err)
	}
...
func TestConfirmLegacyResetV1BindsCrashResumeToExactStagedBinary(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
	db, err := sqlstore.Open(legacyRoot)
	if err != nil {
	...
	resume := resetLegacyV1TestDeps()
	resume.ActiveBinary = func() (resetLegacyV1BinaryIdentity, error) {
	...
	if _, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, resume); err == nil || !strings.Contains(err.Error(), "staged binary") {
...
func TestReconcileLegacyRemoteClaimV1RequiresExactlyOneVerifiedCandidate(t *testing.T) {
	for _, tc := range []struct {
		name         string
		...
		{name: "one", result: port.IssueProviderReconcilePullRequestResult{Candidates: []port.IssueProviderReconcilePullRequestCandidate{legacyResetRemoteCandidateV1()}}, wantOK: true},
		{name: "multiple", result: port.IssueProviderReconcilePullRequestResult{Candidates: []port.IssueProviderReconcilePullRequestCandidate{legacyResetRemoteCandidateV1(), legacyResetRemoteCandidateV1()}}},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			record := legacyResetRemoteRecordV1(t)
			if err := db.Put("issueops", "io-aaaaaaaaaaaa", record); err != nil {
				t.Fatal(err)
			}
			preview, err := PreviewLegacyResetV1(stateDir, 1)
			...
			result, reconcileErr := ReconcileLegacyRemoteClaimV1(context.Background(), stateDir, LegacyResetRemoteReconcileRequestV1{
				TargetSchema: 1, ExpectedFingerprint: preview.Fingerprint,
				LifecycleID: "io-aaaaaaaaaaaa", ClaimID: "claim-1", Confirm: true,
			}, LegacyResetRemoteDependenciesV1{
			...
			if tc.wantOK {
				...
				drained, err := DrainLegacyCycleV1(stateDir, LegacyResetDrainCycleRequestV1{
					TargetSchema: 1, ExpectedFingerprint: preview.Fingerprint, LifecycleID: "io-aaaaaaaaaaaa", Confirm: true,
				})
				...
				ids, err := control.List(issueOpsResetV1Bucket)
				if err != nil || !slices.Equal(ids, []string{issueOpsResetV1ReceiptID}) {
...
func legacyResetRemoteRecordV1(t *testing.T) []byte {
	t.Helper()
	record := map[string]any{
		"schema_version": 9,
		"id":             "io-aaaaaaaaaaaa",
...
func legacyResetRemoteCandidateV1() port.IssueProviderReconcilePullRequestCandidate {
...
func TestReconcileLegacyOrcaTaskV1RequiresAuthoritativeQuiescence(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := db.Put("issueops", "io-aaaaaaaaaaaa", legacyResetOrcaRecordV1(t)); err != nil {
				t.Fatal(err)
			}
			preview, err := PreviewLegacyResetV1(stateDir, 1)
			if err != nil {
			...
			deps := LegacyResetOrcaDependenciesV1{
			...
			result, reconcileErr := ReconcileLegacyOrcaTaskV1(context.Background(), stateDir, LegacyResetOrcaReconcileRequestV1{
			...
			if tc.wantOK {
				...
				drained, err := DrainLegacyCycleWithOrcaV1(context.Background(), stateDir, LegacyResetDrainCycleRequestV1{
					TargetSchema: 1, ExpectedFingerprint: preview.Fingerprint, LifecycleID: "io-aaaaaaaaaaaa", Confirm: true,
				}, deps)
				if err != nil || !drained.Drained {
					t.Fatalf("drain result=%#v err=%v", drained, err)
...
func TestDrainLegacyCycleV1RevalidatesMutableOrcaReceipt(t *testing.T) {
	stateDir := t.TempDir()
	db, err := sqlstore.Open(filepath.Join(stateDir, "issueops"))
	if err != nil {
		t.Fatal(err)
	...
	if _, err := ReconcileLegacyOrcaTaskV1(context.Background(), stateDir, LegacyResetOrcaReconcileRequestV1{
		TargetSchema: 1, ExpectedFingerprint: preview.Fingerprint, LifecycleID: "io-aaaaaaaaaaaa",
		RuntimeID: "runtime-1", TaskID: "task-1", DispatchID: "dispatch-1", Confirm: true,
	}, deps); err != nil {
		t.Fatal(err)
	}
	if _, err := DrainLegacyCycleWithOrcaV1(context.Background(), stateDir, LegacyResetDrainCycleRequestV1{
		TargetSchema: 1, ExpectedFingerprint: preview.Fingerprint, LifecycleID: "io-aaaaaaaaaaaa", Confirm: true,
	}, deps); err != nil {
		t.Fatal(err)
	}
...
func TestConfirmLegacyResetV1RevalidatesMutableOrcaReceiptBeforeDeletion(t *testing.T) {
	stateDir := t.TempDir()
	db, err := sqlstore.Open(filepath.Join(stateDir, "issueops"))
	if err != nil {
		t.Fatal(err)
	...
	orcaDeps := LegacyResetOrcaDependenciesV1{
		Status: func(context.Context) (port.OrcaStatus, error) {
			return port.OrcaStatus{RuntimeID: "runtime-1", RuntimeReachable: true, RuntimeState: "ready", GraphState: "ready"}, nil
		},
		ListTasks: func(context.Context) ([]port.OrcaTask, error) {
	...
	taskStatus = "dispatched"
	confirmDeps := resetLegacyV1TestDeps()
	confirmDeps.Orca = &orcaDeps
	if _, err := confirmLegacyResetV1(stateDir, 1, preview.Fingerprint, confirmDeps); err == nil || !strings.Contains(err.Error(), "still live") {
...
func legacyResetOrcaRecordV1(t *testing.T) []byte {
	t.Helper()
	raw, err := json.Marshal(map[string]any{
		...
		"ownership": map[string]any{
			...
			"attempts": []any{map[string]any{
				...
				"workspace":  legacyResetExecutionAuthorityFixtureV1("ready", "", ""),
				"handoff":    legacyResetExecutionAuthorityFixtureV1("ownership_dispatched", "task-1", "dispatch-1"),
			}},
		},
	})
	if err != nil {
...
func TestIssueOpsV1MutationBarrierReturnsResetRequired(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
	writeResetTestFile(t, legacyRoot, "io-aaaaaaaaaaaa.json", []byte(`{"schema_version":9,"id":"io-aaaaaaaaaaaa"}`))
	v1Root := filepath.Join(stateDir, "issueops_v1")

	err := RequireIssueOpsV1MutationAllowed(v1Root)
	...
	if _, err := WriteIssueOps(v1Root, IssueOpsRecord{SchemaVersion: 1, ID: "io-bbbbbbbbbbbb"}); !errors.As(err, &resetErr) {
		t.Fatalf("write barrier error = %v", err)
	}
	if _, err := os.Stat(v1Root); !os.IsNotExist(err) {
		t.Fatalf("blocked write created v1 state: %v", err)
	}
}

func TestIssueOpsV1MutationBarrierRunsBeforeExternalDependencies(t *testing.T) {
	stateDir := t.TempDir()
	v1Root := filepath.Join(stateDir, "issueops_v1")
	...
	_, prepareErr := PrepareExecutionV1(context.Background(), v1Root, ExecutionPrepareRequestV1{
		ID: "io-bbbbbbbbbbbb", Mode: "direct", Confirm: true,
	}, ExecutionPrepareDependenciesV1{Direct: direct})
	var resetErr *ResetRequiredError
	...
	providerCalls := 0
	_, remoteErr := CreateRemotePullRequestV1(context.Background(), v1Root, RemotePullRequestRequestV1{
		ID: "io-bbbbbbbbbbbb", Confirm: true,
	}, RemotePullRequestDependenciesV1{Create: func(string, port.IssueProviderCreatePullRequestRequest) (port.IssueProviderCreatePullRequestResult, error) {
		providerCalls++
	...
	_, reconcileErr := ReconcileExecutionV1WithDependencies(context.Background(), v1Root, ExecutionReconcileRequestV1{
		ID: "io-bbbbbbbbbbbb", Confirm: true, Actor: model.NativeActorV1{},
	}, ExecutionReconcileDependenciesV1{})
	if !errors.As(reconcileErr, &resetErr) {
		t.Fatalf("reconcile barrier error = %v", reconcileErr)
	}
}

func TestIssueOpsV1MutationBarrierStaysClosedDuringCrashResume(t *testing.T) {
	stateDir := t.TempDir()
	legacyRoot := filepath.Join(stateDir, "issueops")
	db, err := sqlstore.Open(legacyRoot)
	if err != nil {
	...
	v1Root := filepath.Join(stateDir, "issueops_v1")
	_, err = WriteIssueOps(v1Root, IssueOpsRecord{SchemaVersion: 1, ID: "io-bbbbbbbbbbbb"})
	var resetErr *ResetRequiredError
...
func resetLegacyV1TestDeps() resetLegacyV1Deps {
	return resetLegacyV1Deps{
		...
		LiveProcesses:     func(string) ([]resetLegacyV1Process, error) { return nil, nil },
		RequireActivation: func(*sqlstore.DB, string, int, resetLegacyV1BinaryIdentity) error { return nil },
	}
}
