...
func TestOwnershipCleanupPreviewIsReadOnlyAndFreshSourceMayApprove(t *testing.T) {
	...
	if err != nil || retainedCleanupHandoff(unchanged).State != handoff.StateCleanupPendingHumanDecision || retainedCleanupHandoff(unchanged).Cleanup != nil {
		t.Fatalf("preview mutated cleanup authority: %#v err=%v", retainedCleanupHandoff(unchanged), err)
	}
	...
	if retainedCleanupHandoff(approved).State != handoff.StateCleanupExecuting || retainedCleanupHandoff(approved).Cleanup == nil || retainedCleanupHandoff(approved).Cleanup.ApprovedBySession == nil || retainedCleanupHandoff(approved).Cleanup.ApprovedBySession.SessionID != "fresh-source" {
		t.Fatalf("human approval did not atomically select source cleanup session: %#v", retainedCleanupHandoff(approved))
	}
}

func TestOwnershipCleanupRejectsCompletedOwnerAndStalePreview(t *testing.T) {
	stateRoot, record := completedOwnershipCleanupRecord(t)
	owner := retainedCleanupHandoff(record).OwnerSession
...
func TestOwnershipCleanupRecordsOnlyApprovedOrderedReceipts(t *testing.T) {
	...
	if retainedCleanupHandoff(updated).State != handoff.StateCleanupExecuting || len(retainedCleanupHandoff(updated).Cleanup.Receipts) != 1 || retainedCleanupHandoff(updated).Cleanup.Receipts[0].Step != "remote_head_safe" {
		t.Fatalf("first human-approved cleanup receipt = %#v", retainedCleanupHandoff(updated))
	}
	other := candidate
