...
func handoffOwnershipBlockReason(req HookToolUseLifecycleRequest) (bool, string) {
	...
	if searchrouting.IsShellTool(req.Tool) {
		if exactReadOnlyShellCommand(req, record) || allowedClosedOrcaCleanup(req, record) || allowedCancellationTaskTerminalization(req, record) || allowedClosedCancelledWorktreeRemoval(req, record) {
			return true, ""
		}
