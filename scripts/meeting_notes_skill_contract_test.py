...
SKILL = ROOT / "skills" / "meeting-notes" / "SKILL.md"
OPENAI = ROOT / "skills" / "meeting-notes" / "agents" / "openai.yaml"
GITIGNORE = ROOT / ".gitignore"
LOCAL_BACKGROUND = ROOT / "skills" / "meeting-notes" / "background.local.md"
TESTDATA = ROOT / "skills" / "meeting-notes" / "testdata"
HANDOFF = TESTDATA / "ai_devops_onboarding_handoff.md"
BAD_HANDOFF = TESTDATA / "ai_devops_onboarding_handoff.bad.md"
RUBRIC = ROOT / "scripts" / "meeting_notes_quality_rubric.py"
PUBLISH_SCRIPT = ROOT / "skills" / "meeting-notes" / "scripts" / "publish_meeting_canvas.py"


class MeetingNotesSkillContractTest(unittest.TestCase):
    def read_skill(self) -> str:
        self.assertTrue(SKILL.exists(), "skills/meeting-notes/SKILL.md must exist")
    ...
    def test_skill_is_named_after_engelbart(self) -> None:
        ...
        self.assertIn("name: meeting-notes", content)
        self.assertIn("# Meeting Notes", content)
        self.assertNotIn(old_slug, content)
        self.assertNotIn(old_display_name, content)

        self.assertTrue(OPENAI.exists(), "skills/meeting-notes/agents/openai.yaml must exist")
        metadata = OPENAI.read_text(encoding="utf-8")
        self.assertIn('display_name: "Meeting Notes"', metadata)
        self.assertIn("$meeting-notes", metadata)
    ...
    def test_local_background_file_is_configured_for_term_and_speaker_resolution(self) -> None:
        ...
        self.assertIn("skills/meeting-notes/background.local.md", content)
        self.assertIn("skills/meeting-notes/background.local.example.md", content)
    ...
    def test_required_meeting_inputs_are_requested_sequentially(self) -> None:
        ...
        self.assertIn("When both inputs are present, continue with the normal Meeting Notes workflow", content)
    ...
    def test_forward_quality_rubric_is_documented(self) -> None:
        ...
        self.assertIn("python3 scripts/meeting_notes_quality_rubric.py", content)
