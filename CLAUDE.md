...
- 철학: **바퀴를 재발명하지 않는다**. LLM Wiki, CodeGraph, agentmemory 같은 전문 기능은 하네스가 재구현하지 않고 upstream plugin/CLI를 사용한다. 필요하면 `./scripts/install-native.sh --with-upstream-tools`로 함께 설치/갱신한다.
