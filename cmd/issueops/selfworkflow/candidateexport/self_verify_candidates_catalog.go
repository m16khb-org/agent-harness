...
func SelfVerificationCandidateCatalog() []SelfVerificationCandidate {
	return []SelfVerificationCandidate{
		...
		selfVerificationCandidate(8, "self-verify-install-dry-run-smoke", "native integration", 76, selfAugmentCandidateStatusSatisfied, []string{"install --dry-run should be an independent no-write evidence label"}, []string{"temp HOME smoke", "dry-run no-write assertion"}, []string{"install dry-run smoke runs install --dry-run against temp HOME/CODEX_HOME/ISSUEOPS_ROOT and asserts planned writes/links without filesystem mutations"}),
