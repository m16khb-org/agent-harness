...
func SelfVerify(request Request, deps Deps) (model.SelfAugmentResult, error) {
	...
	started := time.Now()
	result := loopresult.New(iterations, request.BaseSeed, request.TargetScore, deps.IssueOpsRoot())
	...
	tempDir, err := os.MkdirTemp("", "issueops-self-verify-*")
	if err != nil {
	...
	tempBin := filepath.Join(tempDir, "issueops")
