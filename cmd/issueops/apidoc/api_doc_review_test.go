...
func TestAPIDocReviewRendersPromptWithoutSpawning(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, ".issueops", "OPEN_API_SPEC.md"), []byte("repo-specific api contract rules\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
---
`
	if err := os.WriteFile(filepath.Join(root, ".issueops", "OPEN_API_SPEC.md"), []byte(spec), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestAPIDocReviewExtraPromptUsesExplicitPromptFileFirst(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, ".issueops", "OPEN_API_SPEC.md"), []byte("repo spec\n"), 0o644); err != nil {
		t.Fatal(err)
	}
