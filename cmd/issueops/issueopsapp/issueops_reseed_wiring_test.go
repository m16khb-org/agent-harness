...
func TestIssueOpsAppReseedWiringUsesVerticalRepository(t *testing.T) {
...
func TestExecutionReseedCompletedStatusExposesReopenContract(t *testing.T) {
	...
	record.Execution.Completion = &model.ExecutionCompletion{Generation: 1, FinalHead: oldHead, VerificationReportPath: ".issueops/verified-execution/old.json", Verification: []string{"old verification"}, RemoteArtifactURL: "https://gitlab.example.com/acme/repo/-/merge_requests/1", CompletedAt: "2026-08-03T00:00:00Z"}
...
func TestCompletedReplacementPreviewRequiresSyncBaseBeforeReseedWhenParentDrifted(t *testing.T) {
	...
	if next != "issueops execution sync-base --id '"+record.ID+"' --preview --completion-generation 1 --json" || strings.Contains(next, "--reseed") {
