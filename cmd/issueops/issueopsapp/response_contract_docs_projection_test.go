...
func TestDocsIndexContractProjectionKeepsShapeAndRequiredDocsOnly(t *testing.T) {
	input := map[string]any{
		...
		"docs": []any{
			...
			map[string]any{"rel_path": ".issueops/CONSTITUTION.md", "title": "old", "bytes": float64(3)},
			map[string]any{"rel_path": ".issueops/ARCHITECTURE.md", "title": "old", "bytes": float64(4)},
			map[string]any{"rel_path": ".issueops/CONVENTIONS.md", "title": "old", "bytes": float64(5)},
			map[string]any{"rel_path": ".issueops/TESTING.md", "title": "old", "bytes": float64(6)},
			map[string]any{"rel_path": ".issueops/CAUTIONS.md", "title": "old", "bytes": float64(7)},
			map[string]any{"rel_path": ".issueops/ADR.md", "title": "old", "bytes": float64(8)},
			map[string]any{"rel_path": ".issueops/OPERATIONS.md", "title": "old", "bytes": float64(9)},
			map[string]any{"rel_path": ".issueops/AGENT_WORKFLOW.md", "title": "old", "bytes": float64(10)},
	...
	if got["ok"] != true || got["version"] != "0.1.0" || got["issueops_root"] != "$ISSUEOPS_ROOT" {
	...
	if !required["AGENTS.md"] || !required[".issueops/TESTING.md"] || !required["skills/self-augment/SELF_AUGMENTATION.md"] {
...
func TestInspectContractProjectionKeepsOnlyRequiredDocs(t *testing.T) {
	input := map[string]any{
		"docs": []any{
			"$ISSUEOPS_ROOT/.issueops/CONSTITUTION.md",
			"$ISSUEOPS_ROOT/.issueops/plans/transient.md",
			"$ISSUEOPS_ROOT/AGENTS.md",
		},
		"version": "0.1.0",
	}
	want := map[string]any{
	...
	want := map[string]any{
		"content": []any{
			map[string]any{
				"type": "text",
				"json": map[string]any{
					"docs": []any{"AGENTS.md", ".issueops/CONSTITUTION.md"},
				},
			},
		},
	}
