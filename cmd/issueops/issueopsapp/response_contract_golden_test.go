...
func TestResponseContractsGolden(t *testing.T) {
	...
	replacements[executable] = "$ISSUEOPS_EXECUTABLE"
	replacements[hex.EncodeToString(executableHash[:])] = "$ISSUEOPS_EXECUTABLE_SHA256"
	...
	addEvalSymlinkReplacement(t, replacements, issueOpsRoot(), "$ISSUEOPS_ROOT")
...
func assertGolden(t *testing.T, name string, got []byte) {
	...
	if err != nil {
		t.Fatalf("read golden %s: %v (run go test ./cmd/issueops/issueopsapp -run Golden -update)", path, err)
	}
