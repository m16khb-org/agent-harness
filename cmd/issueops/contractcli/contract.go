...
func contractUsage() {
	fmt.Fprintf(os.Stderr, `Usage:
	  ...
	  issueops contract conformance baseline|live|replay|serve [flags]
`)
}
...
func BuildCompatibilityContract() CompatibilityContract {
	...
	contract := CompatibilityContract{
		...
		ResponseFields: map[string][]string{
			...
			"issueops_record":                 {"ok", "schema_version", "id", "repo", "branch", "phase", "intent", "design_review", "domain_review", "issue_url", "issue_create_intent", "plan_path", "worktree_path", "issue_links", "branch_prepare", "remote_artifact", "decisions", "plan_prep", "compatibility_review", "devils_advocate_review", "feedback", "regress_events", "delegation", "child_cycles", "execution", "routing_trace", "ai_slop_clean_at", "ai_slop_clean_head", "ai_slop_clean_fingerprint", "ai_slop_clean_categories", "ai_slop_clean_verification", "phase_ledger", "created_at", "updated_at"},
			"issueops_execution":              {"ok", "id", "execution", "issue_snapshot_source", "next_command"},
			"issueops_execution_prepare":      {"ok", "id", "preview", "requested_mode", "resolved_mode", "fallback_code", "probe_attempted", "probe_available", "probe_ready", "probe_code", "readiness_fingerprint", "explicit_direct_reason", "workspace", "execution", "claim_token_path", "issue_body_sha256", "context_packet_path", "context_packet_sha256", "owner_prompt_path", "owner_prompt_sha256", "issue_snapshot_source", "next_command"},
			...
			"issueops_next":                   {"ok", "generated_at", "cwd", "cwd_role", "source_root", "stage", "lease", "missing", "next_command", "next_command_kind", "exits", "review", "warnings"},
			...
			"issueops_remote_reconcile_issue": {"ok", "candidate_count", "candidate_url", "would_adopt", "issue_url", "issue_create_intent"},
