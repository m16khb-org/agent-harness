...
func TestRunInspect_printsText_whenRepoIsPositional(t *testing.T) {
	...
	// 검증
	if !strings.Contains(out, "issueops root:") || !strings.Contains(out, "target repo: "+repo) {
...
func TestRunDoctor_printsHealthyText_whenProjectDocsAreInitialized(t *testing.T) {
	...
	// 검증
	// doctor는 경고를 낼 수 있지만(예: bin/issueops가 소스 파일보다 오래되면
	// binary_drift) 깨끗한 테스트 repo에 대해서는 healthy 검사를 통과해야 한다.
	if !strings.Contains(out, "issueops doctor") || !strings.Contains(out, repo) {
...
func TestRunDoctor_printsIssuesText_whenProjectDocsAreMissing(t *testing.T) {
	...
	// 검증
	if !strings.Contains(out, "issueops doctor found") || !strings.Contains(out, "project_docs_missing") {
		t.Fatalf("unexpected unhealthy doctor text:\n%s", out)
	}
	if !strings.Contains(out, "fix: issueops project bootstrap --repo") {
