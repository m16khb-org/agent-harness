...
func TestRunDaemonRejectsMissingAndUnknownSubcommands(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(stderr, "issueops daemon start") || !strings.Contains(stderr, "issueops daemon stop") {
...
func TestRunDaemonStatusReportsStoppedTextAndJSON(t *testing.T) {
	root := t.TempDir()
	t.Setenv("ISSUEOPS_DAEMON_DIR", root)

	text := captureStatusVerifyStdout(t, func() error {
		return runDaemon([]string{"status"})
	})
	if !strings.Contains(text, "stopped socket="+filepath.Join(root, "issueops.sock")) {
...
func TestRunDaemonStartReportsExistingDaemonTextAndJSON(t *testing.T) {
	...
	if !strings.Contains(text, "issueops daemon running pid="+strconv.Itoa(os.Getpid())) || !strings.Contains(text, "socket="+paths.Socket) {
...
func TestRunDaemonStopReportsAlreadyStopped(t *testing.T) {
	...
	if !strings.Contains(text, "issueops daemon already stopped") {
	...
	if !status.OK || status.Running || status.Code != daemonStatusStopped || status.Message != "issueops daemon already stopped" {
...
func TestDaemonStopCleansStalePIDFileIdempotently(t *testing.T) {
	root := t.TempDir()
	pidPath := filepath.Join(root, "issueops.pid")
	// Stale state: pid file references a dead PID, socket absent.
	if err := os.WriteFile(pidPath, []byte(`{"pid":99999,"process_start_time":"2026-08-20T13:27:20Z","executable":"/bin/issueops","instance_nonce":"n","build_sha":"s","protocol_version":"1","generation":"g"}`), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	status, err := stopDaemonWithDeps(daemonStopDeps{
		checkStatus: func() daemonStatus {
			return daemonStatus{
				...
				PID:   99999,
				Paths: daemonPaths{Dir: root, Socket: filepath.Join(root, "issueops.sock"), PID: pidPath},
			}
		},
