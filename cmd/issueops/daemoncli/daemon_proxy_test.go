...
func TestRunMCPProxyWithDepsReconnectsWithoutReplayingInterruptedRequest(t *testing.T) {
	...
	go func() {
		...
		if _, err := io.WriteString(secondDaemon, `{"jsonrpc":"2.0","id":"issueops-reconnect-1","result":{"protocolVersion":"2025-06-18","capabilities":{"resources":{"listChanged":true},"tools":{"listChanged":true}}}}`+"\n"); err != nil {
			secondReady <- err
			return
		}
		initialized, err := reader.ReadString('\n')
