...
func stopDaemonWithDeps(deps daemonStopDeps) (daemonStatus, error) {
	...
	if status.Code == daemonStatusStopped && !status.Running && !status.Reachable && status.PID > 0 && !deps.processAlive(status.PID) {
		return daemonStoppedStatus(status, status.PID, deps.remove, "issueops daemon already stopped (cleaned stale pid file)"), nil
	}
	...
	for deps.now().Before(deadline) {
		if !deps.processAlive(instance.PID) {
			return daemonStoppedStatus(status, instance.PID, deps.remove, "issueops daemon stopped"), nil
		}
	...
	return daemonStoppedStatus(status, instance.PID, deps.remove, "issueops daemon killed after timeout"), nil
}
