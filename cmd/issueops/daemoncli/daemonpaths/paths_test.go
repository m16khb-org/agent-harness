...
func TestCurrentUsesDaemonDirEnv(t *testing.T) {
	...
	if paths.Socket != filepath.Join(paths.Dir, "issueops.sock") ||
		paths.PID != filepath.Join(paths.Dir, "issueops.pid") ||
		paths.Lock != filepath.Join(paths.Dir, "issueops.lock") ||
		paths.Log != filepath.Join(paths.Dir, "issueops.log") {
...
func TestReadPIDAndProcessAliveBoundaries(t *testing.T) {
	...
	if err := WriteInstance(path, InstanceRecord{
		PID: 123, ProcessStartTime: "start-a", Executable: "/tmp/issueops",
