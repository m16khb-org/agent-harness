...
func TestRunIssueOpsBenchmarkCompareAndGateTextBranches(t *testing.T) {
	...
	candidatePath := writeIssueOpsCandidateForCLITest(t, issueopscore.IssueOpsAutoresearchCandidate{
		...
		EditSurface:      []string{"cmd/issueops/**"},
	})
	...
	gateOut := captureStdoutForContract(t, func() error {
		return runIssueOps([]string{"benchmark", "gate", "--baseline", "baseline", "--candidate", "candidate", "--candidate-file", candidatePath, "--changed-path", "cmd/issueops/issueops.go"})
	})
...
func TestRunIssueOpsBenchmarkUsageAndErrorBranches(t *testing.T) {
	...
	if !strings.Contains(usage, "Usage: issueops benchmark run|compare|gate|reliability|consensus [--json]") {
