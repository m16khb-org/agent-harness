...
func TestRunIssueOpsUsageAndCleanupBranches(t *testing.T) {
	...
	for _, want := range []string{
		...
		"issueops cleanup orphan --id ID --repo ROOT --worktree PATH --branch NAME",
	} {
	...
	if !strings.Contains(cleanupUsage, "Usage: issueops cleanup status --id ID [--merged] [--json]") {
		t.Fatalf("cleanup usage missing status syntax:\n%s", cleanupUsage)
	}
	if !strings.Contains(cleanupUsage, "issueops cleanup orphan --id ID --repo ROOT --worktree PATH --branch NAME") {
