...
func TestRunCatalogHooksWithInjectedPrinter(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(repo, ".issueops", "CONSTITUTION.md"), []byte("# Constitution\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	var printed []any
	config := Config{
...
func TestRunCatalogHooksFormatsHostOutputs(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(repo, ".issueops", "AGENT_WORKFLOW.md"), []byte("# Agent Workflow\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	var printed []any
	config := Config{
