...
func TestValidateCommandPolicyWithDepsCoversSuccessAndSetupFailure(t *testing.T) {
	...
	if !step.OK || len(calls) != 5 || !strings.Contains(step.Command, "policy allow:bin/issueops policy check") || !strings.Contains(step.Command, "policy fake-run:bin/issueops policy fake-run") {
...
func writeCommandPolicyFakeBinary(t *testing.T, dir string) string {
	...
	body := "#!/bin/sh\nset -eu\ncase \"$*\" in\n" +
		"  policy\\ check*issueops-policy-outside-*\" -- git status --short\") printf '%s\\n' '" + cwdOutside + "' ;;\n" +
		"  policy\\ check*\" -- cat \"*issueops-policy-outside-*) printf '%s\\n' '" + pathOutside + "' ;;\n" +
