...
func docsIndexSmokeValidationErrors(index docs.DocsIndexResult, root string) []string {
	...
	if index.IssueOpsRoot != root {
		errs = append(errs, "docs index harness root mismatch")
	}
	if len(index.Docs) == 0 {
		errs = append(errs, "no docs indexed")
	}
	wantDocs := []string{"AGENTS.md", "CLAUDE.md", "GENIUS_THINK.md", ".issueops/COMMIT_POLICY.md", "skills/self-augment/SELF_AUGMENTATION.md", "skills/self-verify/SKILL.md", ".issueops/OPERATIONS.md"}
