...
func TestValidateRedactionAuditWithDepsCoversSuccessSecretAndReadFailure(t *testing.T) {
	...
	fixture := filepath.Join(root, "cmd", "issueops", "testdata", "usage.golden.txt")
	...
	deps := docsValidationDeps{
		...
		glob: func(pattern string) ([]string, error) {
			if strings.Contains(pattern, filepath.Join("cmd", "issueops", "testdata")) {
	...
	if step.OK || !strings.Contains(step.Error, "cmd/issueops/testdata/usage.golden.txt: line 1 contains openai_token") {
...
func TestValidateQAGateWithDepsCoversHealthyAndAggregateFailures(t *testing.T) {
	...
	testingDoc := filepath.Join(root, ".issueops", "TESTING.md")
	testingModule := filepath.Join(root, ".issueops", "testing", "unit-and-contract.md")
