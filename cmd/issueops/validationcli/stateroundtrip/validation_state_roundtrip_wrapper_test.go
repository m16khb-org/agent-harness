...
content="` + contentJSON + `"
list_count_file="$ISSUEOPS_STATE_DIR/.list-count"
history_count_file="$ISSUEOPS_STATE_DIR/.history-count"
case "$*" in
  state\ write*)
    case "$*" in
