...
func TestIssueOpsRootPrefersEnvironmentAndReadHarnessFile(t *testing.T) {
	root := t.TempDir()
	t.Setenv("ISSUEOPS_ROOT", root)
	if got := IssueOpsRoot("missing"); got != root {
		t.Fatalf("IssueOpsRoot env = %q, want %q", got, root)
	}
...
func TestIssueOpsRootFromFollowsExecutableSymlink(t *testing.T) {
	root := t.TempDir()
	marker := filepath.Join("skills", "atomic-commit-push", "SKILL.md")
	markerPath := filepath.Join(root, marker)
	if err := os.MkdirAll(filepath.Dir(markerPath), 0o755); err != nil {
	...
	got := issueOpsRootFrom(marker, "", t.TempDir(), short)
	if got != resolvedRoot {
		t.Fatalf("issueOpsRootFrom symlink = %q, want %q", got, resolvedRoot)
	}
}
