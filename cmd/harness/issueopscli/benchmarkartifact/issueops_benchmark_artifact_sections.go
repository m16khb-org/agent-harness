...
func prDraft(fixture issueopscore.IssueOpsBenchmarkFixture, issueNumber, branchName, worktreePath, guideline, expectedIssue, expectedPR string) string {
	return strings.Join([]string{
		...
		"## 리뷰어 초점",
		"",
		"- reviewer focus: remote artifact template validation, Korean body gate, label/assignee evidence, and target branch correctness.",
		"",
		"## 위험/rollback",
		"",
		"- risk: benchmark 또는 원격 artifact gate가 과도하게 엄격해질 수 있다.",
		"- rollback: renderer/template 변경을 되돌리고 기존 dry-run body path로 복귀한다.",
		"",
		"## 사용자 영향/릴리즈 노트",
		"",
		"- user impact: 에이전트가 생성하는 issue/PR/MR 본문 품질과 검증 가능성이 좋아진다.",
		"- release note: IssueOps remote template contract tightened.",
		"",
		"## 문서/마이그레이션",
		"",
		"- documentation: skills/issueops references and repo-native templates must match the canonical sections.",
		"- migration: 기존 IssueOps state schema는 변경하지 않는다.",
		"",
		"## 범위 관리",
		"",
		"- scope management: provider adapter는 thin path로 유지하고 template policy는 core에 둔다.",
		"",
		"## 워크트리 정리",
		"",
		"- worktree cleanup: cleanup status and choices remain required after merge.",
		"",
		"## 자동화/AI 개입 근거",
		"",
		"- automation evidence: renderer와 remote gate 결과를 기준으로 본문을 생성하고 검증한다.",
		"",
