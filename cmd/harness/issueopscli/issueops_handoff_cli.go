...
const issueOpsHandoffUsage = `Usage:
  ...
  agent-harness issueops handoff publish --id ID --host HOST --session-id SESSION --cwd WORKER_PATH [--agent-id ID] --confirm [--json]
  agent-harness issueops handoff recover --id ID --action reconcile|abandon|cancel|finalize-cancel [--confirm] [--force --reason TEXT] [--json]`
...
func runIssueOpsHandoffPublish(args []string) error {
	...
	cwd := fs.String("cwd", "", "canonical owner worktree cwd")
	...
	record, err := core.RecordIssueOpsHandoffPublishReceipt(context.Background(), core.IssueOpsStateRoot(), core.IssueOpsHandoffPublishRequest{
		ID: *id, Host: *host, SessionID: *sessionID, AgentID: *agentID, CWD: *cwd, Confirm: *confirm,
...
func runIssueOpsHandoffComplete(args []string) error {
	...
	record, err := core.CompleteIssueOpsHandoffWithProjection(context.Background(), core.IssueOpsStateRoot(), core.IssueOpsHandoffCompleteRequest{
		ID: *common.id, Attempt: *common.attempt, OwnershipEpoch: *common.epoch, ContextSHA256: *common.context,
...
func runIssueOpsHandoffRecover(args []string) error {
	...
	id := fs.String("id", "", "issueops id")
	action := fs.String("action", "", "reconcile, abandon, cancel, or finalize-cancel")
	confirm := fs.Bool("confirm", false, "confirm abandonment or cancellation finalization")
