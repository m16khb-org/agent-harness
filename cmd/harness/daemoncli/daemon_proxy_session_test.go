...
func TestDaemonProxyResumeRejectsChangedHandshakeContract(t *testing.T) {
	...
	connection := daemonProxyTestConnection(
		`{"jsonrpc":"2.0","id":"agent-harness-reconnect-1","result":{"protocolVersion":"2025-11-25","capabilities":{"tools":{"listChanged":true}}}}` + "\n",
	)
	...
	_, err := reconnectDaemonProxy(daemonProxyDeps{
		...
		dial: func(context.Context, string, string) (io.ReadWriteCloser, error) {
			...
			return &daemonProxyFakeConn{
				reader: strings.NewReader(`{"jsonrpc":"2.0","id":"agent-harness-reconnect-1","result":{"protocolVersion":"2025-06-18"}}` + "\n"),
			}, nil
		},
...
func TestReconnectDaemonProxyRejectsHandshakeCompletedAfterHostEOF(t *testing.T) {
	hostDone := make(chan struct{})
	response := []byte(`{"jsonrpc":"2.0","id":"agent-harness-reconnect-1","result":{"protocolVersion":"2025-06-18","capabilities":{"tools":{"listChanged":true}}}}` + "\n")
