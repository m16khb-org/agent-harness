...
func TestRunHookPreToolUseSupervisedMultiCycleControlPlaneMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "control lookalike", tool: "functions__update_goal", input: map[string]any{}, want: "allow"},
		...
		{name: "synthetic write stdin", tool: "write_stdin", input: map[string]any{"session_id": "unbound", "chars": `{"method":"hooks/list"}`}, want: "allow"},
		{name: "synthetic config batch write", tool: "write_stdin", input: map[string]any{"session_id": "unbound", "chars": `{"method":"config/batchWrite"}`}, want: "allow"},
