	...
	for i, line := range lines {
		...
		if optional {
			if !strings.Contains(deco, "@ApiPropertyOptional") {
				violations = append(violations, StaticViolation{File: file, Line: i + 1, Code: "missing_api_property_optional", Message: "optional DTO property " + m[1] + " is missing @ApiPropertyOptional"})
			}
			if !strings.Contains(deco, "@IsOptional") {
				violations = append(violations, StaticViolation{File: file, Line: i + 1, Code: "missing_is_optional", Message: "optional DTO property " + m[1] + " is missing @IsOptional"})
			}
		} else if !strings.Contains(deco, "@ApiProperty") || strings.Contains(deco, "@ApiPropertyOptional") {
			violations = append(violations, StaticViolation{File: file, Line: i + 1, Code: "missing_api_property", Message: "required DTO property " + m[1] + " is missing @ApiProperty"})
		}
		decorators = nil
		classDepth += braceDepthDelta(line)
		if classDepth <= 0 {
