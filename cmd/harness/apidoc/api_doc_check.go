...
func runAPIDocCheck(args []string) error {
	...
	result, err := runAPIDocCheckWithOptions(apiDocStaticOptions{Repo: root, Files: fs.Args(), All: *all, JSON: true}, apiDocReviewOptions{Repo: root, Files: fs.Args(), All: *all, DiffFile: *diffFile, PromptFile: *promptFile, ResultFile: *resultFile, JSON: true})
	if *jsonOut {
		_ = printJSON(result)
		return err
	}
