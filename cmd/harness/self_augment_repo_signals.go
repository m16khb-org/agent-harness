...
func collectSelfAugmentRepoSignals(root string, docsIndexed int, skills []string, geniusText string) SelfAugmentRepoSignals {
	return SelfAugmentRepoSignals{
		...
		HasRepoLocalSandbox:         dirContainsTerm(root, filepath.Join("internal", "core"), "path_outside_workspace") && fileContainsTerm(root, filepath.Join("internal", "core", "policy_test.go"), "TestCommandPolicyDeniesPathArgsOutsideWorkspace") && dirContainsTerm(root, filepath.Join("cmd", "harness"), "policy deny outside path arg"),
