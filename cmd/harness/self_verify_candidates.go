...
func selfVerificationCandidateCatalog() []SelfVerificationCandidate {
	return []SelfVerificationCandidate{
		...
		selfVerificationCandidate(4, "completion-evidence-audit", "completion evidence", 80, selfAugmentCandidateStatusOpen, []string{"LangChain-style harness engineering emphasizes forcing verification evidence before completion", "agent-harness has verify-work, but completion reports can still omit structured evidence"}, []string{"verify-work --json fixture", "self-verify candidates --json", "workflow doc completion-report check"}, nil),
		...
		selfVerificationCandidate(8, "self-verify-install-dry-run-smoke", "native integration", 76, selfAugmentCandidateStatusSatisfied, []string{"install-native --dry-run should be an independent no-write evidence label"}, []string{"temp HOME smoke", "dry-run no-write assertion"}, []string{"install dry-run smoke runs install-native --dry-run against temp HOME/CODEX_HOME/HARNESS_ROOT and asserts planned writes/links without filesystem mutations"}),
