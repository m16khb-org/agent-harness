...
func TestSelfWorkflowCandidateExportAndStateWrappers(t *testing.T) {
	...
	t.Cleanup(func() { HarnessRoot = restore })

	result := ExportSelfVerificationCandidates()
	if !result.OK || !result.SourceExists || result.HarnessRoot != root {
...
func TestSelfWorkflowAugmentWrapperHelpers(t *testing.T) {
	root := t.TempDir()
	docsDir := filepath.Join(root, ".agent-harness")
	...
	if !DocsContainTerm(root, "coverage") || !DirContainsTerm(root, ".agent-harness", "qualitySignal") || !FileContainsTerm(root, ".agent-harness/TESTING.md", "self-verify") {
...
func TestSelfWorkflowSummaryAndHistoryWrappers(t *testing.T) {
	...
	if result.Iterations != 1 || result.HarnessRoot == "" {
