...
func TestValidateDaemonRestartResilienceWithDepsCoversSuccessAndSetupFailure(t *testing.T) {
	...
	deps := daemonResilienceValidationDeps{
		...
		writeFile: func(path string, content []byte, _ os.FileMode) error {
			if strings.HasSuffix(path, "agent-harness.sock") {
	...
	step := validateDaemonRestartResilienceWithDeps("bin/agent-harness", root, 777, deps)
...
func writeDaemonResilienceFakeBinary(t *testing.T, dir string) string {
	...
	body := `#!/bin/sh
set -eu
dir="${HARNESS_DAEMON_DIR:?}"
socket="$dir/agent-harness.sock"
pid="$dir/agent-harness.pid"
lock="$dir/agent-harness.lock"
log="$dir/agent-harness.log"
status_running() {
