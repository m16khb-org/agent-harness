...
- 하네스 철학은 **바퀴를 재발명하지 않는다**이다. llm-wiki, CodeGraph, claude-mem, LazyCodex 같은 전문 도구는 `scripts/install-native.sh --with-upstream-tools`로 upstream installer/plugin을 연결하고, core 동작을 agent-harness에 복제하지 않는다.
